...
B) In the absence of sharing, the statement is trivially true since we would have $LN_{ops_2}' > LN_{ops_2}$ and we are starting from the assumption that pre-evaluation is optimal for the monomials considered. As opposed to this extreme case, there could be some degree of sharing between monomials in $P_Z$ an $\overline{P}$. However, as clarified in the next section, this situation is not possible: the cost function "sees sharing" and it would prevent pre-evaluation by counting on sharing-removal for optimality. Finally, partial sharing. Note that by not pre-evaluating $P_Z$, the following: $LN_{ops_1} - LN_{ops_1}' < LN_{ops_2}' - LN_{ops_2}$ trivially holds (i.e., the loss in operation count is larger than the gain). At best, once sharing is remove, 
\end{proof}
