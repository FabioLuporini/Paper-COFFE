...
\label{sec:se-rln}
Finite element expressions can be seen as composition of operations between tensors. Often, the optimal implementation strategy for these operations is to be determined out of two alternatives. To illustrate the concept, consider $J^{-1} \nabla x \cdot J^{-1} \nabla x$, with $J^{-1}$ being the inverse Jacobian matrix for the change of coordinates and $x$ a generic two-dimensional vector. The tensor operation will reduce to $(a x^0_i + b x^1_i) (a x^0_i + b x^1_i) + ...$, in which $x^0_i$ and $x^1_i$ represent components of $x$ that depend on loop $L_i$. To minimize the operation count for sub-expressions of this kind, we have two options.
\begin{Strategy}
\label{strategy:i}
...
\end{Strategy}
In the current example, we observe that, depending on the size of $L_i$, applying Strategy~\ref{strategy:ii} could reduce the operation count, since the expression would be recast as $x^0_i x^0_i a a + x^0_i x^1_i (ab + ab) + x^1_i x^1_i c c + ...$ and some hoistable sub-expressions would be exposed. On the other hand, Strategy~\ref{strategy:i} would have no effect as  $x$ only depends on a single loop, $L_i$. In general, the choice between the two strategies depends on multiple factors: the loop(s) size, the increase in operation count due to expansion (in Strategy~\ref{strategy:ii}), and the gain in code motion. Note that a second example of application of Strategy~\ref{strategy:ii} was provided in Figure~\ref{code:multi_loopnest}. These examples motivate the introduction of a particular class of expressions.
\begin{Def}[Structured expression]
\label{def:struct-expr}
An expression is \textit{structured} with respect to a set of loops $\Lambda$ if and only if there is no ambiguity in the elimination of spatial sharing for the symbols depending on $\Lambda$.
\end{Def}
This leads us to the following, fundamental observation.
\begin{Prop}
\label{prop:multi-struct}
In a multilinear loop nest $\Lambda$, all of the expressions are structured with respect to $\Lambda$.
\end{Prop}
In finite element integration loop nests, for example, any factorization of test and trial functions will always result in exposing temporal sharing (Strategy~\ref{strategy:ii}). Conversely, in a generic loop nest $\Lambda$, the instances of a symbol depending on $\Lambda$ may appear everywhere (e.g., twice in a summand and only once in all other summands; as argument of a higher level function; in the denominator of a division), thus posing the challenge of finding the optimal factorization that maximizes temporal sharing. Proposition~\ref{prop:multi-struct} played a key role in the development of Algorithm~\ref{algo:sharing-elimination}. Finally, we observe that structured sub-expressions sometimes characterize the \textit{entire} variational form (for instance, in the complex hyperelastic model analyzed in Section~\ref{sec:perf-results}).
...
\textit{Note: $|P|$ and $|S_P|$ represent the number of products in the innermost loop induced by $P$ if, respectively, Strategy~\ref{strategy:i} or Strategy~\ref{strategy:ii} were applied. This is a consequence of the expression being structured with respect to $[L_j, L_k]$.}
...
\textit{Note: this pass speculatively assumes that all expressions are structured. If the assumption does not hold, the operation count within the reduction loop will generally be sub-optimal because only a subset of factorizations and code motion opportunities may eventually be considered.}
\end{enumerate}
\end{Algo}

\begin{Rem*}
\normalfont
Although the primary goal of Algorithm~\ref{algo:sharing-elimination} is operation count minimization within the multilinear loop nest, the enforcement of f-decreasing transformations (step 5) and the re-scheduling of sub-expressions within outer loops (step 7) also attempt to optimize the loop nest ``globally''. We will further elaborate this aspect in Section~\ref{sec:proof}.
\end{Rem*}
