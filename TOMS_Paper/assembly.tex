...
\end{enumerate}
\item Consider Algorithm~\ref{algo:sharing-elimination}. Based on loop linearity and normal form of input expressions, we observe that there are only two ways of scheduling the multilinear operands in $P \in \mathbb{P}$: through (i) generalized code motion or (ii) factorization of multilinear symbols, respectively leading to $|P|$ and $|S_P|$ multiplications. An upper bound for $P$ is, therefore, $ub_P = 2{min(}|P|,\ |S_P|{)} - 1$. Thus, when generalized code motion in step (3) is applied, we do know that we are not pruning any potentially optimal solution from the search space. In fact, we are just moving towards the lower bound, given by $lb_P = min(0, ub_P)$, with $0$ reached only if the symbols in $P$ will be grouped and hoisted at step (4). 

Step (4) factorizes multilinear symbols and temporaries produced at step (3) in optimal order; that is, an order that leads to a quasi-optimal synthesis. To demonstrate this, we consider the sharing graph rewrite rules.
\begin{enumerate}[label=(\Alph*)]
\item Rule (A) has no side-effects since $\lbrace T^0, ..., T^n \rbrace$ can only be grouped through the factorization of $S$.
\item By contradiction, we can prove that applying rule (B) is always a step forward towards the optimal solution, assuming (as stated in Algorithm~\ref{algo:sharing-elimination}) no hit is possible for rule (A). Assume then that (B) prunes the path towards quasi-optimality by choosing a sub-optimal factorization. This means that there exists ...
\item Rule (C) ...
\end{enumerate}
