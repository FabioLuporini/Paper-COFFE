...
\begin{gather*}
x_i \text{: a vertex in } S \text{ (1 if a symbol must be factorized, 0 otherwise)}\\
y_{ij} \text{: a direct edge in } E \text{ (1 if the product } s^i s^j \text{ is factorized through } s^i \text{, 0 otherwise)}\\
n_i \text{: the number of edges incident to } x_i \\
\begin{align*}
\min \sum_{i=1}^{n} x_i,\ s.t. ~~~~~~~~~&\sum_{j=1}^{n} y_{ij} \leq n_i x_i,\ i = 1, ..., n \\
& y_{ij} + y_{ji} = 1,\ i, j = 1, ..., n
\end{align*}
\phantom{\hspace{6cm}}
\end{gather*}
...
After structuring the sharing graph $G$ in such a way that only flop-decreasing transformations are possible, the ILP model is instantiated. At this point, the optimality of the algorithm reduces to prove the correctness of the model. The model aims ...
\end{enumerate}
\end{proof}
