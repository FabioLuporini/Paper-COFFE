...
For instance, Figure~\ref{code:multi_loopnest_a} shows a trivial outer product loop nest of arity $n=2$ with sharing along dimension $j$ induced by $b_j$. The optimal synthesis for this loop nest is provided in Figure~\ref{code:multi_loopnest_b}. This example illustrates how factorization can be applied to expose code motion opportunities in multilinear loop nests. 

In this section, we study an optimal transformation to perform \textit{sharing elimination}; that is, a transformation that minimizes the operation count of a loop nest by applying factorization, code motion and (potentially) common sub-expression elimination. In general, transformations of this sort require solving large combinatorial problems; for instance, to evaluate the impact of all possible factorization strategies. However, we show that by restricting ourselves to the class of loops and expressions of Definition~\ref{def:fem-loopnest}, the search space can drastically be pruned. In the following, we assume outer product loop nest of arity $n=2$ (i.e., a bilinear form), although the same reasoning stands for the case $n < 2$ (which is actually just a special case).

The first observation is that finite element expressions can always be reduced to the form $\sum_{w=1}^m \alpha_{eij}^{w} \beta_{eik}^{w} \sigma_{ei}^{w}$, where $m$ depends on the specific form and discretization, $\alpha_{eij}$ ($\beta_{eik}$) represents the product of the inverse Jacobian matrix with test (trial) functions, and $\sigma_{ei}$ is a function of the coefficients. 

Given the peculiar structure of our terms, the key idea is that, in the space $S$ of all possible factorizations, the optimum almost always lies in a tiny subspace (typically of size one). We refer to this space as the \textit{potential search space} ($PSS$). Our approach is therefore as follows: we calculate lower and upper bounds (respectively, $lb$ and $ub$) for families of factorizations and we compare them: if, as expected, $ub(PSS) < lb(\overline{PPS})$, we prune $\overline{PPS}$. Otherwise, we have to search in $\overline{PPS}$ too. 

A potentially optimal factorization is one that groups together test and trial functions. Such a factorization exposes sub-expressions independent of at least one loop and, therefore, hoistable. The converse is not true: factorizing $\sigma_{ei}$ (or any of the terms in it) generally inhibits hoisting of sub-expressions including test and trial functions. Algorithm~\ref{algo:pps} provides a succinct description of how $PPS$ is built.

\begin{Algo}[Construction of the potential search space]
\label{algo:pps}
\normalfont
...
The sub-expressions $\alpha_{eij}^{w} \beta_{eik}^{w}$ are ``flattened'' (by multiplication expansion) such that all $L_j$- and $L_k$-dependent terms lie on the same level of the expression tree. We factorize, in sequence, $L_j$- ($L_k$-) and $L_k$- ($L_j$-) dependent terms (it can easily be demonstrated, by contradiction, that one should start from the loop with the least number of unique terms in order to obtain the smallest possible operation count). Thanks to linearity, each factored product only has one symbol depending on $L_j$ ($L_k$), and this symbol is now unique in the expression. The other sub-expression, independent of $L_j$ ($L_k$), is, by definition, loop-invariant, and therefore hoisted such that redundant computation is avoided. Note that multilinearity ensures deterministic factorization, while perfectness ensures correctness of hoisting.
\end{Algo}

%A special case is when test and trial functions belong to the same function space: this induces common sub-expressions \textit{over different loops}, so a potentially winning strategy consists of hoisting both $\alpha_{eij}$ and $\beta_{eik}$ and exploiting the fact that $\alpha = \beta$. ---- If we note that, for some $w$, $\alpha = \beta$, we add the following strategy to $PPS$: hoist $\alpha_{eij}^{w}$ (or, equivalently, $\beta_{eik}^{w}$); replace common sub-expressions; minimize cost of $\sigma_{ei}$, which we discuss later. 
...
Sharing elimination aims to reduce the operation count by applying factorization, code motion and/or common sub-expression elimination. There is a fourth operator impacting the operation count: \textit{reduction pre-evaluation}. In this section, we discuss role and impact of this operator. We also show that there exists a single point ($\in \overline{PPS}$) in the whole transformation space for which reduction pre-evaluation is legal. 
