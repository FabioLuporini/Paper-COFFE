...
concept of a \textit{linear loop}.

\begin{Def}[Linear loop]
A loop $L$ defining the iteration space $I$ through the iteration variable
$i$, or simply $L_i$, is \textit{linear} if 
\begin{enumerate}
\item $i$ appears in the body of $L$
only as an array index, and
\item whenever an array $a$ is indexed by $i$
($a[i]$), all expressions in which this appears are affine in $a$.
\end{enumerate}
\end{Def}

In particular, we are interested in the following class since it naturally arises in finite element integration.
...
Assume $\Lambda = [L_{i_{0}}, L_{i_{1}}, ..., L_{i_{n-1}}]$ is a perfect multilinear loop nest with the body of $L_{i_{n-1}}$ being an independent basic block. Then sharing can always be eliminated from $\Lambda$.
\end{Prop}
\begin{proof}
  The demonstration is by construction and exploits linearity. We want to
  transform $\Lambda$ into $\Lambda'$ such that there is no sharing in any
  $L_i \in \Lambda'$. Starting from the innermost loop $L_{i_{n-1}}$, the
  expressions are ``flattened'' by expanding all products involving terms
  depending on $L_{i_{n-1}}$. Being now on the same level of the expression
  tree, such terms can be factorized. Due to linearity, each factored
  product only has one term depending on $L_{i_{n-1}}$, and this term is now
  unique in the expression. The other terms, independent of $L_{i_{n-1}}$,
  are, by definition, loop-invariant, and as such can be hoisted at the
  level of $L_{i_{n-2}}$. This procedure can be applied recursively up to
  $L_{i_{0}}$: multilinearity allows factorization at each level;
  perfectness ensures hoisting is safe.
\end{proof}
...
The synthesis of a multilinear loop nest is optimal if the number of
operations performed in the loop nest is minimal.
\end{Def}

%We will see that the restriction to inner loops has the effect of simplifying the transformation model; the price to pay will be the generation of quasi-optimal code in presence of costly outer loops (this, in finite element integration, happens when forms are characterized by extensive use of tensor algebra).

Note that Definition~\ref{def:mln-optimality} does not take into account
memory requirements. If the loop nest were memory-bound -- the ratio of
operations to bytes transferred from memory to the CPU being too low -- then
speaking of optimality would clearly make no sense. In the following, we
assume to operate in a CPU-bound regime, in which arithmetic-intensive
expressions need be evaluated. In the context of finite element, this is
often true for more complex multilinear forms and/or higher order elements.

%\begin{Rem*}
%In the following, when discussing ``optimality'' or ``loop nests in optimal form'', we will always tacitly refer to inner optimality (unless explicitly specified differently). 
%\end{Rem*}

As anticipated, perfect multilinear loop nests can be brought to optimal form. The following result proves this claim.

\begin{Prop}
\label{prop:optimal-mln}
A perfect multilinear loop nest $\Lambda$ with basic block independence can always be reduced to optimal form.
\end{Prop}
\begin{proof}
By construction. Loop-dependent terms are logically grouped into $n$
disjoint sets $S_i$, each $S_i$ containing all terms depending on
$L_i$. These sets are sorted in descending order based on their
cardinality. We produce a new loop permutation that reflects the ${S_i}$
ordering; that is, loops are placed such that each loop has fewer unique
terms than the outer loops containing it. Since the loop nest is perfect,
any such permutation is semantically correct.  Starting from $L_{i_{n-1}}$, we can apply the sharing-removal procedure described in Proposition~\ref{prop:share-removal}. This renders $L_{i_{n-1}}$ optimal. In particular, the number of operations is equal to $\Lambda_{ops} = \# S_{i_{n-1}} + (\# S_{i_{n-1}} - 1)$, in which the second term represents the cost of summing the $\# S_{i_{n-1}}$ terms.
\end{proof}
...
It is then obvious that reaching an optimal form $\Lambda'$ is, in the general case, a challenging issue, since a full exploration of the expression tree transformation space is necessary. Even though we assumed we could generate $\Lambda'$ through a suitable transformation algorithm, the following issues should be addressed
\begin{itemize}
\item as opposed to what happens with hoisting in perfect multilinear loop nests, the temporary variable size would be proportional to the number of non-reduction loops crossed (in the example of Figure~\ref{code:loopnest}, $N \cdot O$ for sub-expressions depending on $[L_i, L_j, L_k]$ and $L \cdot N \cdot O$ for those depending on $[L_e, L_i, L_j, L_k] $). This might shift the loop nest from a CPU-bound to a memory-bound regime, which might be counter-productive for actual runtime
\item the transformations exposing sub-expressions amenable to pre-evaluation could increase the arithmetic complexity within $L_k$ (e.g., expansion may increase the operation count; further examples will be provided later). This could overwhelm the gain inherent in pre-evaluation.
\end{itemize}

Before attacking this problem in Section~\ref{sec:optimal-synthesis}, we shall conclude our construction refining Definition~\ref{def:mln-optimality} for generic loop nests

\begin{Def}[Inner optimality of a loop nest]
\label{def:optimality}
The synthesis of a loop nest is optimal if, under a set of memory constraints \texttt{C}, the total amount of operations performed in all innermost loops is minimum.
\end{Def}
...
\section{Optimal-form Loop Nests in Finite Element Integration}
...
A second, more general fact is that, for certain forms and discretizations, excessive hoisting can make the working set exceed the size of ``some level of local memory'' (e.g. the last level of private cache on a conventional CPU, the shared memory on a GPU). For example, applying tensor contraction mode, which essentially means pre-evaluating geometry-independent expressions outside of $\Lambda$, requires temporary arrays of size $N\cdot O$. This can sometimes break the local memory threshold. We will clarify and experiment this in Section~\ref{sec:perf-results-forms}.

Based on these considerations, we characterize the set of memory constraints $C$ (see Definition~\ref{def:optimality}) as follows
\begin{enumerate}
...
\subsection{Minimizing the Operation Count in Inner Loops}
Definition~\ref{def:optimality} states that a necessary condition for a loop nest synthesis to be optimal is that the sum of the innermost loop operation counts is minimum. We now discuss how we can systematically achieve this.
...
We can easily predict $\theta_{wo}$ thanks to our key property, linearity. This was explained in Proposition~\ref{prop:optimal-mln}. A simple analysis suffices to obtain the cost of a  sharing-free multilinear loop nest, namely $\Lambda_{ops}^{sf}$. Assuming $I$ to be the size of the $L_i$ iteration space, we have that $\theta_{wo} = \Lambda_{ops}^{sf} \cdot I$.
