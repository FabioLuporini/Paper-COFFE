...
The restriction to f-decreasing transformations aims to exclude those ``deceptive'' solutions that, to achieve flop-optimal innermost loops, would rearrange the computation at the level of the outer loops causing, in fact, a ``global'' increase in operation count. 
...
\end{Strategy}
In the current example, we observe that, depending on the size of $L_i$, applying Strategy~\ref{strategy:ii} could reduce the operation count since the expression would be recast as $x^0_i x^0_i a a + x^0_i x^1_i (ab + ab) + x^1_i x^1_i c c + ...$ and some hoistable sub-expressions would be exposed. On the other hand, Strategy~\ref{strategy:i} would have no effect as  $x$ only depends on a single loop, $L_i$. In general, the choice between the two strategies depends on multiple factors: the loop(s) size, the increase in operation count due to expansion (in Strategy~\ref{strategy:ii}), and the gain in code motion. A second application of Strategy~\ref{strategy:ii} was provided in Figure~\ref{code:multi_loopnest}. With the objective of characterizing when these two strategies suffice to reach quasi-optimality, we introduce a particular class of expressions and its relationship with multilinear loop nests.
\begin{Def}[Structured expression]
\label{def:struct-expr}
We say that ``an expression is structured along a loop nest $\Lambda$'' if and only if for eliminating the spatial sharing induced by any $s_{\Lambda}$ -- a generic symbol depending on at least one loop in $\Lambda$ -- it is sufficient to factorize all occurrences of $s_{\Lambda}$ in the expression.
\end{Def}
\begin{Prop}
...
\end{Prop}
\begin{proof}
This is an obvious consequence of multilinearity.
\end{proof}
If $\Lambda$ were an arbitrary loop nest, a given symbol $s_{\Lambda}$ could appear everywhere (e.g., $n$ times in a summand and $m$ times in another summand with $n \neq m$, as argument of a higher level function, in the denominator of a division), thus posing the challenge of finding the factorization that maximizes temporal sharing. If $\Lambda$ is instead a finite element integration loop nest, thanks to Proposition~\ref{prop:multi-struct} we can build the space of f-decreasing transformations for quasi-optimality by just composing Strategy~\ref{strategy:i} and Strategy~\ref{strategy:ii}. The challenge for Algorithm~\ref{algo:sharing-elimination} will be to select the best f-decreasing transformations.

Finally, we observe that the $\sigma_{ei}$ sub-expressions are sometimes ``weakly structured''; that is, Definition~\ref{def:struct-expr} applies to them by replacing spatial sharing ``elimination'' with ``minimization''. This happens, for instance, in the complex hyperelastic model analyzed in Section~\ref{sec:perf-results}, and intuitively explains why under some circumstances (elaborated in Section~\ref{sec:proof}) Algorithm~\ref{algo:sharing-elimination} is capable of reaching actual optimality.
...
\end{enumerate}
\item Consider Algorithm~\ref{algo:sharing-elimination}. Proposition~\ref{prop:multi-struct} states that there are only two ways of scheduling the multilinear operands in $P \in \mathbb{P}$: through generalized code motion (Strategy~\ref{strategy:i}) or factorization of multilinear symbols (achieved through Strategy~\ref{strategy:ii}). If applied, these two strategies would lead, respectively, to performing $|P|$ and $|S_P|$ multiplications at every loop iteration. Since Strategy~\ref{strategy:i} is applied if and only if $|P| < |S_P|$ and does not change the structure of the expression (it requires neither expansion nor factorization), step (3) cannot prune the optimal solution from the search space. 
