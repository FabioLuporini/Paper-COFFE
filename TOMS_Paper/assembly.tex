...
\item Calculate the number of operations in the innermost loop required for the execution of $P \in \mathbb{P}$. There are two possibilities, depending on which strategy -- (i) or (ii), as described in Section~\ref{sec:se-rln} -- is employed. In particular, we have $c_P = {min(}c_P^{i},\ c_P^{ii}{)}$, with $c_P^i = |P|$ and $c_P^{ii} = |S_P|$.
\item For all multilinear operands $o \in P$ such that $c_P = c_P^i$, apply strategy (i). Update $\mathbb{P}$ by replacing hoisted multilinear operands with the corresponding temporary symbols.
\item For all $P \in \mathbb{P}$, determine the profitability of strategy (ii). We introduce $g_P$, which measures the gain in operation count in the innermost loop if (ii) were applied. That is, $g_P = \sum_{Q \in \mathbb{Q}} c_{Q_{old}} - c_{Q_{new}}$, with $\mathbb{Q} = \mathbb{P} \setminus \lbrace P \rbrace$. Note that $c_{Q_{new}}$ can be evaluated without applying the actual transformation. Eventually, apply (ii) if $g_P > 0$.
\item Perform a depth-first visit of the expression tree and, for each $L_i$-dependent or previously hoisted sub-expression, apply (i) or (ii). This pass speculatively assumes that structured tensor operations are in place. If the assumption does not hold, the result will generally be sub-optimal since only a subset of code motion opportunities may have been exposed.
\end{enumerate}
\end{Algo}
...
%|\Lambda| + \sum_{s \in S_P} (n_{s} - 1)$. Note that:
%\begin{itemize}
