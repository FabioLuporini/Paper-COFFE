...
\begin{enumerate}
\item $i$ appears only as an array index, and
\item for each augmented assignment statement $S$ (e.g., increments), arrays indexed by $i$ appear only on the right hand side of $S$.
\end{enumerate}
\end{Def}

\begin{Def}[Free space loop]
A loop $L_i$ is said to be a free space loop if its iterations can be executed in any arbitrary order; that is, no dependencies exist across different iterations. 
\end{Def}

Altogether, these definitions lead us to formalize the class of loop nests for which we seek optimality.

\begin{Def}[Finite element integration loop nest]
A finite element integration loop nest is a loop nest in which we identify, in order, an imperfect free space loop, a (generally) imperfect, linear or non-linear reduction loop, and an outer product loop nest.
\end{Def}

To contextualize, consider Equation~\ref{eq:quadrature} and the (abstract) loop nest implementing it illustrated in Figure~\ref{code:loopnest}. The imperfect nest $\Lambda=[L_e, L_i, L_j, L_k]$ comprises a free space loop $L_e$ (over the elements in the mesh), a reduction loop $L_i$ (along which numerical integration is performed), and an outer product loop nest $[L_j, L_k]$ (over test and trial functions). The expression \texttt{F} implements the operator. 
...
\begin{itemize}
\item a characterization of the space of legal transformations for the class of loop nests considered impacting the operation count
\item a cost model to select the optimal point in the transformation space
\end{itemize}
In this section, we construct the transformation space. We leave a few loose ends, which we progressively tie up in Section~\ref{sec:optimal-synthesis}.
...
\end{CenteredBox}\caption{Reducing a simple outer product loop nest to optimal form.}\end{figure}
...
\end{CenteredBox}\caption{Exposing (through factorization) and pre-evaluating a reduction.}\end{figure}
...
We expand on these points in Section~\ref{sec:optimal-synthesis}. In order to address point 2, we conclude this section by providing a more refined definition of optimality.
...
Note that due to the memory constraints imposed in the previous section, sharing elimination and pre-evaluation fully describe the transformation space in which the optimal is searched. We consider them high level operators defined on top of expression-restructuring transformations such as code motion, re-association, and factorization. Other classes of loop or expression transformations, for instance aimed at improving data locality or exposing parallelism, can obviously be neglected since they do not impact the operation count.

Finally, with Proposition~\ref{prop:optimal-approach}, we elaborate on the optimality of this approach. 
