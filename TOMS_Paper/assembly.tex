...
\item $\lbrace T^1, ..., T^n \rbrace \subset \mathbb{T}$ adjacent to $S$, $n > 1$: apply strategy (ii); $\mathbb{S} = \mathbb{S} \setminus \lbrace T^1, ..., T^n \rbrace$.\\ \textit{Note: this recasts the expression as $S (...T^1 + ... + ...T^n)$}. 
\item Only one terminal $T$ adjacent to $S$: apply strategy (ii) including one non-terminal symbol $S^{nt}$. $\mathbb{S} = \mathbb{S} \setminus \lbrace T, S^{nt} \rbrace$.
\item $\mathbb{T} = \emptyset$: take $S$ with highest degree and apply strategy (ii).
\end{enumerate}
Go back to step (2).
...
\end{enumerate}
\item Consider Algorithm~\ref{algo:sharing-elimination}. Because of the normal form of expressions and the structure inherent in tensor operations involving test and trial functions, there are only two ways of scheduling the multilinear operands in $P \in \mathbb{P}$: through generalized code motion (strategy (i)) or factorization of multilinear symbols (achieved through strategy (ii)). If applied, these two strategies would lead, respectively, to performing $|P|$ and $|S_P|$ multiplications at every loop iteration. Since strategy (i) is applied if and only if $|P| < |S_P|$ and requires neither expansion nor factorization, step (3) will not prune any optimal solution from the search space; in fact, it simply represents a step forward towards quasi-optimality. 

Step (6) factorizes symbols following the ordering dictated by the sharing graph and its rewrite rules. Non f-decreasing factorizations are excluded by construction (step (5)). We now have to prove that applying in sequence rewrite rules (A), (B), and (C), with applications of rule (C) that may expose further opportunities for (A) and (B), always represents progress towards quasi-optimality. 
\begin{description}
\item[Rule (A)] $\lbrace T^0, ..., T^n \rbrace$ can only be grouped through the factorization of $S$, so this rule is side effects free (i.e., no need to choose between different factorizations).
\item[Rule (B)] Since $S^{nt}$ is no terminal, there must exist at least one edge between $S^{nt}$ and a generic vertex $V$. Let us consider a generic alternative factorization $S^{nt}(...S + ... + ...V)$. If $V$ were a terminal, we would be back to where we started, because rule (B) with $V = T$ and $S = S^{nt}$ would still be applicable. Otherwise ($V$ non terminal), we would have $S$ appearing in two different sub-expressions, which would imply sub-optimality as a consequence of spatial sharing.
\item[Rule (C)] We can interpret $G$ as a bipartite graph $\mathbb{B} = (\mathbb{S}_j, \mathbb{S}_k, \mathbb{E})$, in which $\mathbb{S}_j$ and $\mathbb{S}_k$ represent the partitions of symbols depending on $L_j$ and $L_k$, respectively. For each disconnected component $\mathbb{B}^d = (\mathbb{S}_j^d, \mathbb{S}_k^d, \mathbb{E}^d)$ in $\mathbb{G}$, we consider two possibilities, namely $\mathbb{B^d}$ 1) is not or 2) is complete. Case 1) is the simplest: since, at this point, there are no more hits for rules (A) and (B), the lower bound on the operation count (as number of products performed at every loop iteration), $lb$, is given by $lb = {min(}|\mathbb{S}_j^d|, |\mathbb{S}_k^d|{)}$. This is reachable by factorizing the vertex with highest degree, as any other choice may break factorization opportunities. In case 2), we have $lb = 1$ because factorization is applicable iteratively (e.g., $a_j a_k + a_j b_k + b_j a_k + b_j b_k = (a_j + b_j)(a_k + b_k) = t_j t_k$); this is handled by going back to step (2), at the end of step (6).
\end{description}
The algorithm is guaranteed to terminate because of the jump to step (6) when $\mathbb{S} = \mathbb{T}$. 
\end{enumerate}
\end{proof}

%TODO : In addition, if X holds, then $\Lambda'$ is optimal as established by Definition ?.
%TODO : A list of preconditions that eventually result in optimality
