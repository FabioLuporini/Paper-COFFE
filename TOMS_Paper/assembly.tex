...
\end{itemize}
In the following Sections~\ref{sec:sharing-elimination} and~\ref{sec:pre-evaluation}, we focus on the construction of such a transformation space. We consider multilinear loop nests of arity $n=2$ (i.e., bilinear forms), although a similar (simpler) reasoning straightforwardly applies to the case $n < 2$.
...
\begin{itemize}
\item For all terms $w$, there exists a common symbol $f_i \in \sigma_{ei}$ (e.g., a scalar coefficient) such that the normal form reduces to $f_i \sum_{w=1}^m \alpha_{eij}^{w} \beta_{eik}^{w} \eta_{ei}^{w}$ and $\eta_{ei} = \sigma_{ei} \setminus \lbrace f_i \rbrace$. 
