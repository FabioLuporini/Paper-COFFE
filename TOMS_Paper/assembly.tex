...
\begin{enumerate}
\item Assume $\alpha \neq \beta$; that is, test and trial functions belong to different function spaces. By factorizing, in sequence, any $a_{ij} \in \alpha_{eij}$) and then any $b_ik \in \beta{eik}$) (or vice-versa), sub-expressions independent of one of the outer product loops are exposed. This systematically exposes all code motion opportunities to minimize the operation count in the innermost loop.
\item In the special case $\alpha = \beta$, common sub-expressions arise over different iteration spaces ($ij$ and $ik$). Hoisting and capturing these sub-expressions can make the operation count of outer loops lower than in point 1), but the gain (if any) is, in practice, always negligible (note: the operation count in the innermost loop does not vary).
\item Since loop trip counts are never lower than 3 (P1 Lagrange elements on triangles), it is never the case that applying points 1) or 2) increases the operation count (it might have been the case, otherwise, in pathological scenarios in which the loops degenerate, i.e. loops having an iteration space of size 1).
\item Factorizing any of the terms in $\sigma_{ei}$ (e.g., a coefficient, or a coefficient derivative along a certain spatial dimension) can have different implications. We distinguish two cases:
\begin{itemize}
\item Performed before points 1) or 2). This either (i) inhibits (decrease) code motion by producing non-factorizable, non-hoistable sub-expressions or (ii) reduces to cases 1 or 2 (for example, in the case of simple forms with scalar coefficients).
\item After points 1) or 2) are applied; for example, $a_{ij}(b_{ik}(\sigma'_{ei} + \sigma''_{ei} + ...))$ and $\sigma_{ei} = \sigma'_{ei} + \sigma''_{ei} + ...$. Here, geometry-dependent expressions can be split from integration-dependent ones through suitable factorization and code motion. This is generally fundamental to minimize the operation count in the reduction loop.
\end{itemize}
\end{enumerate}

Based on these observations, in Algorithm~\ref{algo:opt} we describe a sequence of transformations that, in most cases, lead to optimal sharing elimination. 
...
%Our approach is therefore as follows: we define families of sharing elimination strategies; we calculate a lower bound ($lb$) for their operation count; we calculate the actual cost of $opt$; if, as expected, $cost(opt) < lb(S \setminus \lbrace opt \rbrace)$, then we know $opt$ is actually optimal. 

%We postpone to Section~\ref{sec:check-se} the discussion on how we assess the actual optimality of $opt$.

%In the best and simplest case -- that is, in forms where $\alpha_{eij}$ and $\beta{eik}$ are multiplied by a single, scalar coefficient -- there is just one $\sigma_{ei}$ identical to all terms in the expression. Its factorization 

%Code motion tends to break due to , no longer through factorization would not be hoistable due to dependencies on loop indices. For example, assuming $f_i \in \sigma_{ei}^{v}, \sigma_{ei}^{w}$ and $\upsilon_{ei} = \sigma_{ei} \setminus \lbrace f_i \rbrace$, we would have $f_i (\alpha_{eij}^{v} \beta_{eik}^{v} \upsilon_{ei}^{v} + \alpha_{eij}^{w} \beta_{eik}^{w} \upsilon_{ei}^{w})$
