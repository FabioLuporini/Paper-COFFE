...
\label{sec:zeros}
COFFEE expects generic abstract syntax trees, so clearly any form compiler can be adapted to produce this kind of input. This is what we have done with the FEniCS Form Compiler, suitably modified to produce abstract syntax trees rather than strings. However, form compilers that translate quadrature-based expressions into code could decide, for simplicity, to implement non-scalar function spaces in trivial yet sub-optimal ways. For example ...by producing a trivial loop nest that iterates over basis function tables filled with blocks of zeros 


Another viewpoint is: COFFEE takes care of the optimization process, ...
