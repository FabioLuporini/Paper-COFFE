...
\subsubsection{Observations and underlying assumptions}
\label{sec:se-assumptions}
We observe that there almost never are two ore more conflicting factorizations among which to choose for minimizing the operation count. The idea is that, by choosing a factorization that places a sub-expression within the loop that it logically belongs to, a reduction in operation count is generally obtained. There only are two pathological circumstances in which such a reduction may not be obtained: 
\begin{itemize}
\item As explained in Section~\ref{sec:se-opln}, having both test and trial functions in the same function space exposes sub-expressions over different loops, which introduces a choice between two alternative transformations.
\item The trip count of a loop is extremely small (typically it needs be 1 for the following to happen). This decreases or nullifies the impact of code motion -- exposed through factorization, as described in Sections~\ref{sec:se-opln} and~\ref{sec:se-rln}. For instance, the reader can verify that expanding the expression $(a f_i + b g_i) (c f_i + d g_i)$ within a loop $L_i$\footnote{The example was excerpted from the multiplication of the Jacobian matrix for the change of coordinates with the derivative of a coefficient} and factorizing $L_i$-dependent symbols leads to optimality if and only if the size of $L_i$ is strictly greater than 1.
\end{itemize}

\subsubsection{Algorithm}
Algorithm~\ref{algo:opt} details the steps for sharing elimination assuming as input an abstract syntax tree representation of the loop nest. Neglecting the pathological scenarios described in the previous section, the algorithm performs all necessary transformations and checks to reach optimality. A constructive proof of optimality can be derived taking into account both the algorithm itself and the observations in Sections~\ref{sec:se-opln},~\ref{sec:se-rln} and~\ref{sec:se-assumptions}. 
...
We factorize, in sequence, $L_j$- ($L_k$-) and $L_k$- ($L_j$-) dependent terms. The choice of starting with $L_j$ or $L_k$ depends on two parameters: the number of unique terms, $n_{L}$, and the trip count, $tc_{L}$, of a loop $L$. It is fairly simple to see (and easily demonstrable by contradiction) that taking first $L$ such that $n_L \cdot tc_L$ is smallest leads to the largest operation count reduction. 
