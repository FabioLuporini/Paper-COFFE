...
To answer this question, we make use of a result -- the foundation of tensor contraction mode -- in~\cite{Kirby:TC}. Essentially, multilinear forms can be seen as sums of monomials, each monomial being an integral over the equation domain of products (of derivatives) of functions from discrete spaces; such monomials can always be reduced to the product of two tensor, a reference tensor and a geometry tensor. This result can be interpreted at the loop nest level: if our input is, conceptually, as in Figure~\ref{code:loopnest}, then we can always dissect \texttt{F} into distinct sub-expressions (the monomials), factorize each sub-expression to split constant from $[L_i, L_j, L_k]$-dependent terms, hoist the latter ones outside of $N$, pre-evaluate the reduction. 
...
for each monomial in M:
    predict the cost of 
    calculate the cost of reduction pre-evaluation 
\end{lstlisting}
