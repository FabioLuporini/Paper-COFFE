...
Note that nothing prevents a perfect multilinear loop nest from being rooted in a deeper, possibly imperfect loop nest. In fact, we will later focus on this particular structure. First, we need to define optimality for perfect multilinear loop nests. To this purpose, it is convenient to introduce the notion of sharing.
...
We can now present a simple yet fundamental result.
...
Eliminating sharing from the multilinear loops does not suffice. In fact, we wonder whether, and under what transformations, the reduction imposed by $L_i$ could be pre-evaluated, as already suggested in~\ref{def:optimality}, and the operation count be reduced.
...
\label{sec:op_count}
The loose hand we have to tie regards the construction of the pre-evaluation cost function. We introduce the cost function $\theta : M \to \mathbb{N} \times \mathbb{N}$ that, given a monomial, returns two natural numbers representing the ideal operation count without ($\theta_{wo}$) and with ($\theta_{w}$) pre-evaluation. Since $\theta$ is integrated in a compiler, a requirement is that its execution is fast.  

We start with $\theta_{wo}$. Without performing any kind of transformations, we can easily predict the number of operations in the innermost loop once sharing is removed. This is because of our key property, linearity. By just counting the number of unique terms along the multilinear loop nest, and by taking into account $I$, the size of $L_i$, we can infer the cost. This is also explained in Proposition~\ref{prop:optimal-mln}.

For $\theta_{w}$, things are more complicated. We first need to account for the presence of (derivatives of) coefficients. This is because we want to estimate the \textit{increase factor} $\iota$. The increase factor captures the increase in arithmetic complexity within the multilinear loop nest if pre-evaluation is performed. To contextualize, consider the example in Figure~\ref{code:increase_factor}. 
...
One can think of this as the (simplified) loop nest originating from a pre-multiplied mass matrix. The sub-expression \texttt{f[0]*B[i,0]+f[1]*B[i,1]+f[2]*B[i,2]} expresses the field $f$ over a function space from which $B$ was derived. In order to apply pre-evaluation, the expression needs be transformed to separate $f$ from the other (reference element specific) quantities. This requires expanding a product, which, in turn, increases the number of $L_k$-dependent operations by a factor 3 (the number of the coefficient local degrees of freedom). $\iota$, or increase factor, precisely captures this growth in the number of $L_k$-dependent operations. Note that in an expression is not innermost loop dependent, then it can be hoisted, and therefore not significant for the operation count.

To determine $\iota$, we distinguish two cases: coefficients expressed over different function spaces and coefficients expressed over the same function space. This is essential because there could be more than one (derivative of) coefficient pre-multiplying the same monomial. In the former case, $\iota$ is equal to the cost of expansion. In the latter, after expansion we obtain identical summands, which eventually pre-evaluate to the same table. Consequently, some sharing would be eventually removed from the loop nest resulting from pre-evaluation (e.g., expression X at line Y in Figure~\ref{code:loopnest-opt}). We then need to calculate the number of combinations with repetitions of the symbols appearing in the monomial's coefficients to precisely determine $\iota$.
...
If, on the other hand, $\iota < I$, a further step is necessary to estimate $\theta_{w}$. In particular, we need to calculate the number of reference-element specific terms, $\rho$, by means of which we can set $\theta = \rho \cdot \iota$. Consider again Figure~\ref{code:increase_factor}. In the case of the mass matrix, the body of $L_k$ is characterized by the dot product of test and trial functions, \texttt{B[j]*B[k]}, so trivially $\rho = 1$. In general, $\rho$ varies with the discretization and the differential operators employed. For example, in the case of the bi-dimensional Poisson equation, $\rho = 3$. We determine $\rho$ by projecting the  
