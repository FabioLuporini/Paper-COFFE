...
As shown later, outer product loop nests are important because they possess properties that we can take advantage of to synthesize optimal code. We will see that they arise naturally when translating bilinear or linear forms into code. 
...
A finite element integration loop nest is a loop nest in which we identify, in order, an imperfect free order loop, a (generally) imperfect, linear or non-linear reduction loop, and an outer product loop nest.
\end{Def}
...
The sharing factor of a loop $L_i$, $L^{sf}_i$, is given by the cardinality of the set of symbols depending on $i$ (i.e., arrays indexed by $i$). It provides an indication of the maximal number of symbols which can be factorized to the level of $L_i$.
\end{Def}
...
Figure~\ref{code:multi_loopnest_a} shows an example of a trivial outer product loop nest of arity $n=2$ with sharing along dimension $j$ induced by $b_j$. Figure~\ref{code:multi_loopnest_b} shows an optimal synthesis for the loop nest in Figure~\ref{code:multi_loopnest_a}. More generally, arbitrarily complex outer product loop nests can systematically be reduced to optimal form using an algorithm that exploits the properties of linearity and perfectness. The algorithm and the proof that we provide below focus on outer product loop nests of arity $n=2$. For $n < 2$ the proof would be trivial, whereas for $n > 2$ a more general treatment would be required; fortunately, this case is never of interest in finite element integration.

\begin{Algo}[Optimal code motion for outer product loop nests]
\label{algo:outer-product-optimal}
\normalfont
Consider an outer product loop nest $\Lambda = [L_i, L_j]$, with the body of $L_j$ being an independent basic block. The algorithm starts with ``flattening'' the expressions (e.g., by multiplication expansion) in the body of $L_j$; this exposes the space of all possible factorizations. Since $\Lambda$ is an outer product loop nest, the resulting expressions are summations of outer products. We traverse the expression tree and determine the sharing factor of the two linear loops. We take the symbols depending on the loop with smallest sharing factor, say $L_i$, and we incrementally factorize them. Due to linearity, each factored product $P$ only has one symbol depending on $L_i$, and this symbol is now unique in the expression. The other term in $P$, independent of $L_i$, is, by definition, loop-invariant, and therefore hoisted such that redundant computation is avoided. We repeat the same process for the symbols along the loop with higher sharing factor, $L_j$. The transformations are semantically correct: multilinearity ensures deterministic factorization; perfectness ensures safeness of hoisting.
\end{Algo}

\begin{Prop}
\label{prop:optimal-mln}
Let $\Lambda = [L_i, L_j]$ be an outer product loop nest with the body of $L_j$ being an independent basic block. Then Algorithm~\ref{algo:outer-product-optimal} leads to an optimal synthesis $\Lambda'$. 
\end{Prop}
\begin{proof}
Assume, without loss of generality, that $L_i$ is the loop with smallest sharing factor. We reduce the proof to showing that the two following facts are true (we do it by contradiction).
\begin{enumerate}
...
\item Partial elimination of sharing would lead to a sub-optimal $\Lambda'$ (optimal only in the best case). Let us assume, for a moment, that this is false. In this scenario, at least one symbol is excluded from the factored product. Without loss of generality, consider, for instance, $a_i(b_j + c_j + ...) + a_i e_j + ...$. We see that either there is another factorization opportunity along $L_j$ ($e_j$ is shared with some other terms), in which case no additional operations need be performed, or $\Lambda'$ is sub-optimal. Consequently, the hypothesis is generally contradicted.
\end{enumerate} 
\end{proof}
