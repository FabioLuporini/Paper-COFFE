...
\begin{itemize}
\item We define optimality for finite element integration as number of flops given a memory bound; we build the transformation space within which the optimal is sought.
...
\item We state for which class of problems optimality can be reached, and for which class heuristics are instead needed to minimize (reduce) the operation count.
...
A loop $L_i$ is said to be a free order loop if its iterations can be executed in any arbitrary order; that is, no loop-carried dependence exists across two different iterations. 
\end{Def}

Altogether, these definitions allow us to formalize the class of loop nests for which we seek optimality.
...
\end{itemize}
In the following Sections~\ref{sec:sharing-elimination} and~\ref{sec:pre-evaluation}, we construct the transformation space. We leave a few loose ends, which we progressively tie up in Section~\ref{sec:optimal-synthesis}.
...
The sharing factor of a loop $L_i$, $L^{sf}_i$, is given by the cardinality of the set of expressions depending on $i$ (e.g., number of unique arrays indexed by $i$ if the expressions are just symbols). It provides an indication of the maximal number of expressions that can be factorized to the level of $L_i$.
\end{Def}

\begin{figure}[h]\begin{CenteredBox}
{\subfigcapskip = 7pt \subfigure[With sharing. Note that $L^{sf}_i = 2$ and $L^{sf}_j = 1$.]{\label{code:multi_loopnest_a}\lstinputlisting[basicstyle=\footnotesize\ttfamily]{listings/multilinear_loopnest.code}}}
~~~~~~~~~~~~~~
...
For example, Figure~\ref{code:multi_loopnest_a} shows a trivial outer product loop nest of arity $n=2$ with sharing along dimension $j$ induced by $b_j$. The optimal synthesis for this loop nest is provided in Figure~\ref{code:multi_loopnest_b}.

Our objective is a transformation capable of eliminating sharing from finite element integration loop nests. The transformation should find a factorization that maximizes code motion (as in the example) and common sub-expressions. In general, this would be a large combinatorial problem, but thanks to the specific structure of the loop nests considered, simpler approaches can be found. In particular, we describe two algorithms: the former leads to ``inner loop optimality'' by only performing an additional visit of the expression tree, whereas the latter allows reaching actual optimality at the price of a more costly worst-case analysis.

By inner loop optimality we mean optimality at the level of the outer product loop nest; that is, factorization opportunities are sought only for terms depending on the inner loops. The cost of the outer loops may or may not be optimal. It is interesting to study this problem because of three reasons: 1) in many cases, especially for higher order methods, the bulk of the computation is carried out in the outer product loop nest; 2) it is easily solvable, so it can be implemented with minimum effort; 3) the analysis has negligible cost. This algorithm (and its proof) exploits the key properties of linearity and perfectness, as well as the structure inherent in the expressions.

\begin{Algo}[Sharing elimination for inner-loop optimality]
\label{algo:outer-product-optimal}
\normalfont
Consider the finite element integration loop nest $\Lambda=[L_e, L_i, L_j, L_k]$. The algorithm starts with ``flattening'' the expressions (e.g., by multiplication expansion) in the body of $L_k$ such that $L_j-$ and $L_k-$dependent terms lie on the same level of the expression tree. This exposes the space of all possible factorizations for (derivatives of) test and trial functions. 

Since $[L_j, L_k]$ is an outer product loop nest, the resulting expressions are summations of outer products. We traverse the expression tree and determine the sharing factor of the two linear loops. We take the symbols depending on the loop with smallest sharing factor, say $L_j$, and we incrementally factorize them. Due to linearity, each factored product $P$ only has one symbol depending on $L_j$, and this symbol is now unique in the expression. The other term in $P$, independent of $L_j$, is, by definition, loop-invariant, and therefore hoisted such that redundant computation is avoided. We repeat the same process for the symbols along the loop with higher sharing factor, $L_k$. The transformations are semantically correct: multilinearity ensures deterministic factorization; perfectness ensures safeness of hoisting.
\end{Algo}

\begin{Prop}
\label{prop:optimal-mln}
Let $\Lambda=[L_e, L_i, L_j, L_k]$ be a finite element integration loop nest. Then Algorithm~\ref{algo:outer-product-optimal} leads to an optimal synthesis of $\Lambda'=[L_j', L_k']$.
\end{Prop}
\begin{proof}
Assume, without loss of generality, that $L_j$ is the loop with smallest sharing factor. We reduce the proof to showing that the two following facts are true (we do it by contradiction).
\begin{enumerate}
\item Taking $L_k$, the loop with higher sharing factor, as point of departure leads to a sub-optimal $\Lambda'$. If we assumed this were false, there would be two possibilities to consider: A) both $L_j$ and $L_k$ present sharing over disjoint sets of outer products; B) at least one outer product induces sharing along both $L_j$ and $L_k$. Case A) is quite trivial since there are no side effects in swapping the order of factorizations, so $\Lambda'$ would actually be optimal. In case B), due to linearity, factorization produces a summation of $L^{sf}_{k}$ outer products, as opposed to $L^{sf}_{j}$ if we had started with $L_j$. However, factorization followed by hoisting is always a winning strategy: evaluation, at every iteration, of hoisted symbols requires one operation (a sum) instead of the two operations (a product and a sum) needed by an outer product. Since $L^{sf}_{k} > L^{sf}_{j}$, it is clear that $\Lambda'$ cannot be optimal, which contradicts the hypothesis.
\item Partial elimination of sharing can lead to a sub-optimal $\Lambda'$. Let us assume that this is false. In this scenario, at least one term is excluded from a factored outer product, for instance $a_j e_k$ in $a_j(b_k + c_k + ...) + a_j e_k + ...$. We see that either there is another factorization opportunity along $L_k$ ($e_k$ is shared with some other outer products), or $\Lambda'$ is sub-optimal. Consequently, the hypothesis is generally contradicted.
\end{enumerate}
\end{proof}

\begin{Rem*}
\normalfont
The algorithm and the proof focus on outer product loop nests of arity $n=2$, although a similar (simpler) reasoning is possible for $n < 2$. If $n > 2$, a more general treatment is required; this is however a case that, in finite element integration, is never of interest.
\end{Rem*}

To reach a global optimum (i.e., such that the number of operations at each level of the loop nest is minimum), we need to consider all factorization opportunities. In practice, however, the structure inherent in our expressions often allows to significantly prune the search space. As we explain below, the key observation to make is that an expression can always be reduced to a summation of terms of the form $a_{ij} b_{ik} \sigma_{ei}$, in which $a_{ij}$ and $b_{ik}$ represent test and trial functions, while $\sigma_{ei}$ a sub-expression including geometry- ($L_e$) and integration- ($L_i$) dependent terms. The idea is that this form exposes all possible factorizations leading to code motion.

\begin{Algo}[Sharing elimination]
\label{algo:fem-optimal}
\normalfont
Consider the finite element integration loop nest $\Lambda=[L_e, L_i, L_j, L_k]$. We start with restructuring the expressions within $L_k$ such that they are in the form $\sum_{a \in V, b \in U} a_{ij} b_{ik} \sigma_{ei}^{ab}$, where $V$ and $U$ represent sets of test and trial functions. 

The loose end we have to tie up is the transformation of $\sigma_{ei}^{ab}$ terms. If $L_i$ is a linear loop, we can straightforwardly apply Algorithm~\ref{algo:outer-product-optimal} (with arity $n=1$) so that integration-dependent terms are grouped and geometry-dependent sub-expressions are hoisted. If, on the other hand, $L_i$ is non linear, we try all possible factorizations of $L_i$-dependent terms to maximize motion of geometry-dependent sub-expressions (although, we believe, a more refined approach, which avoids searching the whole space, can be found). 
\end{Algo}

\begin{Prop}
\label{prop:optimal-mln}
Let $\Lambda=[L_e, L_i, L_j, L_k]$ be a finite element integration loop nest. Then Algorithm~\ref{algo:fem-optimal} leads to an optimal synthesis of $\Lambda'=[L_e', L_i', L_j', L_k']$.
\end{Prop}
\begin{proof}
... This should be interpreted as our initial ``guess solution'' ...
\end{proof}

\subsection{Pre-evaluation}
\label{sec:pre-evaluation}
For finite element integration loop nests, the presence of sharing is a \textit{sufficient but not necessary} condition for being in \textit{non-optimal} form. Consider again the bilinear form implementation in Figure~\ref{code:loopnest}. We pose the following question: are we able to identify sub-expressions within $F$ for which the reduction imposed by $L_i$ can be pre-evaluated, thus obtaining a decrease in operation count proportional to the size of $L_i$, $M$? The transformation we look for is exemplified in Figures~\ref{code:loopnest_red} and~\ref{code:loopnest_nored}; Figure~\ref{code:loopnest_red}, which represents the input, can be seen as a simple instance of the abstract loop nest in Figure~\ref{code:loopnest}.
...
Pre-evaluation opportunities can be exposed through an exploration of the expression tree transformation space. This would be challenging if we were to deal with arbitrary loop nests and expressions. However, we can make use of a result -- the foundation of tensor contraction mode -- to build a relatively simple algorithm. As summarized in Section~\ref{sec:tc}, multilinear forms can be seen as sums of monomials, each monomial being an integral over the equation domain of products (of derivatives) of functions from discrete spaces; such monomials can always be reduced to a product of two tensors. This result can be turned into a loop and expression transformation algorithm, for which we provide a succinct description below. 

\begin{Algo}[Pre-evaluation]
\label{algo:pre-evaluation}
\normalfont
Consider a finite element integration loop nest $\Lambda = [L_e, L_i, L_j, L_k]$ as in Figure~\ref{code:loopnest}. We dissect \texttt{F} into distinct sub-expressions (the monomials). Each sub-expression is factorized so as to split constants from $[L_i, L_j, L_k]$-dependent terms. This transformation is feasible, as a consequence of the results in~\citeN{Kirby:TC}. These $[L_i, L_j, L_k]$-dependent terms are hoisted outside of $\Lambda$ and stored into temporaries. As part of this process, the reduction induced by $L_i$ is evaluated. Consequently, $L_i$ disappears from $\Lambda$. 
\end{Algo}

The pre-evaluation of a monomial introduces, however, some critical issues:
\begin{itemize}
...
\item The need for a strategy to coordinate sharing elimination and pre-evaluation opportunities: sharing elimination could inhibit pre-evaluation, while pre-evaluation might expose further sharing
\end{itemize}

We address these points in Section~\ref{sec:optimal-synthesis}. In the perspective of addressing point 2), we conclude providing a more refined definition of optimality.

\begin{Def}[Optimality of a loop nest with bounded working set]
\label{def:optimality}
Let $\Lambda$ be a generic loop nest, and let $\Gamma$ be a generic transformation function $\Gamma : \Lambda \rightarrow \Lambda'$ such that $\Lambda'$ is semantically equivalent to $\Lambda$ (possibly, $\Lambda' = \Lambda$) and a set of memory constraints is satisfied. We say that $\Lambda' = \Gamma (\Lambda)$ is an optimal synthesis of $\Lambda$ if the number of operations (sums, products) that it performs to evaluate the result is minimal.
\end{Def}

\subsection{Completeness}
It is helpful to consider sharing elimination and pre-evaluation as high level operators defined on top of basic transformations such as code motion, expansion, and factorization. These two operators allow us to analyze all possible ways in which an expression can be factorized; that is, by grouping:
\begin{itemize}
\item Test or trial functions (sharing elimination in the outer product loop nest)
\item Integration-dependent terms (sharing elimination in the reduction loop)
\item Together test functions, trial functions, and integration-dependent terms (pre-evaluation)
\end{itemize}
Factorization addresses \textit{spatial redundancy}. The presence of spatial redundancy means that some operations are needlessly executed at two points in an expression.

Sharing elimination and pre-evaluation also aim to remove \textit{temporal redundancy}, which arises whenever the same operation with the same operands is performed more than once. The problem of removing temporal redundancy reduces to minimize the execution of invariant sub-expressions. This requires two transformations:
\begin{itemize}
 \item Generalized code motion (occurs in both sharing elimination and pre-evaluation). Generalized, as explained in~\citeN{Luporini}, means that invariant sub-expressions are sought with respect to all loops composing a loop nest (and not just the innermost, as in traditional loop-invariant code motion). Examples were provided in Figures~\ref{code:multi_loopnest} and~\ref{code:loopnest_rednored}.
\item Evaluation of reduction loops (occurs in pre-evaluation). An example was provided in Figure~\ref{code:loopnest_rednored}.
 \end{itemize}

Sharing elimination and pre-evaluation allow us to construct an algorithm to minimize spatial and temporal redundancy in loop nests and expressions as in Definition~\ref{def:fem-loopnest}.

\section{Framework for Optimal Synthesis}
\label{sec:optimal-synthesis}

In this section, we build a transformation algorithm capable of reducing finite element integration loop nests to optimal form. A fundamental aspect is the definition of a suitable cost function to assess the profitability of applying pre-evaluation to a monomial.

\subsection{Memory constraints}
\label{sec:mem-const}
Two observations motivate the need for memory constraints. Our starting point is again the bilinear form implementation in Figure~\ref{code:loopnest}. Analogous considerations apply to forms of arbitrary arity.
...
We here discuss how we can systematically reach the goal set by Definition~\ref{def:optimality} under Constraints~\ref{const:Le} and~\ref{const:TH} for the class of loop nests in Definition~\ref{def:fem-loopnest}.

Two main issues should be addressed: 
\begin{itemize}
\item Understanding for which monomials pre-evaluation is profitable. In order to expose pre-evaluation opportunities, Algorithm~\ref{algo:pre-evaluation} transforms the input expression by expanding products. This increases the operation count. We therefore say that pre-evaluation is profitable if the gain due to pre-evaluating the reduction loop is larger than such increase.
\item Consider a form comprising two monomials $m1$ and $m2$. Assume that pre-evaluation is profitable for $m1$ but not for $m2$, and that $m1$ and $m2$ share at least one term (e.g. some basis functions). If pre-evaluation were applied to $m1$, sharing between $m1$ and $m2$ would be lost. We then need a mechanism to understand what transformation -- pre-evaluation or sharing elimination -- provides the highest operation count reduction.
\end{itemize}

For the moment, let us assume that we own a cost function $\theta : M \rightarrow \mathbb{Z}$ that, given a monomial $m \in M$, returns the number of operations saved or introduced if pre-evaluation were applied to $m$. $\theta(m) < 0$ indicates that pre-evaluation is not profitable, because the operation count would actually increase by $|\theta(m)|$. Analogous considerations stand for $\theta(m) > 0$. We will show how to construct a proper $\theta$ in Section~\ref{sec:op_count}. Having defined $\theta$, we can now elaborate on the transformation algorithm, for which pseudo-code is provided in Figure~\ref{code:intuition}.
...
maybe_P = M $\setminus$ S
B = $\lbrace$b | b is a bipartite graph from complete graph G = (maybe_P, E)$\rbrace$
for each $b \in B$:
  for each $(b_S, b_P) \in b$:
    if memory_required($b_P$) $\geq$ $T_H $:
      continue
    op_count[b] = \eta(S $\cup$ b_S) + $|\theta(b_P)|$
for each expression:
  remove sharing
\end{lstlisting}
...
Finally, with Proposition~\ref{prop:optimal-approach}, we show the optimality of this approach. 
...
\end{Prop}
\begin{proof}
We first comment on the assumptions. 1) Inherently a pathological case, so we simply neglect it. 2) Might occur in forms with several monomials, although this was never the case in the problems that we have so far investigated (which are obviously more than those presented in Section~\ref{sec:perf-results}). These assumptions allow us to avoid a ``global analysis'' of $M$.
...
A) The only corner situation in which this might occur is whenever some of the pre-evaluated monomials in $Z_P$ result in tables identical to those already deriving from $P$. If that were the case, more sharing elimination opportunities would be exposed, which in turn could break the optimality claim. Assumption 1) prevents this situation, so the statement is true. 

B) Let $\theta_{wo}(m)$ and $\theta_{w}(m)$ represent, respectively, the operation count for a monomial $m$ without and with pre-evaluation and let us consider the non-trivial case in which two monomials $p \in P_Z$ and $z \in Z$ share at least one term. We have to contemplate the fact that despite $\theta_{wo}(z) < \theta_w(z)$ and $\theta_{w}(p) < \theta_{wo}(p)$, we could have $\theta^G_{wo}(z, p)  < \theta_w(p) + \theta_{wo}(z)$, where $\theta^G_{wo}$ is a hypothetical global oracle. That is, there could be a larger potential gain in sharing elimination that goes lost if monomials are inspected in isolation. Potential examples are forms in which multiple monomials differ only for the number and/or the space of coefficient functions. The previously discussed assumption 2) allows us to neglect this possibility. 
\end{proof}

\subsection{Cost Function}
\label{sec:op_count}
It remains to tie up one loose end: the construction of the cost function $\theta$. Since $\theta$ is expected to be used by a compiler to drive the transformation algorithm, requirements are simplicity and velocity.

According to what established in the previous section, we have $\theta(m) = \theta_{wo}(m) - \theta_{w}(m)$; we recall that $\theta_{wo}$ and $\theta_{w}$ represent the operation counts without and with pre-evaluation. In the following, we explain how to derive these two values.
...
The rewrite operators, which implementation is based on abstract syntax tree manipulation, compose the COFFEE language. A non-exhaustive list of such operators includes expansion, factorization, re-association, generalized code motion. We have primarily used these operators to implement sharing elimination and pre-evaluation. Further, we have exploited them for heuristic optimization of the integration loop when this is non-linear (e.g., whenever the form has non-linear coefficients). In such a case, expansion, factorization, and code motion are applied greedily, selecting the term with highest sharing factor at each iteration, based on the observation that this should minimize sharing and promote hoisting. 
