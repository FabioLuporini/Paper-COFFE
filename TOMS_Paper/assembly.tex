...
\item Restructure the expression based on the structure of $G$ and a set of rewrite rules $<$\textit{precondition}: \textit{action}$>$, to be applied in the order given below.
\begin{enumerate}[label=(\Alph*)]
\item $\lbrace t^1, ..., t^n \rbrace \subset T$ adjacent to $s \in S$, $n > 1$: take $s$ and apply Strategy~\ref{strategy:ii}.\\
...
\item $T = \emptyset$, $C = [s, ..., v, ..., s]$ is a cycle, $v$ has highest degree in $C$, $\lbrace s^1, ..., s^n \rbrace$ adjacent to $v$: take $v$ and apply Strategy~\ref{strategy:ii}.\\
\textit{Note: this recasts the expression as $v (...s^1 + ... + ...s^n)$}.
\end{enumerate}
Rule (C) may expose further hits for rules (A) and (B). Rules are therefore applied cyclically until no more hits are possible.\\
\textit{Note: the last two steps only impact bilinear forms, since otherwise $E = \emptyset$.}
...
After structuring the sharing graph $G$ in such a way that only flop-decreasing transformations are possible, a set of rewrite rules are applied. We here discuss why applying, in rigorous order, rules (A), (B), and (C), leads to the optimum. 

Rules (A) and (B) represent ``unavoidable'' factorizations, for which one multiplication must be paid no matter how Strategy~\ref{strategy:ii} is scheduled. In particular:
\begin{description}
\item[Rule (A)] The terminals $\lbrace t^1, ..., t^n \rbrace$ can only be grouped through the factorization of $s$ (by definition of terminal), so this rule has no side effects.
\item[Rule (B)] Since $s^{nt}$ is not a terminal, there must exist at least one edge between $s^{nt}$ and a generic vertex $v$. Let us consider the alternative factorization $s^{nt}(...s + ... + ...v)$. If $v$ were a terminal, we observe that setting $v = t$ and $s = s^{nt}$ would simply expose another hit for rule (B). Otherwise ($v$ non terminal), we would have $s$ now appearing in two different sub-expressions ($(...s + ... + ...v)$ and $(...s\ t)$), which would clearly be sub-optimal as a consequence of the spatial sharing induced by $s$. From this, we evince that applying (C) prior (B) may prune the optimum from the search space.
\end{description}
To reach the optimum, rule (C) discards all those potential factorizations that can be rather ``aggregated'' with \textit{essential} factorizations. A factorization is essential if a vertex is in a cycle. Being in a cycle is the fundamental condition to 

Having higher degree simply means grouping more symbols (i.e., avoiding more products); 
