...
The concept of quasi-optimality, which relies on the particular class of \textit{f-decreasing} transformations, is also introduced.

\begin{Def}[F-decreasing transformation]
A sequence of transformations reducing the operation count is called f-decreasing (abbreviation for ``flop-decreasing'').
\end{Def}

\begin{Def}[Quasi-optimality of a loop nest]
\label{def:mln-quasi-optimality}
Given $\Lambda$, $\Lambda'$ and $\Gamma$ as in Definition~\ref{def:mln-optimality}, we say that $\Lambda' = \Gamma (\Lambda)$ is a quasi-optimal synthesis of $\Lambda$ if the number of operations (additions, products) in the innermost loops performed to evaluate the result is minimal and was achieved through a sequence of f-decreasing transformations.
\end{Def}

The restriction to f-decreasing transformations aims to exclude those ``deceptive'' solutions that, to achieve flop-optimal innermost loops, would rearrange the computation at the level of the outer loops causing a ``global'' increase in operation count. 
...
Achieving optimality in polynomial time is not generally feasible, since the $\sigma_{ei}$ sub-expressions can be arbitrarily unstructured. On the other hand, multilinearity suggest a certain degree of regularity for $\alpha_{eij}$ and $\beta_{eik}$. In the following sections, we will elaborate on these observations and formulate an approach that achieves: (i) optimality whenever the $\sigma_{ei}$ sub-expressions are ``sufficiently regular'';  (ii) quasi-optimality in all other cases. To this purpose, we will construct:
\begin{itemize}
...
\item[Temporal sharing] there is at least one non-trivial sub-expression (e.g., an addition or a product) that is redundantly executed, as independent of $\lbrace L_{i_1}, L_{i_1}, ...L_{i_n} \rbrace \subset \Lambda$
\end{description}
\end{Def}

To illustrate the definition, we show in Figure~\ref{code:multi_loopnest} how sharing evolves as factorization and code motion are applied to a trivial multilinear loop nest. In the original loop nest (Figure~\ref{code:multi_loopnest_a}), spatial sharing is induced by $b_j$. Factorization eliminates spatial sharing and promotes temporal sharing (Figure~\ref{code:multi_loopnest_b}). Finally, generalized code motion \cite{Luporini}, which seeks redundant sub-expressions analyzing all levels of the loop nest\footnote{Traditional loop-invariant code motion, which is commonly applied by general-purpose compilers, only considers innermost loops.}, leads to optimality (Figure~\ref{code:multi_loopnest_c}). 
...
\label{sec:se-rln}
Finite element expressions can be seen as composition of operations between tensors. We observe that, when multiplying two tensors, it often happens that the optimal scheduling strategy can be determined by comparing two possibilities. To illustrate the concept, consider the operation $J^{-1} \nabla x \cdot J^{-1} \nabla x$, with $J^{-1}$ being the inverse Jacobian matrix for the change of coordinates and $x$ a generic two-dimensional vector. The tensor operation will reduce to $(a x^0_i + b x^1_i) (c x^0_i + d x^1_i) + ...$, in which $x^0_i$ and $x^1_i$ represent components of $x$ that depend on loop $L_i$. To minimize the operation count for sub-expressions of this kind, we have two options.
\begin{Strategy}
\label{strategy:i}
Applying generalized code motion, in presence of temporal sharing.
\end{Strategy}
\begin{Strategy}
\label{strategy:ii}
...
\end{Strategy}
In the current example, we observe that applying Strategy~\ref{strategy:ii} would improve the operation count independently of the size of $L_i$, since the expression would be recast as $x^0_i (a + c) + x^1_i (b + d)$, thus exposing the two hoistable sub-expressions $(a + c)$ and $(b + d)$. On the other hand, Strategy~\ref{strategy:i} would have no effect as  $x$ only depends on a single loop, $L_i$. In general, however, the choice between the two strategies depends on multiple factors: the loop(s) size, the increase in operation count due to expansion (in Strategy~\ref{strategy:ii}), and the gain in code motion. Note that a second example of application of Strategy~\ref{strategy:ii} was provided in Figure~\ref{code:multi_loopnest}. These examples motivate the introduction of a particular class of expressions.
\begin{Def}[Structured expression]
\label{def:struct-expr}
An expression is \textit{structured} with respect to a set of loops $L$ if and only if there is no ambiguity in how to eliminate the spatial sharing induced by the symbols depending on $L$.
\end{Def}
The fundamental observation is the fact that a multilinear loop nest $L$ makes expressions structured with respect to $L$. In finite element integration loop nests, for example, a factorization of test and trial functions always results in exposing temporal sharing (Strategy~\ref{strategy:ii}). This will play a key role in Algorithm~\ref{algo:sharing-elimination}, which targets quasi-optimality. Finally, it is remarkable that structured sub-expressions often characterize the \textit{entire} variational form (for instance, in the complex hyperelastic model analyzed in Section~\ref{sec:perf-results}).


In the following, we regard as \textit{structured} any sub-expression resulting from tensor operations for which there is no ambiguity in how to perform spatial sharing elimination -- implying that the optimal operation scheduling is to be determined out of Strategy~\ref{strategy:i} and Strategy~\ref{strategy:ii}. From the perspective of achieving quasi-optimality, it is fundamental to observe how multilinearity promotes structured expressions. : factorization of test and trial functions will always expose temporal sharing, \footnote{It is also remarkable that structured sub-expressions often characterize the \textit{entire} variational form. For instance, they systematically arise in the weak variational form of the complex hyperelastic model analyzed in Section~\ref{sec:perf-results}.}. This is a consequence of multilinearity, which ensures that the factorization of any test or trial function will always expose some hoistable sub-expression (i.e., temporal sharing).
...
\normalfont
The algorithm has three main phases: initialization (step 1); scheduling of multilinear operands for reaching quasi-optimality (steps 2-5); scheduling of all other sub-expressions (step 6).
\begin{enumerate}
\item Perform a depth-first visit of the expression tree to factorize identical terms within additions. \\ 
...
\item For each $P \in \mathbb{P}$ such that $|P| \leq |S_P|$, apply Strategy~\ref{strategy:i} to the multilinear operands. \\
\textit{Note: $|P|$ and $|S_P|$ represent the number of products in the innermost loop induced by $P$ if, respectively, Strategy~\ref{strategy:i} or Strategy~\ref{strategy:ii} were applied. We will clarify that this directly descends from loop linearity and the structure inherent in tensor operations.}

\item Build the \textit{sharing graph} $\mathbb{G} = (\mathbb{S}, \mathbb{E})$, with $S^i \in \mathbb{S}$ representing a multilinear symbol or a temporary produced at step (3). An edge $(S^i$, $S^j)$ indicates that a product $S^i S^j$ would appear if the sub-expressions including $S^i$ and $S_j$ were expanded. Let $\mathbb{T}$ be the subset of vertices with at most one incident edge, or ``terminals''. 
...
\textit{Note: this ensures the f-decreasing nature of the transformations at step (6)}.\\
\textit{Note: this pass can be accomplished by visiting the expression tree and collecting several information -- for example, the number of products wrapping an instance of $S^i$ or $S^j$, from which the expansion cost can be derived.}


\item If $\mathbb{S} = \mathbb{T}$, jump to step (6). Otherwise apply, in order, the following rewrite rules (\textit{precondition}: \textit{action}):
\begin{enumerate}[label=(\Alph*)]
\item $\lbrace T^1, ..., T^n \rbrace \subset \mathbb{T}$ adjacent to $S$, $n > 1$: apply Strategy~\ref{strategy:ii}.\\
...
\item $\mathbb{T} = \emptyset$: take $S$ with highest degree and apply Strategy~\ref{strategy:ii} such that all symbols $\lbrace S^1, ..., S^n \rbrace$ adjacent to $S$ are grouped.\\
\textit{Note: this recasts the expression as $S (...S^1 + ... + ...S^n)$}.
\end{enumerate}
Go back to step (2).

\item Perform a depth-first visit of the expression tree and, for each yet unhandled or hoisted sub-expression, apply the most profitable between strategies (i) and (ii). \\
\textit{Note: this pass speculatively assumes all sub-expressions to be structured. If the assumption does not hold, the result will generally be sub-optimal since only a subset of factorizations and code motion opportunities may be considered.}
\end{enumerate}
\end{Algo}

\begin{Rem*}
\normalfont
It is true that Algorithm~\ref{algo:sharing-elimination} aims for quasi-optimality, but clearly this does not mean that only those sub-expressions depending on the multilinear loop nest are considered for optimization. We emphasize once more that the enforcement of f-decreasing transformations (step 5) and the re-scheduling within outer loops (step 7) attempt to minimize the operation count of the whole loop nest. We will further elaborate this aspect in Section~\ref{sec:proof}.
\end{Rem*}
...
Note that Constraint~\ref{const:Le} reflects the policy decision that the compiler should not silently consume memory on global data objects. It has the effect of shrinking the transformation space. Constraint~\ref{const:TH} has both theoretical and practical implications, which will be carefully analyzed in the next sections.
...
\item \textit{Search for a global optimum.} Consider a form comprising two monomials $m_1$ and $m_2$. Assume that pre-evaluation is profitable for $m_1$ but not for $m_2$, and that $m_1$ and $m_2$ share at least one term (e.g. some basis functions). If pre-evaluation were applied to $m_1$, sharing between $m_1$ and $m_2$ would be lost. We then need a mechanism to understand what transformation -- pre-evaluation or sharing elimination -- results in the highest operation count reduction when considering the whole set of monomials (i.e., the expression as a whole).
\end{enumerate}

Let $\theta : M \rightarrow \mathbb{Z}$ be a cost function that, given a monomial $m \in M$, returns the gain/loss achieved by pre-evaluation over sharing elimination. In particular, we define $\theta(m) = \theta_{se}(m) - \theta_{pre}(m)$, where $\theta_{se}$ and $\theta_{pre}$ represent the operation counts resulting from applying sharing elimination and pre-evaluation to, respectively. Thus pre-evaluation is profitable for $m$ if and only if $\theta(m) > 0$. We return to the issue of deriving $\theta_{se}$ and $\theta_{pre}$ in Section~\ref{sec:op_count}. Having defined $\theta$, we can now describe the transformation algorithm (Algorithm~\ref{algo:gamma}).
...
\item Discard $b = (b_S, b_P) \in B$ if the memory required after applying pre-evaluation to the monomials in $b_P$ exceeds $T_H$ (see Constraint~\ref{const:TH}); otherwise, add $D[b] = \theta_{se}(S \cup b_S) + \theta_{pre}(b_P)$. \\ \textit{Note: $\mathbb{B}$ is in practice very small, since even complex forms usually have only a few monomials. This pass can then be accomplished rapidly as long as the cost of calculating $\theta_{se}$ and $\theta_{pre}$ is negligible. We elaborate on this aspect in Section~\ref{sec:op_count}.}
\item Take $b$ such that $min(D[b])$.
\item Apply pre-evaluation to all monomials in $b_P$. Apply sharing elimination to all resulting expressions. \\ \textit{Note: because of reuse of basis functions, pre-evaluation may produce some identical tables, which will be mapped to the same temporary variable. Sharing elimination is therefore transparently applied to all expressions, including those resulting from pre-evaluation.}
\end{enumerate}
\end{Algo}
...
The most trivial way of evaluating $\theta_{se}$ and $\theta_{pre}$ would consist of applying the actual transformations and simply count the number of operations. If, on one hand, this would be tolerable for $\theta_{se}$ (Algorithm~\ref{algo:sharing-elimination} tends to have negligible cost), the overhead would be unacceptable if we applied pre-evaluation -- in particular, symbolic execution -- to all bipartitions analyzed by Algorithm~\ref{algo:gamma}. We then seek an analytic way of determining $\theta_{pre}$.
...
\item 
\begin{enumerate}
\item Let $T_m$ represent the set of tables resulting from applying pre-evaluation to a monomial $m$. Consider two monomials $m_1, m_2 \in Y$ and the respective sets of pre-evaluated tables, $T_{m_{1}}$ and $T_{m_{2}}$. If $T_{m_{1}} \cap\ T_{m_{2}} \neq \emptyset$, at least one table is assignable to the same temporary. $\Gamma$, therefore, may skip a potential global optimum, since $\theta$ only reasons on monomials in ``isolation''. We neglect this scenario (see assumptions) because of its purely pathological nature and its -- with high probability -- negligible impact on the operation count.
\item Let $m_1 \in X$ and $m_2 \in Y$ be two monomials that share some generic multilinear symbols. If $m_1$ were carelessly pre-evaluated, there may be a potential gain in sharing elimination that is lost, potentially leading to a local -- instead of a global -- optimum. This situation is prevented by construction, because $\Gamma$ analyzes all possible bipartitions searching for a global optimum preserving Constraint~\ref{const:TH}. Recall that since the number of monomials is in practice very small, this pass can rapidly be accomplished. Finally, note how this problem can be seen as an instance of the well-known Knapsack problem.
\end{enumerate}
\item Consider Algorithm~\ref{algo:sharing-elimination}. Because of the normal form of expressions and the structure inherent in tensor operations involving test and trial functions, there are only two ways of scheduling the multilinear operands in $P \in \mathbb{P}$: through generalized code motion (Strategy~\ref{strategy:i}) or factorization of multilinear symbols (achieved through Strategy~\ref{strategy:ii}). If applied, these two strategies would lead, respectively, to performing $|P|$ and $|S_P|$ multiplications at every loop iteration. Since Strategy~\ref{strategy:i} is applied if and only if $|P| < |S_P|$ and does not change the structure of the expression (it requires neither expansion nor factorization), step (3) will not prune optimal solutions from the search space. 

Step (6) applies factorization following the ordering dictated by the sharing graph and its rewrite rules. Non f-decreasing factorizations are excluded by construction at step (5). We then prove that applying in sequence rewrite rules (A), (B), and (C), with applications of rule (C) that may expose further opportunities for (A) and (B), represents a step forward towards quasi-optimality. 
\begin{description}
...
\item[Rule (B)] Since $S^{nt}$ is no terminal, there must exist at least one edge between $S^{nt}$ and a generic vertex $V$. Let us consider the alternative factorization $S^{nt}(...S + ... + ...V)$. If $V$ were a terminal, it is easy to see that setting $V = T$ and $S = S^{nt}$ would just expose another possible hit for rule (B). Otherwise ($V$ non terminal), we would have $S$ now appearing in two different sub-expressions ($(...S + ... + ...V)$ and $(...S\ T)$), which would clearly be sub-optimal as a consequence of the spatial sharing induced by $S$. From this, we see that applying rule (C) prior to rule (B) may prune the optimal solution from the search space.
\item[Rule (C)] We interpret $G$ as a bipartite graph $\mathbb{B} = (\mathbb{S}_j, \mathbb{S}_k, \mathbb{E})$, in which $\mathbb{S}_j$ and $\mathbb{S}_k$ represent the partitions of symbols depending on $L_j$ and $L_k$, respectively. For each disconnected component $\mathbb{B}^d = (\mathbb{S}_j^d, \mathbb{S}_k^d, \mathbb{E}^d)$ in $\mathbb{G}$, we consider two possibilities, namely $\mathbb{B^d}$ 1) is not or 2) is complete. Case 1) is the simplest: since, at this point, there are no more hits for rules (A) and (B), and thanks to loop linearity, the lower bound on the operation count (as number of products performed at every loop iteration), $lb$, is given by $lb = {min(}|\mathbb{S}_j^d|, |\mathbb{S}_k^d|{)}$. This is reachable by factorizing the vertex with highest degree, as any other choice may break factorization opportunities. In case 2), we have $lb = 1$ because factorization is applicable iteratively (e.g., $a_j a_k + a_j b_k + b_j a_k + b_j b_k = (a_j + b_j)(a_k + b_k) = t_j t_k$). This lower bound is achieved by repeating the sharing elimination algorithm (notice the jump back to step (2) at the end of step (6)).
\end{description}
The algorithm is guaranteed to terminate because of the jump to step (6) as $\mathbb{S} = \mathbb{T}$. 
\end{enumerate}
\end{proof}

%\item $\lbrace T^1, ..., T^n \rbrace \subset \mathbb{T}$ adjacent to $S$, $n > 1$: apply strategy (ii)
%\item Only one terminal $T$ adjacent to $S$: take an arbitrary non-terminal symbol $S^{nt}$ adjacent to $S$ and apply strategy (ii).\\
%\item $\mathbb{T} = \emptyset$: take $S$ with highest degree and apply strategy (ii) such that all symbols $\lbrace S^1, ..., S^n \rbrace$ adjacent to $S$ are grouped.\\
...
For a fair comparison, small patches (publicly available) were written for compatibility of \texttt{quad}, \texttt{tens}, and \texttt{ufls} with Firedrake. By executing all simulations in Firedrake, we guarantee that matrix insertion and mesh iteration have a fixed cost independent of the transformation system employed. The patches, in particular, adjust the data storage layout (e.g., by generating array of pointers instead of pointer to pointers and by replacing flattened arrays with bi-dimensional ones, as Firedrake expects).
