...
An expression is \textit{structured} with respect to a set of loops $L$ if and only if there is no ambiguity in spatial sharing elimination for the symbols depending on $L$.
\end{Def}
The fundamental observation is that if a loop nest $L$ is multilinear, then all of its expressions will be structured with respect to $L$. In finite element integration loop nests, for example, a factorization of test and trial functions will always result in exposing temporal sharing (Strategy~\ref{strategy:ii}). This property will play a key role in Algorithm~\ref{algo:sharing-elimination}, which targets quasi-optimality. Finally, we observe that it is remarkable that structured sub-expressions sometimes characterize the \textit{entire} variational form (for instance, in the complex hyperelastic model analyzed in Section~\ref{sec:perf-results}).
...
\begin{Rem*}
\normalfont
Although the primary goal of Algorithm~\ref{algo:sharing-elimination} is operation count minimization within the multilinear loop nest, the enforcement of f-decreasing transformations (step 5) and the re-scheduling of sub-expressions within outer loops (step 7) also aim at optimizing the loop nest ``globally''. We will further elaborate this aspect in Section~\ref{sec:proof}.
\end{Rem*}
