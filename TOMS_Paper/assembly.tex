...
In this work, we are particularly interested in the following class since it naturally arises from the math described in~\ref{sec:background}.

\begin{Def}[Perfect multilinear loop nest]
A \textit{perfect multilinear} loop nest of arity $n$ is a perfect nest composed of $n$ loops, in which all of the expressions appearing in the body of the innermost loop are linear in each loop $l \in [0, n-1]$.
\end{Def}
...
An optimal synthesis for a perfect multilinear loop nest $N$ can be found in ....
\end{Prop}
\begin{proof}
...
In practice, this result is too weak if we consider generic loop nests in which multilinearity applies to a subset of loops only and reductions are present. Consider the example in Figure~\ref{code:loopnest}. 
...
The imperfect nest $LN=[L_e, L_i, L_j, L_k]$ comprises a reduction loop $L_i$ and a perfect doubly nested loop $[L_j, L_k]$, which we assume to be multilinear. The right hand side of the statement computing the multidimensional array \texttt{A} is a generic expression \texttt{F} including standard arithmetic operations such as addition and multiplication. We observe that \texttt{F} might contain sub-expressions that depend on a subset of loops only and, as such, hoistable outside of \texttt{N}, provided that data dependencies are preserved (note that \texttt{N} is imperfect). One could think of pre-evaluating the reduction, thus obtaining a decrease proportional to \texttt{M} in the number of operations executed. However, finding or exposing such sub-expressions is, in general, challenging. Further, even though we assumed we could do so, the following issues should be addressed
\begin{itemize}
\item as opposed to what happens with hoisting in perfect multilinear loop nests, in this case the size of the temporaries would be proportional to the number of non-reduction loops (in the example, \texttt{N$\cdot$O} for \texttt{ijk} sub-expressions, \texttt{L$\cdot$N$\cdot$O} for \texttt{eijk} ones). This might shift the loop nest from a CPU-bound to a memory-bound regime, which might be counter-productive for actual runtime;
...
\begin{itemize}
\item The size of a temporary due to code motion cannot be bigger than the size of the multilinear loop nest iteration space ($N\cdot O$ for the bilinear form in the example). A corollary is that hoisting of expressions involving geometry terms outside of $L_e$ is forbidden.
\item The total size of the hoisted temporaries cannot exceed a threshold \texttt{$T_H$}
\end{itemize}

\subsection{Operation Minimization under Memory Constraints}
...
The intuition of the algorithm to synthesize an optimal loop nest is shown in Figure~\ref{code:intuition}. 
...
The point of departure consists of understanding the impact, as number of operations saved or introduced, of pre-evaluation. This is studied "locally"; that is, for each monomial, in isolation. We will later discuss the optimality of this approach and how we can precisely quantify such impact exploiting simple mathematical properties. 

%now show that local reasoning is optimal in our context, while we postpone the discussion about the variation in \texttt{ops}. 
%For the moment, just note that if we exclude pathological cases in which different monomials reduce to multiple identical tables, local reasoning satisfies our needs. 

If we estimate that, for a given monomial, pre-evaluation will decrease the workload, then the corresponding sub-expression is extracted, a sequence of transformation steps -- involving expansion, factorization, code motion -- takes place (details in Section~\ref{sec:codegen}, and the evaluation eventually performed. The result is a set of $n$-dimensional tables (these can be seen as "slices" of the reference tensor at the math level), $n$ being the arity of the multilinear form. Identical tables are mapped to the same temporary. Eventually, sharing is removed from all resulting expressions by applying a procedure as described in Proposition~\ref{prop:optimal-mln}. 

The transformed, optimal loop nest is as in Figure~\ref{code:loopnest-opt}. 
...
Before elaborating on how we determine the pre-evaluation gain (or loss), we demonstrate the optimality of the approach.
%We introduce the following notation ...

\begin{Prop}
Consider an expression comprising a set of monomials $M$. Let $P$ be the set of all monomials for which it has been estimated that pre-evaluation decreases their operation count. Then, if pre-evaluation is applied to all of the monomials in $P$, the resulting loop nest $LN$, enclosing an expression which is the sum of all monomials in $Z = M \setminus P$, is optimal under memory constraints $C$ once sharing is removed.
\end{Prop}
\begin{proof}
We distinguish two classes of loop nests rooted in $LN$: $[L_e, L_j, L_k]$ for the pre-evaluated expressions in $P$, and $[L_e, L_i, L_j, L_k]$ enclosing the remaining monomials in $Z$. Since they only differer for the presence of $L_i$, we relieve the notation by omitting all shared loops when discussing operation counts. In particular, we use $I$ to refer to the iteration space size of $L_i$. The operation count of what we are proving to be the optimal $LN$ synthesis is, therefore, $LN_{ops} = LN_{ops_1} + LN_{ops_2} = \sum_{\alpha}^{\# P} p_{\alpha} + I \sum_{\beta}^{\# Z} z_{\beta}$, where $p_{\alpha}$ and $z_{\beta}$ represent the operation cost of monomials in $P$ and $Z$.

We start noting that, as explained in Section~\ref{sec:mem-const}, $C$ imposes constraints on hoisting. This allows us to search the optimum in the syntheses space by only 1) selecting candidates for pre-evaluation and 2) dealing with sharing. We need to prove that reasoning about the operation count on a monomial basis, rather than globally, suffices to reach our definition of optimality. We reduce this problem to demonstrating the following: A) pre-evaluating any $Z_P: Z_P \subseteq Z$ would increase $LN_{ops}$; B) not pre-evaluating any $P_Z: P_Z \subseteq P$ would increase $LN_{ops}$. That is, we show there is no better way of constructing $P$ and $Z$ other than local reasoning.

A) We prove that $LN_{ops}' = LN_{ops_1}' + LN_{ops_2}' > LN_{ops}$. It is rather obvious that $LN_{ops_1}' \geq LN_{ops_1}$ (it is equal only if, trivially, $Z_P = \emptyset$). We note that if all symbols in $Z_P$ are also shared in $\overline{Z} = Z \setminus Z_P$,  then we have $LN_{ops_2}' = LN_{ops_2}$, so out statement is true. If, on the other hand, at least one symbol is not shared, we obtain $LN_{ops_2}' < LN_{ops_2}$ or, equivalently, $LN_{ops_2}' = LN_{ops_2} - I \cdot\delta$. What we have to show now is that even by exposing more pre-evaluated terms, $LN_{ops_1}' \geq LN_{ops_1} +  I \cdot\delta$ holds. This is the case if and only if pre-evaluated terms are not identical to others in $P$, which would otherwise expose sharing. However, if we exclude the pathological cases in which different sub-expressions reduce to the same values (due to symmetries in basis functions, for example), this must be true, otherwise the assumption about the optimality of the cost prediction would be contradicted.

B) This holds due to a simple observation. In the absence of sharing, the statement is trivially true since we would have $LN_{ops_2}' > LN_{ops_2}$ and we are assuming that pre-evaluation was optimal for the monomials at hand. In the worst case, instead, all non pre-evaluated monomials share terms with those in $Z$. Anyway, the best we can do 

%: regardless of whether there is sharing between terms in $Z_P$ and $\overline{Z} = Z \setminus Z_P$, then the statement is trivially true, since we are assuming that $Z$ contains all monomials for which pre-evaluation is sub-optimal. If, on the other hand, $\delta \in Z_P \cap \overline{Z}$, one can reason as follows. We can first remove sharing; this makes  to reach local optimality and we compare the operation counts. the cost of the loop nest would be $Z_P^{pre} + P + \sharp L_i \cdot Z \setminus Z_P$, which is greater than $...$, since $Z_P^{pre} > \sharp L_i \cdot Z_P$. 
