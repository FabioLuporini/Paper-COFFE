...
\end{itemize}
In the following Sections~\ref{sec:sharing-elimination} and~\ref{sec:pre-evaluation}, we focus on the construction of the transformation space. We consider outer product loop nests of arity $n=2$ (i.e., bilinear forms), although a similar (simpler) reasoning straightforwardly applies to the case $n < 2$.
...
\item In the special case $\alpha = \beta$, common sub-expressions arise over different iteration spaces ($ij$ and $ik$). Compared to point 1), by hoisting and capturing these sub-expressions the reduction loop operation count becomes smaller, although the gain tends to be negligible (the operation count in the innermost loop is not affected).
\item If we neglect the pathological cases in which the loops degenerate (i.e., when the trip count becomes 1, which is the case of P0 discontinuous elements), factorizing and hoisting as in points 1) or 2) systematically reduces the operation count. The reduction is proportional to the outer product loop nest extent.
\item Factorizing any of the terms in $\sigma_{ei}$ (e.g., a coefficient, or a coefficient derivative along a certain spatial dimension) also impacts the operation count. We distinguish two cases:
\begin{enumerate}
\item Test/trial functions are yet to be factorized. This, in general, inhibits (decreases) code motion by producing non-factorizable, non-hoistable sub-expressions. In the best case (e.g., there is a single scalar coefficient pre-multiplying the form) this simply reduces to cases 1) or 2).
\item Within $\sigma_{ei}$ itself; for example, after applying point 1) we have $a_{ij}(b_{ik}(\sigma'_{ei} + \sigma''_{ei}))$, with $\sigma_{ei} = \sigma'_{ei} + \sigma''_{ei}$. Splitting $L_e$-dependent from $L_i$-dependent sub-expressions (through suitable re-factorization) and applying code motion naturally tends to operation count minimization. Similarly to point 3), however, with very low trip counts this strategy may be sub-optimal. For example, consider the following sub-expression, $(a v_i + b w_i) (c v_i + d w_i)$, which is excerpted from a multiplication of the inverse Jacobian matrix for the change of coordinates with the derivative of a vector-valued coefficient. It is true that factorizing $v_i$ and $w_i$ exposes $L_i$-independent sub-expressions, but this will actually increase the operation count when the size of $L_i$ is 1.
\end{enumerate}
\end{enumerate}
...
If, on the other hand, $\iota < I$, a further step is necessary to estimate $\theta_{pre}$. In particular, we need to calculate the number of terms $\rho$ such that $\theta_{pre} = \rho \cdot \iota$. Consider again Figure~\ref{code:increase_factor}. In the case of the mass matrix, the body of $L_k$ is simply characterized by the dot product of test and trial functions, \texttt{B[j]*B[k]}, so trivially $\rho = 1$. In general, $\rho$ varies with the discretization and the differential operators used in the form. For example, in the case of the bilinear form originating from a standard bi-dimensional Poisson equation, the reader could verify that after a suitable factorization we would have $\rho = 3$. To determine $\rho$, we simply apply Algorithm~\ref{algo:pre-evaluation} until the evaluation of the reduction (obviously, excluded).


\subsection{Trading optimality for analysis cost}
Code generation time is fundamental for productivity. In this section, we discuss how to reduce it by relaxing the constraints on optimality.
\label{sec:obsheu}
\begin{itemize}
\item \textit{Sharing elimination.} In Section~\ref{sec:sharing-elimination}, we explained that very small loop trip counts render certain factorizations sub-optimal. To reach optimality, Algorithm~\ref{algo:opt} needs to backtrack if it detects an increase in operation count after applying sharing elimination at a given loop nest level. Since this situation is particularly rare, we can relieve the algorithm by avoiding these checks.
\item \textit{Transformation algorithm.} The transformation algorithm (Figure~\ref{code:intuition}) evaluates all possible ways of scheduling monomials candidate to pre-evaluation. In extremely complex forms, this could increase the analysis cost. Despite we have never encountered such a situation, a possibility is to disable these checks and to apply the transformation to all candidates. In the worst case scenario, this leads to a local optimum.
\item \textit{Construction of $\theta$.} Instead of determining $\theta_{se}$ and $\rho$ (for $\theta_{pre}$) by manipulating the expression tree, we can approximate these values exploiting the key property of outer product loop nests, linearity. The number of unique terms in a multilinear loop nest can be used to calculate an upper bound on the cost of the innermost loop, where the bulk of the computation naturally tends to be performed. For example, in Figure~\ref{code:multi_loopnest_a} the fact that only a single term depends on $L_j$, $b_j$, suggests that in the innermost loop no more than one multiplication will be performed once sharing is eliminated. Therefore, an approximation for $\theta_{se}$ is, for example, $\theta_{se} = n_{iterations} n_{unique}$, where $n_{iterations}$ is the total number of iterations, and $n_{unique}$ is the smallest number of unique terms along one the linear loops.
\end{itemize}

In our code generation system, described in the next section, we make use of all of these heuristics.
