...
\item We know how many times a multilinear symbol $s$ appears in the input expression. Let $n_{s}$ denote this quantity. Let $c_{op}$ be the cost of (i.e., number of operations in) $op$. Also, recall from Figure~\ref{code:loopnest} that $L_i$ and $L_k$ have size $M$ and $O$. Assume, for example, that $P$ depends on $[L_i, L_k]$. We observe that the increase in operation count at the level of the outer loops would be: $M O \sum_{w \in P} c_{op}^{w}$ if (i) were applied; $M n_{s}$ if (ii) were applied. Therefore, we take (i) or (ii) depending on which quantity is minimum.
\end{enumerate}

On a last depth-first visit of the expression tree, we apply point (2) above to all of the $L_i$-dependent expressions. 
\end{Algo}

Section~\ref{sec:proof} elucidates the claims of quasi-optimality and optimality. 
