...
\normalfont
The algorithm has three main steps: initialization (step 1); scheduling of multilinear operands, to reach quasi-optimality (steps 2-5); scheduling of all other sub-expressions (step 6).
\begin{enumerate}
...
\item Perform a depth-first visit of the expression tree to collect and partition multilinear operands into $p$ disjoint sets, $\mathbb{P} = \lbrace P^0, P^1, ..., P^{p-1}\rbrace$. $\mathbb{P}$ is such that multilinear operands in $P$ share a set of multilinear symbols $S_P$, whereas there is no sharing across different partitions.
