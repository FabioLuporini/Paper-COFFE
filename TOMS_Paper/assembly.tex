...
As reiterated throughout the paper, the execution time of a kernel is influenced not only by the operation count, but also by the chosen code generation strategy (e.g., loop unrolling rather than preserving the loop structure). Figure~\ref{fig:flops-correlation} aggregates the \texttt{cfO2} results from the previous sections and displays them as a function of the reduction in executed flops. Besides summarizing the performance improvements achieved in this article, this figure highlights a number of aspects. The speed-ups achieved by \texttt{cfO2} tend to increase with the complexity of the operators and $q$. Further, the pre-multiplying functions have a limited impact in complex operators, and they occasionally have a negative effect in simple and small problems. Finally, this plot shows that even a powerful general-purpose compiler like Intel's is inadequate to fully exploit the structure of non-trivial finite element operators (analogous behaviors were observed with different compilers).
