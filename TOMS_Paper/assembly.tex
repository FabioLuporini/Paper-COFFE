...
In the next sections, we will often refer to the local element matrix evaluation, such as Equation~\ref{quadrature} for the weighted Lapalce operator, as the \textit{assembly expression} deriving from the weak variational problem.


\subsection{COFFEE: a Compiler for Optimizing Quadrature-based Finite Element Integration}
\label{sec:summary-opts}
If high performance code needs to be generated, assembly expressions must be optimized with regards to three interrelated aspects: 1) arithmetic intensity, 2) instruction-level parallelism, and 3) data locality. In this paper, we tackle these three points building on our previous work (\cite{Luporini}), in which we presented COFFEE, a mature, platform-independent compiler capable of optimizing local assembly code.

\begin{figure}
\begin{center}
\includegraphics[scale=0.6]{pictures/coffee-scheme}
\caption{Outline of COFFEE. The input is either a string of C code or an abstract syntax tree representation. The optimization process goes through two main steps: \textit{expression rewriting} and \textit{code specialization}. The autotuner, incorporated with the Code Specializer, use heuristics to select the set of code variants that, for a given variational form, are likely to reach high performance.}\label{fig:coffee-outline}
\end{center}
\end{figure}

The high-level structure of COFFEE is outlined in Figure~\ref{fig:coffee-outline}. The input is either C code, explicitly provided by the user as a string, or an abstract syntax tree (henceforth AST) representation automatically generated at the higher level of abstraction. We have integrated COFFEE with the Firedrake framework; here, the input is originated by the FEniCS Form Compiler~\cite{FFC-Compiler} in the form of an AST. Decoration of special AST nodes, or equivalently annotation of C code, is extensively used to convey domain knowledge to COFFEE. This is used to introduce more effective, specialized optimizations, as well as to simplify the implementation.
...
To neatly distinguish the contributions of this paper from those in~\cite{Luporini}, in this section we summarize the results of our previous work. Moreover, in Section~\ref{sec:perf-results}, we will explicitly compare the performance achieved with the transformations presented in this paper to both~\cite{Luporini} and~\cite{quadrature1}.
...
In Figure~\ref{fig:coffee-outline}, we show where these transformations are applied in the COFFEE pipeline; also, the novel contributions of this work are highlighted in red. In the next two sections, we describe the new functionalities of Expression Rewriter and Code Specializer, respectively.
...
Secondly, the Expression Rewriter re-structures iteration spaces so as to avoid arithmetic operations over tabulated basis functions columns that are statically known to be zero-valued. Zero-valued columns in tabulated basis functions appear, for example, when taking derivatives on a reference element or when using mixed elements. A code transformation eliminating floating point operations on zeros was presented in~\cite{quadrature1}; however, the issue with that is the use of indirection arrays in the generated code, which break many of the optimizations applicable at code specialization time, including SIMD vectorization. In Section~\ref{sec:zeros}, we present a novel approach based on symbolic execution.
...
\tiny
\centering     
\subfigure[Original (simplified) code]{\label{fig:original-code}\lstinputlisting{listings/simple.code}}
~~~~~~~~~~~~~~~~
\subfigure[The code after that generalized loop-invariant code motion has been applied]{\label{fig:invariant-code}\lstinputlisting{listings/invariant.code}}
~\\
~\\
\subfigure[Once terms are factorized, further code hoisting opportunities arise. Here, \texttt{T3} stores the precomputation of an invariant sub-expression exposed by factorization]{\label{fig:factorized-code}\lstinputlisting{listings/factorized.code}}
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
\subfigure[Hoisting and scalar-expansion of integration-dependent terms allow to achieve full SIMD vectorization of the assembly code]{\label{fig:scalarexp-code}\lstinputlisting{listings/vectexpanded.code}}
\caption{Applying a sequence of transformations to an excerpt from a Burgers' local assembly kernel, suitably simplified to improve readability.}
\end{figure}

Consider the local element matrix computation in Figure~\ref{fig:original-code}, which is an excerpt from a Burgers problem. The assembly expression, produced by the FEniCS Form Compiler, has been deliberately simplified, and code details have been omitted for brevity and readability. In practice, assembly expressions can be much more complex, for example depending on the differential operators employed in the variational form; however, this example is representative enough for highlighting patterns that are common in a large class of problems. 
...
\label{sec:code-spec}
Code specialization provides a range of code transformations tailored to optimizing for instruction-level parallelism and register locality. As summarized in Section~\ref{sec:summary-opts} and described in~\cite{Luporini}, padding and data alignment, expression splitting, and outer-product vectorization, which is a domain-aware implementation of vector-register tiling, are examples of optimizations that the Code Specializer is capable of applying. In this paper, we revisit and enrich this set of transformations as explained in the following sections.
...
All of the previous points contribute to minimize the overhead of the autotuning process. We will discuss the actual cost of autotuning in Section~\ref{sec:auto-analysis}.
...
\cline{1-10}
problem & \specialcell{number of\\variants} & \specialcell{rewrite\\strategy} & \specialcell{zero-valued columns\\avoidance} & \specialcell{precompute\\integration terms} & \specialcell{padding\\data alignment} & split & \specialcell{vector-register\\tiling} & unroll & M \\
\cline{1-10}
...
\begin{table*}[t]
\tbl{Summary of the optimizations selected by the autotuner in a number of forms}{ \scriptsize \autotuneroutput }
