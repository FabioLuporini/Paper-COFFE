...
Given $\Lambda$, $\Lambda'$ and $\Gamma$ as in Definition~\ref{def:mln-optimality}, we say that $\Lambda' = \Gamma (\Lambda)$ is a quasi-optimal synthesis of $\Lambda$ if the number of operations (additions, products) that it performs to evaluate the result is at least minimal in the innermost loop.
\end{Def}
...
Achieving optimality in polynomial time is clearly not always possible, since the $\sigma_{ei}$ sub-expressions can be arbitrarily complex. Multilinearity, on the other hand, ensures a certain degree of regularity for the  $\alpha_{eij}$ and $\beta_{eik}$ sub-expressions. In the following sections, we build an approach that achieves: (i) optimality whenever the $\sigma_{ei}$  expressions are ``sufficiently regular'';  (ii) quasi-optimality in all other cases. In particular, we need:
\begin{itemize}
\item the space of legal transformations impacting the operation count
\item an algorithm to select points in the transformation space
\end{itemize}
In the following Sections~\ref{sec:sharing-elimination} and~\ref{sec:pre-evaluation}, we construct the transformation space and we explain the conditions for optimality. We show that whenever an expression (i.e., its monomials) is ``too unstructured'', optimality may not be reached. We will consider multilinear loop nests of arity $n=2$ (i.e., bilinear forms), although a similar (simpler) reasoning straightforwardly applies to the case $n < 2$.
...
In this section, we study \textit{sharing elimination}, a transformation that aims to reduce the operation count by removing sharing through the application of factorization, code motion and (if necessary) common sub-expression elimination. If the objective is reaching optimality and expressions are generally unstructured, transformations of this sort require solving large combinatorial problems, for instance to evaluate the impact of all possible factorization strategies. In our domain, however, expressions may be characterized by certain properties. These properties hold either in general -- as inherent in finite element integration -- or in forms making ``special use'' of most common differential and tensor operators. We exploit them to build an algorithm that results in optimal or quasi-optimal sharing elimination depending on the problem to which it is applied. In Sections~\ref{sec:se-mln} and~\ref{sec:se-rln} we discuss these properties, while in Section~\ref{sec:sec:se-algo} we present the algorithm. We assume the usual finite element integration loop nest $\Lambda=[L_e, L_i, L_j, L_k]$ with expressions in the canonical form $\sum_{w=1}^m \alpha_{eij}^{w} \beta_{eik}^{w} \sigma_{ei}^{w}$.

\subsubsection{Common sub-expressions}
\label{sec:se-mln}
A typical optimization to reduce the arithmetic complexity of a code is common sub-expression elimination (CSE). This often revolves around searching, possibly across different statements, and replacing with temporaries, sub-expressions that (i) are symbolically identical and (ii) reduce to the same value.  

We observe that ``classic CSE'' (CCSE) can straightforwardly be applied without the need for data dependence analysis, since the body of a finite element integration loop nest is an independent basic block. More interestingly, we observe that in the ``special case'' (SCSE) in which test and trial functions are in the same function space, there may exist $w_1$ and $w_2$ such that $\alpha_{eij}^{w1} = \beta_{eik}^{w2}$, in which case common sub-expressions will arise along different loops ($L_j$ and $L_k$). General-purpose compilers cannot detect this (very domain-specific) property. Our algorithm for sharing elimination will exploit this extra information.

Both CSE and factorization target spatial sharing. We will see that in the cases in which factorization is found to increase the operation count (due to the need for preliminary transformations), CCSE will instead be employed to remove spatial sharing.

%TODO : if also the Ri loop is linear, then optimality full stop.

\subsubsection{Structured inner products}
\label{sec:se-rln}
