...
\item $c_o^l$: the cost of an expression $o$ within a loop $L_l$; that is, the number of operations in $o$ times $|L_l|$.
...
We perform a second depth-first visit of the expression tree to collect and partition multilinear operands into $p$ disjoint sets, with $\mathbb{P} = \lbrace P^0, P^1, ..., P^{p-1}\rbrace$. The multilinear operands within a partition share the same set of multilinear symbols, whereas there is no sharing across different partitions. We have to establish:
\begin{enumerate}
\item an ordering for $\mathbb{P}$, such that if $P^i \prec P^j$, then $P^i$ is handled before $P^j$, and
\item a scheduling strategy for each $P^i$
\end{enumerate}

We start with point 1). We further partition $\mathbb{P}$ into two disjoint sets $\mathbb{P}_{j}$ and $\mathbb{P}_{k}$, which logically separates partitions based on loop dependency. We then build a reverse lexicographic ordering for $\mathbb{P}$, with the position of $P \in \mathbb{P}_l$, $l \in \lbrace j, k \rbrace$, given by ${<}|\mathbb{P}_{l}|,\ \sum_{o \in P} c_o^l{>}$. Intuitively, this simply means that operands appearing more often in the expression are handled first. We will later elaborate on this.
...
\item Let $n_{o}$ and $n_{s}$ denote, respectively, the number of times a multilinear operand $o$ and a multilinear symbol $s$ appear in the input expression. The increase in operation count at the level of the outer loops is: if (i) were applied, $\sum_{o \in P_l} (c_{o}^{l} + n_{o} - 1)$; if (ii) were applied, $\sum_{s \in S_P} (n_{s} - 1)$. We pick (i) or (ii) such that the increase is minimum. Intuitively, this is a way of estimating how two different factorizations affect the operation count, given the fact (criterion 1) that both will eventually minimize the innermost loop cost.
\end{enumerate}
...
Consider a multilinear form comprising a set of monomials $M$, and let $\Lambda$ be the corresponding finite element integration loop nest. Let $\Gamma$ be the transformation algorithm. Let $P$ be the set of pre-evaluated monomials, and let $Z = M \setminus P$. Assume that pre-evaluating different monomials does not result in identical tables. Then, $\Lambda' = \Gamma(\Lambda)$ is quasi-optimal in the sense of Definition~\ref{def:mln-quasi-optimality}.
\end{Prop}
\begin{proof}
We first observe that the cost function $\theta$ predicts the exact gain/loss in monomial pre-evaluation, so $P$ is always correct by construction. 
...
\begin{enumerate}
\item \textit{pre-evaluating any $Z_P \subseteq Z$ would result in $c_{\Lambda''_I} > c_{\Lambda'_I}$}
\item \textit{not pre-evaluating any $P_Z \subseteq P$ would result in $c_{\Lambda''_I} > c_{\Lambda'_I}$}
\end{enumerate}
\item \textit{Sharing elimination (Algorithm~\ref{algo:sharing-elimination}) produces (at least) quasi-optimal loop nests.}\\
\end{enumerate}
...
\item 
\begin{enumerate}
\item Let $m_T$ represent the set of tables $T$ resulting from applying pre-evaluation to a monomial $m$. Consider two generic monomials $m_1, m_2 \in Z$ and the respective sets of pre-evaluated tables, $m_{T_{1}}$ and $m_{T_{2}}$. If $m_{T_{1}} \cap m_{T_{2}} \neq \emptyset$, at least one table is assigned to the same temporary. $\Gamma$ would then skip a potential global optimum, since $\theta$ only reasons ``locally'' (i.e., on a monomial basis). By assumption, this case is neglected. The assumption is reasonable because such a scenario is of pathological nature (we have never experienced it) and, with high probability, the retrieved solution would be extremely close to the ideal one.
\item Let $p \in P$ and $z \in Z$ two monomials that share some subsets of multilinear symbols. If, as by assumption $\theta(p)$ suggests, $p$ will be pre-evaluated, there may be a potential gain in sharing elimination going lost, which potentially leads to a locally optimal solution -- instead of a global one. We avoid this situation by construction. $\Gamma$ models the problem as a graph, analyzing all possible bipartitions that may break the quasi-optimality claim (lines 7-13 in Figure~\ref{algo:gamma}). Since the graph is in practice very small (the number of monomials in a form rarely exceeds a few units), this pass can rapidly be accomplished.
\end{enumerate}
\item ...
\end{enumerate}
