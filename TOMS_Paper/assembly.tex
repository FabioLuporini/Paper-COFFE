...
Consider a multilinear form comprising a set of monomials $M$, and let $\Lambda$ be the corresponding finite element integration loop nest. Let $\Gamma$ be the transformation algorithm. Let $X$ be the set of monomials that $\Gamma$ establishes to pre-evaluate, and let $Y = M \setminus X$. Assume that the pre-evaluation of different monomials does not result in identical tables. Then, $\Lambda' = \Gamma(\Lambda)$ is quasi-optimal in the sense of Definition~\ref{def:mln-quasi-optimality}, and satisfies Constraint~\ref{const:TH}.
\end{Prop}
\begin{proof}
...
\end{enumerate}
\item Consider Algorithm~\ref{algo:sharing-elimination}. Because of normal form of expressions, loop linearity and structure of tensor operations, there are only two ways of scheduling the multilinear operands in $P \in \mathbb{P}$: through (i) generalized code motion or (ii) factorization of multilinear symbols. These two strategies lead, respectively, to performing $|P|$ or $|S_P|$ products at every loop iteration. Since (i) is applied if and only if $|P| < |S_P|$ and does not require neither expansion nor factorization, step (3) will not prune any optimal solution from the search space. %; in fact, it simply shifts towards the objective. 

Steps (4)-(5) factorize symbols following the ordering dictated by the sharing graph and its rewrite rules. Recall that rules (A), (B), and (C) are applied in sequence, with applications of rule (C) that may expose further possibilities for (A) and (B). We prove that the application of these rules, in this sequence, represents a step towards quasi-optimality. 
\begin{description}
