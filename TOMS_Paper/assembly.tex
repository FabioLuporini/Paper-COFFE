...
Two issues should be addressed: understanding for which monomials pre-evaluation is profitable; coordinating pre-evaluation with sharing elimination, since the latter inhibits the former. In this section, we tackle the second issue, while we deal with the second in Section~\ref{sec:op_count}. 
...
%TODO add this somewhere: Further, we note that because of the memory constraints discussed in the previous section, sharing elimination and pre-evaluation fully describe the transformation space.
...
Because of symmetries in basis functions, pre-evaluation could produce identical tables. These are mapped to the same temporary. The transformed expression, therefore, can be characterized by sharing. This is why the last two lines of the algorithm, which goal is to eliminate sharing following a procedure as described in Proposition~\ref{prop:optimal-mln}, are transparently applied to all expressions, regardless of whether pre-evaluation was used. The output of the transformation algorithm is as in Figure~\ref{code:loopnest-opt}, assuming the input is still the usual loop nest in Figure~\ref{code:loopnest}. 
...
The operation count for $\Lambda'$ is $\Lambda'_{ops} = \Lambda'_{ops_1} + \Lambda'_{ops_2} = L N O (\sum_{\alpha}^{\# P} p_{\alpha} + I \sum_{\beta}^{\# Z} z_{\beta}$), where $p_{\alpha}$ and $z_{\beta}$ represent operation counts for monomials in $P$ and $Z$, respectively. $\Lambda'_{ops}$ can easily be evinced from Figure~\ref{code:loopnest-opt}.
...
We evaluate the speed-ups achieved by a wide variety of transformation systems over ``original code'' produced by the FEniCS Form Compiler (i.e., no optimizations applied). We analyze the following transformation systems
\begin{itemize}
...
A second observation regards the use of pre-evaluation in \texttt{cfO2}. The fact that the \texttt{cfO2} and \texttt{cfO1} bars match suggests pre-evaluation was found unprofitable and, therefore, rejected. The trend -- although there are quite a few exceptions -- is that as $nf$ increases, pre-evaluation tends not to be performed. This is a consequence of a larger increase factor, as discussed in Section~\ref{sec:op_count}.

A third point concerns autovectorization. The discretizations employed result in inner loops and basis function tables of size multiple of the machine vector length. This, combined with the chosen compilation flags, promotes autovectorization in the majority of code variants. An exception is \texttt{quad} due to the presence of indirection arrays in the generated code. In \texttt{tens}, loop nests are fully unrolled, so the standard loop vectorization is not feasible; manual inspection of the compiled code suggests, however, that block vectorization (\cite{SLP-vect}) is often triggered. In \texttt{ufls}, \texttt{cfO1}, and \texttt{cfO2} the iteration spaces have similar structure (there are a few exceptions in \texttt{cfO2} due to zero-elimination), with loop vectorization being regularly applied, as far as we could evince from compiler reports and manual inspection of assembly code.

Other less relevant aspects are
\begin{itemize}
\item The lack of (or marginal) improvements in all code variants when $q=1$ is due to the cost of matrix insertion, which outweighs that of local assembly.
\item The slow-downs experienced with \texttt{quad} are imputable to the presence of indirection arrays in the generated code; especially with dense tables, trading autovectorization for a relatively small save in floating point operations can be counter-productive (see also Section~\ref{sec:zeros}).
\item If a bar or a subplot is missing, it means that the form compiler failed at generating code because of either exceeding the memory limit of the system or being unable to handle the form.
\end{itemize}

\paragraph{Mass}
We start with the simplest of the bilinear forms that we are investigating, the mass matrix. From analysis of Figure~\ref{fig:mass}, we see that \texttt{cfO2} shows the highest speed-ups. It is worth noting how \texttt{auto} does not select the fastest implementation (out of \texttt{quad} and \texttt{tens}) in some of the test cases: \texttt{auto} always opts for \texttt{tens}, while as $nf \geq 2$ \texttt{quad} tends to be preferable. On the other hand, one can infer that \texttt{cfO2} constantly makes correct decisions about pre-evaluation. 

\paragraph{Helmholtz}
The main message of Figure~\ref{fig:helmholtz} is that \texttt{cfO2} still outperforms the other code generation systems. For $nf=0$, the adoption of pre-evaluation results in increasingly notable speed-ups over \texttt{cfO1} as $q$ increases; \texttt{tens} is comparable to \texttt{cfO2}, with \texttt{auto} making the right choice. With $nf=1$, \texttt{auto} still picks \texttt{tens}, although this time the choice is sub-optimal: this can indirectly be inferred from \texttt{cfO2}, which rejects pre-evaluation for any $q$. For $nf=2$ and $nf=3$, \texttt{auto} reverts to \texttt{quad}, which would theoretically be the right choice (the flop count is much lower than in \texttt{tens} or what would be produced by pre-evaluation); however, the generated code suffers from the presence of indirection arrays, which break autovectorization and ``traditional'' code motion. The sporadic slow-downs or marginal improvements exhibited by \texttt{ufls} are imputable to the presence of sharing. 

An interesting experiment we performed was relaxing the memory threshold by setting it to $T_H = L3_{size}$. We found that this makes \texttt{cfO2} generally slower as $nf \geq 2$, with a maximum slow-down of 2.16$\times$ with $nf=2, q=2$. Presumably, the effects of not having a sensible threshold could even be worse in parallel runs, since the L3 cache is a shared memory.
