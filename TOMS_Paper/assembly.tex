...
As shown later, multilinear loop nests are important because they have a structure that we can take advantage of to synthesize optimal code. We will see that they arise naturally when translating bilinear or linear forms into code. 

Two other classes of loops need be defined. 
...
To contextualize, consider Equation~\ref{eq:quadrature} and the (abstract) loop nest implementing it illustrated in Figure~\ref{code:loopnest}. The imperfect nest $\Lambda=[L_e, L_i, L_j, L_k]$ comprises a free order loop $L_e$ (over elements in the mesh), a reduction loop $L_i$ (performing numerical integration), and a multilinear loop nest $[L_j, L_k]$ (over test and trial functions). In the body of $L_k$ we have one statement evaluating the local tensor for element $e$; more statements may be present if an element is a facet (instead of a cell, as we are assuming here). An expression implements a set of monomials. The expression we are showing is in \textit{normal form}: $m$ is the (problem-dependent) number of ``terms'', $\alpha_{eij}$ ($\beta_{eik}$) represents the product of the inverse Jacobian matrix for the change of coordinates with test (trial) functions, and $\sigma_{ei}$ is a function of coefficients and geometry. Given the rigid structure of monomials (summarized in Section~\ref{sec:monomials}), we observe that any expression from any problem can be reduced to this form. 
...
We emphasize that our aim is a strategy to synthesize optimal implementations of loop nests and expressions of this particular form. In particular, we characterize optimality as follows.
...
\end{itemize}
In the following Sections~\ref{sec:sharing-elimination} and~\ref{sec:pre-evaluation}, we focus on the construction of such transformation space. We consider multilinear loop nests of arity $n=2$ (i.e., bilinear forms), although a similar (simpler) reasoning straightforwardly applies to the case $n < 2$.
...
These transformations minimize the operation count \textit{within} the multilinear loop nest. However, when both of them are applicable, there is no simple way of predicting which one provides the largest decrease in operation count. The difference in operation count tends however to be negligible, since only the outer loops are affected.

\subsubsection{Reduction and free order loop}
\label{sec:se-rln}
Integration- and geometry-dependent expressions should logically be placed, respectively, within the reduction and free order loops. The factorization of $\sigma_{ei}$, therefore, also plays a key role for optimal sharing elimination. This is particularly important in complex forms, characterized by several coefficients, derivatives of coefficients, and extensive use of differential operators. We distinguish two cases:
\begin{itemize}
\item For all terms $w$, there exists a common symbol $f_i \in \sigma_{ei}$ (e.g., a scalar coefficient) such that the normal form reduces to $f_i \sum_{w=1}^m \alpha_{eij}^{w} \beta_{eik}^{w} \eta_{ei}^{w}$ and $\eta_{ei} = \sigma_{ei} \setminus f_i$. 
\item There is no common symbol among all terms. There still can be two terms $w_1$ and $w_2$ such that $f_i \in \sigma_{ei}^{w1}, \sigma_{ei}^{w2}$, but factorizing $f_i$ would only increase the operation count since sharing elimination opportunities across test and trial functions would be lost. On the other hand, sharing elimination must occur \textit{within} each term $\sigma_{ei}^{w}$: factorization of (derivatives of) coefficients promotes reduction-loop invariant code motion. 
\end{itemize}

\subsubsection{Observations and underlying assumption}
\label{sec:se-assumptions}
What we are advocating is the fundamental fact that there never are two ore more conflicting factorizations among which to choose for minimizing the operation count. The idea is that sub-expressions must be placed within the loops that they logically belong to (and not in a deeper level), and this can only be accomplished by systematic factorization and code motion. 

There only are two pathological circumstances in which this approach might not lead to optimality: 
\begin{itemize}
\item As already explained in Section~\ref{sec:se-opln}, having both test and trial functions in the same function space exposes two alternative transformations.
\item The loops degenerate (i.e., their trip count is 1). This makes the factorizations described in Sections~\ref{sec:se-opln} and~\ref{sec:se-rln} sub-optimal since code motion is now irrelevant. For example, the reader can verify that expanding the expression $(a f_i + b g_i) (c f_i + d g_i)$ and factorizing $L_i$-dependent symbols is optimal if and only if the size of $L_i$ is strictly greater than 1 (the sub-expression was excerpted from the multiplication of the Jacobian matrix for the change of coordinates with the derivative of a coefficient).
\end{itemize}

\subsubsection{Algorithm}
Algorithm~\ref{algo:opt} describes how we manipulate the input loop nest and expression for performing sharing elimination. Sections~\ref{sec:se-opln} and~\ref{sec:se-rln} can be seen as a constructive proof of optimality under the (reasonable) assumptions in Section~\ref{sec:se-assumptions}.

\begin{Algo}[Sharing elimination]
\label{algo:opt}
\normalfont
Consider a finite element integration loop nest $\Lambda=[L_e, L_i, L_j, L_k]$. The algorithm starts with reducing the input expression to the normal form $\sum_{w=1}^m \alpha_{eij}^{w} \beta_{eik}^{w} \sigma_{ei}^{w}$. The sub-expressions $\alpha_{eij}^{w} \beta_{eik}^{w}$ are then ``flattened'' (by multiplication expansion) such that all $L_j$- and $L_k$-dependent terms lie on the same level of the expression tree. 

We factorize, in sequence, $L_j$- ($L_k$-) and $L_k$- ($L_j$-) dependent terms. The choice of what we should factorize first impacts the resulting operation count. It depends on two factors: the number of unique terms in each loop, $n$, and the trip counts, $tc$. It is fairly simple to see (and easily demonstrable by contradiction) that by starting from the loop such that $n \cdot tc$ is smallest leads to the maximal operation count reduction. 

Assume, without loss of generality, that we start with $L_j$. Thanks to linearity, each factored product only has one symbol depending on $L_j$ ($L_k$), and this symbol is now unique in the expression. The other sub-expression, independent of $L_j$ ($L_k$), is, by definition, loop-invariant, and therefore hoisted such that redundant computation is avoided. Note that multilinearity ensures deterministic factorization, while perfectness ensures correctness of hoisting. 

For each of the expressions lifted at the level of $L_i$, we factorize $L_i$-dependent terms such that $L_e$-dependent sub-expressions are hoisted. Every time a sub-expression is hoisted, we search and replace all common sub-expressions. This last step is probably practically useless for the actual operation count, since we expect any general-purpose compiler to be able to perform aggressive common sub-expression elimination\footnote{By inspection of assembly code, we have verified that this is actually the case with the Intel compiler v15.2.}, but it helps making the generated code much more readable.
\end{Algo}
...
\label{sec:obsheu}
\begin{itemize}
\item \textit{Sharing elimination.} In Section~\ref{sec:sharing-elimination}, we explained that very small loop trip counts may render certain factorizations sub-optimal. To reach optimality, Algorithm~\ref{algo:opt} needs to backtrack if it detects an increase in operation count after applying sharing elimination at a given loop nest level. Since such increases are, in practice, rare and negligible, the algorithm can be relieved by avoiding these checks.
