...
A loop $L$ defining the iteration space $I$ through the iteration variable $i$, or simply $L_i$, is \textit{linear} if all expressions appearing in the body of $L$ that use $i$ to access some memory locations are linear in $I$.
\end{Def}
...
Sharing in a perfect multilinear loop nest $N = L_{i_{0}, i_{1}, ..., i_{n-1}}$ can always be eliminated.
\end{Prop}
\begin{proof}
...
The synthesis of a multilinear loop nest is \textit{optimal} if the amount of operations performed in the innermost loop is minimal.
\end{Def}
...
\item the process by which multi-invariant sub-expressions are exposed could require transformations like expansion and factorization. Consequently, the save originating from the elimination of the reduction loop could be overwhelmed by a potentially larger increase in the number of operations in the body of \texttt{k} (for example, consider how expansion tends to increase operations).
\end{itemize}

We then review our definition of optimality for generic loop nests as follows

\begin{Def}[Optimality of a loop nest]
The synthesis of a loop nest is \textit{optimal} if, under a set of memory constraints \texttt{C}, the amount of operations performed in the innermost loop is minimal.
\end{Def}
...
The model, or loop nest structure, we are interested in arises in the domain of finite element integration. As explained in Section~\ref{sec:background}, integration occurs locally at the element level. In a weak variational formulation of a problem, a bilinear and a liner forms need be evaluated, typically using a quadrature scheme. The loop nest in Figure~\ref{code:loopnest} is a simplified view of a typical bilinear form implementation. The \texttt{e} loop represents iteration over the elements of a mesh; the \texttt{i} loop derives from using numerical quadrature; the perfect loop nest \texttt{jk} implements the computation over the test and trial functions' degrees of freedom. 

Domain knowledge tells us that in our model, for real-world scenarios, the following holds: 1) $L >>> M, N, O$; that is, even with high order basis functions, the number of elements is typically order of magnitude larger than both the number of quadrature points and degrees of freedom; 2) the loop nest enclosed by the integration loop is always perfect and multilinear (descends from the straightforward implementation of Equation~\ref{quadrature} into a loop nest). Based upon these observations, we now discuss optimality.

\subsection{Operation-minimal synthesis}
