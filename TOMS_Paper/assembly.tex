...
Finite element integration expressions can be reduced to the normal form $\sum_{w=1}^m \alpha_{eij}^{w} \beta_{eik}^{w} \sigma_{ei}^{w}$, where $m$ (the number of ``terms'') depends on the problem, $\alpha_{eij}$ ($\beta_{eik}$) represents the product of the inverse Jacobian matrix for the change of coordinates with test (trial) functions, and $\sigma_{ei}$ is a function of coefficients and geometry. Note that this normal form respects Definition~\ref{def:fem-loopnest}. We use the normal form for the following considerations:

\begin{enumerate}
\item In the special case in which test and trial functions are in the same function space, there exists $w1$ and $w2$ such that $\alpha_{eij}^{w1} = \beta_{eik}^{w2}$. That is, common sub-expressions arise over different iteration spaces ($L_j$ and $L_k$). By hoisting $\alpha$ or $\beta$ at the level of the closest common loop ($L_i$), the operation count is reduced.
\item Let us now consider the general case in which test and trial functions belong to different function spaces. By factorizing, in sequence, any $a_{ij} \in \alpha_{eij}$ and then any $b_{ik} \in \beta_{eik}$ (or vice-versa), sub-expressions independent of one of the outer product loops are exposed (a simplistic example is showed in Figure~\ref{code:multi_loopnest}). Similarly to point 1), such a factorization aims to minimize the operation count ``locally'' at the level of the outer product loop nest. This transformation is independent of the chosen function spaces, but it may not be optimal in case point 1) is applicable (a common sub-expression elimination opportunity is lost). Two observations follow:
\begin{enumerate}
\item In case both 1) and 2) are applicable, there is no simple way of predicting what transformation provides the largest decrease in operation count; 
\item However, the difference between the two tends to be negligible, since the operation count within the outer product loop nest is identical.
\end{enumerate}
\item Factorizing any of the symbols in $\sigma_{ei}$ (e.g., a coefficient, or a coefficient derivative along a certain spatial dimension) also impacts the operation count. We distinguish two cases:
\begin{enumerate}
...
\item There is no common symbol among terms. Then, factorizing any $f_i \in \sigma_{ei}$ \textit{across two terms} $w1$ \textit{and} $w2$ always increases the operation count with respect to points 1) or 2). This is because of two facts. Firstly, the number of operations at every iteration is larger. Secondly, sub-expressions that are not hoistable due to the inherent dependencies on loop indices are exposed. On the other hand, factorizing any $f_i \in \sigma_{ei}$ \textit{within a generic term} $w$ promotes hoisting of geometry-dependent sub-expressions at the level of $L_e$. 
\end{enumerate}
\item Inherent in the previous points is the fundamental fact that there never are two ore more conflicting factorizations among which to choose for maximizing code motion. The fact that expressions result from sequences of inner products and loop nests adhere to a specific structure allows us to define a rigid transformation process leading, in general, to operation count minimization. 
\item There only are a few corner circumstances in which point 4) decades. Briefly:
\begin{enumerate}
\item As already explained, the dichotomy between points 1) and 2).
\item The case in which loops degenerate (i.e., their trip counts is 1), which makes code motion irrelevant and certain factorizations sub-optimal (e.g., the factorization $(a f_i + b g_i) (c f_i + d f_i)$ is optimal if and only if the size of $L_i$ is 1).
\end{enumerate}
\end{enumerate}

These considerations can easily be turned into a constructive proof showing that any loop nest produced by Algorithm~\ref{algo:opt} is optimal if and only if: (i) we restrict, as specified at the beginning of the section, to factorization, code motion and common sub-expression elimination (what we mean by this is also clarified in the next section); (ii) we neglect the pathological cases in point 5) above. 

\begin{Algo}[Sharing elimination]
\label{algo:opt}
\normalfont
Consider a finite element integration loop nest $\Lambda=[L_e, L_i, L_j, L_k]$. The algorithm starts with reducing the input expression to the normal form $\sum_{w=1}^m \alpha_{eij}^{w} \beta_{eik}^{w} \sigma_{ei}^{w}$. The sub-expressions $\alpha_{eij}^{w} \beta_{eik}^{w}$ are then ``flattened'' (by multiplication expansion) such that all $L_j$- and $L_k$-dependent terms lie on the same level of the expression tree. We factorize, in sequence, $L_j$- ($L_k$-) and $L_k$- ($L_j$-) dependent terms. The choice concerning what factorize first, which impacts the resulting operation count, depends on two factors: the number of unique terms in each loop, $n$, and the trip counts, $tc$. It is fairly simple to see (and easily demonstrable by contradiction) that by starting from the loop such that $n \cdot tc$ is smallest leads to the maximal operation count reduction. Assume then we start with $L_j$. Thanks to linearity, each factored product only has one symbol depending on $L_j$ ($L_k$), and this symbol is now unique in the expression. The other sub-expression, independent of $L_j$ ($L_k$), is, by definition, loop-invariant, and therefore hoisted such that redundant computation is avoided. Note that multilinearity ensures deterministic factorization, while perfectness ensures correctness of hoisting. Finally, we factorize $L_i$-dependent terms such that $L_e$-dependent sub-expressions are hoisted, checking that this actually results in reduced operation count (see point 4b) above).
\end{Algo}
