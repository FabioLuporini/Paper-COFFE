...
An outer product loop nest is a perfect multilinear loop nest of arity $n \leq 2$ in which all of the expressions appearing in the body of the innermost loop are summations, and every loop index occurs in every summand.
\end{Def}
...
\begin{itemize}
\item a characterization of the space of legal loop transformations for the class of loop nests considered
...
Note that if we were considering outer product loop nests of arity $n > 2$ we would need a much more complex construction to guarantee optimality. 
\end{proof}
...
\label{sec:mem-const}
We will first consider two examples which motivate the need for memory constraints. Our point of departure is again the bilinear form implementation in Figure~\ref{code:loopnest}. Analogous considerations apply to forms of arbitrary arity.
...
Note that due to the memory constraints imposed in the previous section, sharing elimination and pre-evaluation fully describe the transformation space in which the optimal is searched. We have defined them as high level operations on top of traditional expression-restructuring transformations such as code motion, re-association, and factorization. Other classes of loop transformations, for instance aimed at improving data locality or exposing parallelism, can obviously be neglected since they do not impact the operation count.
...
\end{Prop}
\begin{proof}
We first comment on the assumptions. First, we notice that neither of them is a sufficient condition for non-optimality. In addition: 1) is inherently a pathological case, which we neglect; 2) might actually occur in forms with several monomials, although, in practice, this was never the case in none of the problems that we have investigated (which are more than those studied in this article). Therefore, we also neglect this scenario, which would otherwise require a ``global analysis'' of $M$.

The operation count for $\Lambda'$ is $\Lambda'_{ops} = L N O (\Lambda'_{ops_1} + I \Lambda'_{ops_2})$ where $\Lambda'_{ops_1}$ and $\Lambda'_{ops_2}$ represent the operation counts for $P$ and $Z$, respectively. $L$, $M$, $N$, and $O$ are the trip counts of the corresponding loops.
...
A) By pre-evaluating the monomials in $Z_P$, we clearly have $\Lambda''_{ops_2} = \Lambda'_{ops_2} - I \cdot\delta$, where $\delta > 0$ is a parameter to indicate the reduction in operation count exhibited by the non pre-evaluated monomials. To prove A), we then need to convince ourselves that $\Lambda''_{ops_1} -  I \cdot\delta \geq \Lambda'_{ops_1}$ holds. The only (corner) situation in which this might occur is whenever some of the pre-evaluated monomials in $Z_P$ result in tables identical to those already deriving from $P$. If that were the case, more sharing elimination opportunities would be exposed, which in turn could break the optimality claim. Assumption 1) prevents this situation, so the statement is true. 

B) Let us consider the non-trivial case: there are at least two monomials $p$ and $z$ that share terms, with $p \in P_Z$ and $z \in Z$. We have to consider the fact that there could be (practically rare) circumstances in which despite $\theta_{wo}(z) < \theta_w(z)$ and $\theta_{w}(p) < \theta_{wo}(p)$, we could have $\theta^G_{wo}(z, p)  < \theta_w(p) + \theta_{wo}(z)$, where $\theta^G_{wo}$ is a hypothetical global oracle. The intuition behind this is that there could be a larger potential gain in sharing elimination that goes lost if monomials are inspected in isolation. An extreme example is a summation of mass matrices, in which each monomial is characterized by same test and trial functions, and by a different pre-multiplying coefficient (i.e., known) function. The previously discussed assumption 2) allows us to neglect this possibility, which then makes B) true. 
\end{proof}

\subsection{Cost Function}
\label{sec:op_count}
It remains to tie up one loose end: the definition of the cost function $\theta$. Since $\theta$ is expected to be used by a compiler to drive the transformation algorithm, requirements are simplicity and velocity.

We define $\theta(m) = \theta_{wo}(m) - \theta_{w}(m)$, where $\theta_{wo}$ and $\theta_{w}$ represent the operation counts for a monomial $m$ without and with pre-evaluation, respectively. In the following, we explain how to derive these two values.
...
As opposed to what happens in the FEniCS Form Compiler, in which quadrature and tensor modes are completely separated code modules, COFFEE implements all optimizations by composing ``building-block'' transformations, which we refer to as ``rewrite operators''. This has several advantages. Firstly, extendibility: novel transformations -- for instance, sum-factorization in spectral methods -- could be expressed using the existing operators, or with small effort building on what is already available. Secondly, generality: any sort of codes characterized by (linear, but also non-linear) loop nests with sharing can be optimized (obviously, without guarantees on optimality). COFFEE exploits domain properties, although is not specifically tied to finite element in any way. Thirdly, robustness: the same operators are exploited, and therefore stressed, by different optimization pipelines.

A non-exhaustive list of such operators includes expansion, factorization, re-association, generalized code motion. The rewrite operators can be seen as the COFFEE language. They are flexible: for example, one could ask to factorize constants instead of linear terms. Their implementation is based on manipulation of the abstract syntax tree representing the integration routine. 

\subsubsection{Heuristic Optimization of Non-linear Loops}
\label{sec:heuristic-int-dep}
As a proof-of-concept of our generality claim, we briefly discuss our optimization strategy for non-linear loops. Non-linear loops (expressions) are the natural result of non-linear coefficients in the form. The non-linear expressions logically depend on $L_i$. In complex monomials and/or with coarser discretizations, the operation count can experience a significant increase due to non-linear terms.

Despite the fact that we are not characterizing optimality for this wider class of problems, we still heuristically apply the same reasoning of Sections~\ref{sec:optimal-impl} and~\ref{sec:optimal-synthesis} for (potentially) partial sharing elimination, thus reducing the operation count. This is straightforward in our code generation system by composing rewrite operators. Our strategy is as follows. 
