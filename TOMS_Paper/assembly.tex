...
\item there is at least a non-trivial sub-expression (an addition or a product) depending on $\lbrace L_{i_0}, L_{i_1}, ...L_{i_n} \rbrace \subset \Lambda$ (temporal sharing)
\end{enumerate}
\end{Def}
...
\label{sec:se-rln}
Finite element expressions can be seen as composition of operations between tensors. When multiplying two tensors it often happens that the optimal scheduling strategy is to be searched in a space of size 2. 

To illustrate the concept, we take a matrix-vector product $y_{\Psi \Delta} = A_{\Psi} x_{\Delta}$, in which the entries of $y, A$ and $x$ depend on $\Psi, \Delta \subset \Lambda$. Forms are typically rich of operations of this kind, or equivalent. Let us assume that $y_{\Psi \Delta}$ itself is an operand of a larger scalar-valued expression, so its entries can represent any possible sub-expression in $\lbrace \alpha_{eij}, \beta_{eik}, \sigma_{ei} \rbrace$. For simplicity, assume that $A$ is the inverse Jacobian matrix of a given coefficient and $x$ the gradient of a function in two dimensions, and consider the case $\Psi = \emptyset$, $\Delta = \lbrace L_i \rbrace$. Thus an entry in $y_{L_i}$ is of the form $(a f_i + b g_i) (c f_i + d g_i) \in \sigma_{ei}$. To schedule this expression, we basically have two options:
\begin{enumerate}
\item We leave the expression unmodified in the body of $L_i$; we only assign it to a temporary in the case it is a common sub-expression
\item We notice the presence of spatial sharing. We can re-factorize the expression as $f_i (a + c) + g_i (b + d)$, which promotes temporal sharing and, therefore, code motion. 
\end{enumerate}
The reader can verify that option 2) reduces the operation count with respect to 1) if and only if the size of $L_i$ is strictly greater than $c=1$. One can find this dichotomy in a wide range of variational forms\footnote{In fact, it systematically appears in all of the forms used for experimentation (Section~\ref{sec:perf-results}), and many more.}, obviously with values of $c$ varying with the operational cost inherent in removing spatial sharing. 

In general, given a product of two sub-expressions in a finite element integration loop nest, we observe that whenever there is no ambiguity in the elimination of spatial sharing, the problem of finding an optimal scheduling reduces to the comparison of two alternatives. We note that this condition simply holds if a loop is guaranteed to be linear; in the next section, we will show how take advantage of this property to ensure quasi-optimal sharing elimination. Further, if the $\sigma_{ei}$ expressions are sufficiently regular, then the same approach can be applied to all expressions logically belonging to $L_i$. The regularity of $\sigma_{ei}$ is not as uncommon as it may seem at first glance: for instance, this property holds for many of the tensors that need be evaluated in the complex hyperelastic model analyzed in Section~\ref{sec:perf-results} (e.g., the Cauchy-Green tensor). 


%
%can make the factorization space explode 
%
%On the other hand, the possibly unstructured nature of the 
%
%In absence of further coefficient functions or if their use is such that  So optimal 
%Due to the possibly unstructured nature of the $\sigma_{ei}$ expressions, Clearly, in general, the problem is much more complex 
