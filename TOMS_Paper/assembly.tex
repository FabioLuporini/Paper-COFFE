...
We first notice the lack of improvements when $q=1$. The operation count of \texttt{cfO2} and \texttt{tensor} is roughly 9$\times$ lower than that of the alternatives (tens instead of hundreads operations), but nontheless no gain in execution time is obtained. This is due to the fact that local assembly is memory-bound. For instance, in the simplest case $nf=0$, in addition to the determinant of the Jacobian inverse, \texttt{tens} only requries three unique multiplications to evaluate the local element matrix (26 operations in total), whereas almost a hundred memory accesses are performed. In particular, we have: 12 loads for the vertex coordinates; for each vertex, 1 load to indirectly access its data; for each of the 16 local elements, 4 loads (optimistically) and 1 store to update an entry in the global matrix in compressed sparse row format. This unsurprisingly leads to a very low arithmetic intensity (i.e., $\frac{\text{operation count}}{\text{bytes moved}}$), which collocates the kernels in a memory-bound regime.
...
\begin{figure}
\includegraphics[scale=0.685]{perf-results/elasticity}
