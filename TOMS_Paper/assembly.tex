...
\item Use rewrite rules to restructure the expression. Let $V = \lbrace v^1, ..., v^n \rbrace$ be the vertices adjacent to $s \in S$; the preconditions for the three rewrite rules (A), (B), and (C) are:
\begin{enumerate}[label=(\Alph*)]
\item $A \subsetneq V$ is a set of vertices not standing on cyclic paths, $A \neq \emptyset$
\item $s$ is in a cycle $C$ and has highest degree among all vertices in $C$
\item $\lbrace v^1, ..., v^{n-1} \rbrace$ are leaves of a tree, $p = v^n$ is the parent of $s$ (unless $s$ is the root)
\end{enumerate}
If a precondition is met, we take $s$ and apply Strategy~\ref{strategy:ii} (i.e., we factorize $s$ and perform code motion), and we remove all edges incident to $s$. The preconditions are checked cyclically (first all (A)s, then all (B)s, with (B) that may expose further hits for (A), and finally all (C)s) until no more hits are possible (i.e., until $S = T$).  \\
...
After structuring the sharing graph $G$ in such a way that only flop-decreasing transformations are possible, a set of rewrite rules are applied. We here discuss why applying in order rules (A), (B) and (C) leads to the optimum. 

Rule (A) is the critical one because it coordinates factorization at the junction between a cyclic and an acyclic components. It exploits the observation that a vertex outside of cyclic paths may be ``aggregated'' within one or more ``potentially unavoidable'' factorizations, thus saving on the final number of multiplications. This is quite evident for terminals (for which there is at most one way of being factorized), although not so obvious for vertices of arbitrary degree. Let $a$ be a vertex outside of cyclic paths, and let $V = \lbrace v^1, ..., v^n \rbrace$ be the set of vertices adjacent to $a$. Consider the non-trivial case in which at least one vertex $s \in V$ is within a cycle. If $a$ and $s$ were factorized separately, we could end up with the two sub-expressions $a(...v^1 + ... s + ...v^n)$ and $s(...)$. The spatial sharing induced by $s$ would make the synthesis sub-optimal. Conversely, if $s$ is factorized such that a single sub-expression $s(...a + ...)$ will be produced, the operation count is at worst unchanged.

Rule (B) acts within cycles by iteratively picking the vertex with highest degree. Since the assumption is that there are no more hits for rule (A), the problem here is as simple as selecting the minimum number of vertices whose incident edges would cover the whole cycle. With a very similar concept, rule (C) schedules the remaining (disconnected) sub-trees.
\end{enumerate}
\end{proof}
