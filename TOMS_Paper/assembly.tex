...
{\subfigcapskip = 11pt \subfigure[With spatial sharing]{\label{code:multi_loopnest_a}\lstinputlisting[basicstyle=\footnotesize\ttfamily]{listings/multilinear_loopnest.code}}}
~~~~~
{\subfigcapskip = 11pt \subfigure[With temporal sharing]{\label{code:multi_loopnest_b}\lstinputlisting[basicstyle=\footnotesize\ttfamily]{listings/multilinear_loopnest_int.code}}}
~~~~~
...
\begin{itemize}
\item $|.|$: a generic ``sizeof'' operator, denoting the cardinality of a set or the extent of a loop
\item $c_o$: the cost (i.e., number of operations) of an expression $o$
\item A \textit{multilinear operand} is any $\alpha_{eij}$ or $\beta_{eik}$ in the expression; a \textit{multilinear symbol} is a symbol appearing in a multilinear operand depending on $L_j$ or $L_k$ (e.g., test functions, trial functions, first order derivatives of test functions, etc.)
\end{itemize}

The initialization step consists of a depth-first visit of the expression tree with recursive collection of identical sub-expressions. For example, an expression $(a + a + b + c + a)$ would be transformed into $(3a + b + c)$. 

We perform a second depth-first visit of the expression tree to collect all multilinear operands and partition them into $p$ disjoint sets -- that is $\mathbb{P} = \lbrace P_0, P_1, ..., P_{p-1}\rbrace$ such that $\cup_{i=1}^p P_i = \mathbb{P}$ and $\forall i, j = 0,..., p-1$, $i \neq j$, $P_i \cap P_j = \emptyset$. The multilinear operands in a partition $P_i$ share a set of multilinear symbols, whereas there is no sharing across different partitions. We have to establish:
\begin{enumerate}
\item an ordering for $\mathbb{P}$, such that if $P_i \prec P_j$, then $P_i$ is scheduled before $P_j$, and
\item a scheduling strategy for each $P_i$
\end{enumerate}

We start with point 1). We further partition $\mathbb{P}$ into two disjoint sets $\mathbb{P}^{j}$ and $\mathbb{P}^{k}$, which logically separates partitions based on loop dependency. We then order $\mathbb{P}$ lexicographically, with the position of $P \in \mathbb{P}^j$ given by ${<}|\mathbb{P}^{j}|,\ |L_j| \sum_{o \in P} c_o{>}$ (and analogously for $P \in \mathbb{P}^k$). Intuitively, this simply means that operands appearing more often in the expression are handled first. We will later elaborate on this.

We now tackle point 2). For each partition, we have two options, as already explained in Section~\ref{sec:se-rln}. We recall them briefly: (i) applying generalized code motion, SCSE (if possible), and CCSE; (ii) expanding products to expose spatial sharing, which enables factorization and, in turn, generalized code motion. We adopt the following criteria:
\begin{enumerate}
\item Let $S_P$ be the set of multilinear symbols in $P$. If $|P| < |S_P|$, (i) is applied; if $|P| > |S_P|$, (ii) is applied; if $|P| = |S_P|$, we rely on criterion 2), which attempts to optimize for the outer loops.
\item Let $n_{o}$ and $n_{s}$ denote, respectively, the number of times a multilinear operand $o$ and a multilinear symbol $s$ appear in the input expression. Assume that $P$ depends on $[L_i, L_k]$. The increase in operation count at the level of the outer loops is: if (i) were applied, $|L_i| (|L_k| \sum_{o \in P} c_{o} + \sum_{o \in P} (n_{o} - 1))$; if (ii) were applied, $|L_i| \sum_{s \in P_S} (n_{s} - 1)$. We pick (i) or (ii) such that the increase is minimum. Intuitively, this is a way of estimating how two different factorizations affect the operation count, given the fact (criterion 1) that they both minimize the cost of the innermost loop.
\end{enumerate}

It remains to schedule $L_i$-dependent expressions. We perform a last depth-first visit of the expression tree and, for each of such expressions, we apply sharing elimination following a strategy which is very close to point (2) above. In doing this, we speculatively assume that all expressions result from structured tensor operations. If this were not the case, factorization would be applied greedily (i.e., eventually, some code motion opportunities will be exposed, but this may lead to a local optimum).
\end{Algo}

\subsubsection{Examples}
Figure~\ref{code:poisson} applies Algorithm~\ref{algo:sharing-elimination} to the bilinear form arising from a Poisson equation in 2D. We observe that $P_0 = \lbrace (z_0 a_{ik} + z_2 b_{ik}), (z_1 a_{ik} + z_3 b_{ik})\rbrace$ and $P_1 = \lbrace (z_0 a_{ij} + z_2 b_{ij}), (z_1 a_{ij} + z_3 b_{ij})\rbrace$. Clearly, $\mathbb{P} = \lbrace P_0, P_1 \rbrace$, with $\mathbb{P}^k = \lbrace P_0 \rbrace$ and $\mathbb{P}^j = \lbrace P_1 \rbrace$. A much more complex example, using the hyperelastic model evaluated in Section~\ref{sec:perf-results}, is made available\footnote{Sharing elimination example - application to a hyperelastic model: \url{https://gist.github.com/FabioLuporini/14e79457d6b15823c1cd}}.
\begin{figure}\begin{CenteredBox}
...
~~~~~~~~~~
{\subfigcapskip = 7pt \subfigure[Optimal form]{\label{code:poisson_b}\lstinputlisting[basicstyle=\footnotesize\ttfamily]{listings/poisson_after_se.code}}}
...
We start with an example. Consider again the bilinear form implementation in Figure~\ref{code:loopnest}. We pose the following question: are we able to identify sub-expressions within $F$ for which the reduction induced by $L_i$ can be pre-evaluated, thus obtaining a decrease in operation count proportional to the size of $L_i$, $I$? The transformation we look for is exemplified in Figures~\ref{code:loopnest_red} and~\ref{code:loopnest_nored}; Figure~\ref{code:loopnest_red}, which represents the input, can be seen as a simple instance of the abstract loop nest in Figure~\ref{code:loopnest}.
...
~~~~~~~~~~
{\subfigcapskip = 5pt \subfigure[Pre-evaluated reduction]{\label{code:loopnest_nored}\lstinputlisting[basicstyle=\footnotesize\ttfamily]{listings/loopnest_nored.code}}}
...
In this section, we build a transformation algorithm that, given a memory bound, produces optimal or quasi-optimal finite element integration loop nests. In the perspective of integrating the transformation algorithm with a code generation system, we also discuss an approach that trades optimality for compilation time.
...
\label{sec:proof}
The following proposition states that our approach guarantees quasi-optimality. The proof re-uses most of the concepts and explanations provided throughout the paper.

\begin{Prop}
\label{prop:optimal-approach}
Consider a multilinear form and the corresponding finite element integration loop nest $\Lambda$ implementing it. The form comprises a set of monomials, $M$. Let $P$ be the set of pre-evaluated monomials, and let $Z = M \setminus P$. Assume that pre-evaluating different monomials does not result in identical tables. Then, given the memory Constraints~\ref{const:Le} and~\ref{const:TH}, the transformation algorithm produces a $\Lambda'$ which is quasi-optimal in the sense of Definition~\ref{def:mln-quasi-optimality}.
\end{Prop}
\begin{proof}
The function $\theta$ provides cost or gain of pre-evaluation. $\theta$ retrieves exact values, rather than estimates\footnote{An estimate is used, instead, in~\citeN{Kirby:TC}. The estimate is actually for the whole form, rather than for its monomials.}, so $P$ and $Z$ are constructed correctly. 

Let $\Lambda_{ops}$ denote the operation count (number of additions and products) in $\Lambda$. Let $\Lambda'_I \subset \Lambda'$ be the set of all innermost loops produced by the transformation algorithm (all $L_k$ loops in Figure~\ref{code:loopnest-opt}). We need to show that there is no other synthesis $\Lambda''_{I}$ such that $\Lambda''_{I_{ops}} < \Lambda'_{I_{ops}}$, which holds if and only if
\begin{enumerate}
\item \textit{The coordination of pre-evaluation with sharing elimination is optimal}. This boils down to prove that
\begin{enumerate}
\item \textit{pre-evaluating any $Z_P: Z_P \subseteq Z$ would result $\Lambda''_{ops} > \Lambda'_{ops}$}
\item \textit{not pre-evaluating any $P_Z: P_Z \subseteq P$ would result in $\Lambda''_{ops} > \Lambda'_{ops}$}
\end{enumerate}
~
\begin{enumerate}
\item Let $m_T$ represent the set of tables $T$ resulting from applying pre-evaluation to a monomial $m$. Consider two generic monomials $m_1, m_2 \in M$ and the respective sets of pre-evaluated tables, $m_{T_{1}}$ and $m_{T_{2}}$. In a scenario in which $m_{T_{1}} \cap m_{T_{2}}$, we would have that at least one table would be assigned to the same temporary, with the transformation algorithm skipping a potential global optimum. By assumption, we neglect this possibility. The assumption is sustained by (i) the pathological nature of the scenario, (ii) the complexity inherent in handling it, and (iii) the fact that such a potential global optimum would probably be very close to the retrieved solution.
\item ...
\end{enumerate}

\item \textit{Sharing elimination (Algorithm~\ref{algo:sharing-elimination}) produces loop nests that are at least quasi-optimal.}\\
...
\end{enumerate}
