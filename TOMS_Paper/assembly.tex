...
\item $\lbrace T^1, ..., T^n \rbrace \subset \mathbb{T}$ adjacent to $S$, $n > 1$: apply strategy (ii); $\mathbb{S} = \mathbb{S} \setminus \lbrace T^1, ..., T^n \rbrace$.\\ \textit{Note: this recasts the expression as $S (...T^0 + ... + ...T^n)$}. 
...
%\item $\mathbb{T} = \emptyset$: take $S$ at will such that the number of adjacent non terminals is maximum, and apply strategy (ii). 
...
This observation that, for certain forms and discretizations, hoisting may bring the working set to exceed the size of some level of local memory (e.g. the last level of private cache on a conventional CPU, the shared memory on a GPU) led us to formulating the following \textit{memory constraints}.
...
Note that Constraint~\ref{const:Le} reflects the policy decision that the compiler should not silently consume memory on global data objects. We will carefully analyze both theoretical and practical implications of these memory constraints.
...
The most trivial way of evaluating $\theta_{se}$ and $\theta_{pre}$ is to apply the actual transformations and count the number of operations. If, on one hand, this is tolerable for $\theta_{se}$ (Algorithm~\ref{algo:sharing-elimination} tends to have negligible cost), the overhead would be unacceptable if we applied pre-evaluation -- in particular, symbolic execution -- to all bipartitions analyzed by Algorithm~\ref{algo:gamma}. We then seek an analytic way of determining $\theta_{pre}$.

The first step consists of estimating the \textit{increase factor}, $\iota$. This number captures the increase in arithmetic complexity due to the transformations enabling pre-evaluation. To contextualize, consider the example in Figure~\ref{code:increase_factor}. One can think of this as the (simplified) loop nest originating from the integration of a pre-multiplied mass matrix. The sub-expression \texttt{$f_0$*$b_{i0}$+$f_1$*$b_{i1}$+$f_2$*$b_{i2}$} represents the coefficient $f$ over (tabulated) basis functions (array $B$). In order to apply pre-evaluation, the expression needs be transformed to separate $f$ from all $L_i$-dependent quantities. By product expansion, we observe an increase in the number of $[L_j, L_k]$-dependent terms of a factor $\iota = 3$.
...
If $\iota < I$, we still need to find the number of terms $\rho$ such that $\theta_{pre} = \rho \cdot \iota$. Consider again the mass matrix operator in Figure~\ref{code:increase_factor}. The corresponding monomial is characterized by the dot product of test and trial functions, so trivially $\rho = 1$.  If we instead take the example in Figure~\ref{code:poisson}, we have that $\rho = 3$ after a suitable factorization of basis functions. In general, therefore, $\rho$ depends on both form and discretization employed. To determine this parameter, we re-factorize the expression as per Algorithm~\ref{algo:pre-evaluation} and simply count the terms amenable to pre-evaluation.

\subsection{Formalization}
\label{sec:proof}
The following proposition states that our approach guarantees quasi-optimality. The proof re-uses concepts and explanations provided throughout the paper, as well as the terminology introduced in Section~\ref{sec:se-algo}.

\begin{Prop}
\label{prop:optimal-approach}
Consider a multilinear form comprising a set of monomials $M$, and let $\Lambda$ be the corresponding finite element integration loop nest. Let $\Gamma$ be the transformation algorithm. Let $X$ be the set of pre-evaluated monomials, and let $Y = M \setminus X$. Assume that the pre-evaluation of different monomials does not result in identical tables. Then, $\Lambda' = \Gamma(\Lambda)$ is quasi-optimal in the sense of Definition~\ref{def:mln-quasi-optimality}, and satisfies Constraints~\ref{const:Le} and~\ref{const:TH}.
\end{Prop}
\begin{proof}
We first observe that the cost function $\theta$ predicts the \textit{exact} gain/loss in monomial pre-evaluation, so $X$ and $Y$ can actually be constructed correctly.

Let $c_\Lambda$ denote the operation count for $\Lambda$ and let $\Lambda_I \subset \Lambda$ be the subset of innermost loops (all $L_k$ loops in Figure~\ref{code:loopnest-opt}). We need to show that there is no other synthesis $\Lambda''_{I}$ satisfying Constraints~\ref{const:Le} and~\ref{const:TH} such that $c_{\Lambda''_I} < c_{\Lambda'_I}$. This holds if and only if
\begin{enumerate}
...
\item Let $m_1 \in X$ and $m_2 \in Y$ be two monomials that share some generic multilinear symbols. If $m_1$ were carelessly pre-evaluated, there may be a potential gain in sharing elimination that is lost, potentially leading to a local -- instead of a global -- optimum. This situation is prevented by construction, because $\Gamma$ analyzes all possible bipartitions searching for a global optimum that also preserves the memory constraints. Recall that since the number of monomials is in practice very small, this pass can rapidly be accomplished. Finally, note how this problem can be seen as an instance of the well-known Knapsack problem.
\end{enumerate}
\item Consider Algorithm~\ref{algo:sharing-elimination}. Because of loop linearity and normal form of expressions, there are only two ways of scheduling the multilinear operands in $P \in \mathbb{P}$: through (i) generalized code motion or (ii) factorization of multilinear symbols. These two strategies lead, respectively, to performing $|P|$ or $|S_P|$ products at every loop iteration. Since (i) is side effects free (i.e., neither expansion nor factorization are needed) and is applied if and only if $|P| < |S_P|$, it is clear that step (3) cannot prune any optimal solution from the search space. %; in fact, it simply shifts towards the objective. 
...
\item[Rule (B)] Since $S^{nt}$ is no terminal, there must exist at least one edge between $S^{nt}$ and a generic vertex $V$. Let us consider the alternative factorization $S^{nt}(...S + ... + ...V)$. If $V$ is a terminal, we are back to where we started, because rule (B) will be applicable by setting $V = T$ and $S = S^{nt}$. Otherwise ($V$ non terminal), we have $S$ appearing in two different sub-expressions, which implies sub-optimality because of the presence of spatial sharing.
