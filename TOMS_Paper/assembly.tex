...
\item There exists a complete bipartite sub-graph $\mathbb{B}^d = (\mathbb{S}_j^d, \mathbb{S}_k^d, \mathbb{E}^d)$, with $\mathbb{S}_j$ and $\mathbb{S}_k$ being partitions of symbols depending on $L_j$ and $L_k$, respectively: take an arbitrary symbol in $\mathbb{S}_j^d \cup \mathbb{S}_k^d$ and apply Strategy~\ref{strategy:ii}.
\item $\mathbb{T} = \emptyset$: take $S$ with highest degree and apply Strategy~\ref{strategy:ii}. \end{enumerate}
Go back to step (2).
...
\item[Rule (B)] Since $S^{nt}$ is not a terminal, there must exist at least one edge between $S^{nt}$ and a generic vertex $V$. Let us consider the alternative factorization $S^{nt}(...S + ... + ...V)$. If $V$ were a terminal, we see that setting $V = T$ and $S = S^{nt}$ would simply expose another hit for rule (B). Otherwise ($V$ non terminal), we would have $S$ now appearing in two different sub-expressions ($(...S + ... + ...V)$ and $(...S\ T)$), which would clearly be sub-optimal as a consequence of the spatial sharing induced by $S$. From this, we see that applying rule (C) prior to rule (B) may prune the optimum from the search space.
\item[Rule (C)] We interpret $G$ as a bipartite graph $\mathbb{B} = (\mathbb{S}_j, \mathbb{S}_k, \mathbb{E})$, in which $\mathbb{S}_j$ and $\mathbb{S}_k$ represent the partitions of symbols depending on $L_j$ and $L_k$, respectively. There can be multiple connected components, each of which we denote by $\mathbb{B}^d$. We identify with $lb^d$ the lower bound on the operation count, which we recall is an affine function of the number of products, induced by the symbols in $\mathbb{B}^d$. The factorization in any $\mathbb{B}^d$ is independent of that in other connected components. It therefore suffices to consider an arbitrary $\mathbb{B}^d$. We distinguish the two following possibilities.
\begin{description}
\item[$\mathbb{B^d}$ is complete] We simply have that $lb^d = 1$, which the algorithm reaches through iterative factorization (e.g., $a_j a_k + a_j b_k + b_j a_k + b_j b_k = (a_j + b_j)(a_k + b_k) = t_j t_k$), by jumping back to step (2) at the end of step (6). 
\item[$\mathbb{B^d}$ is not complete] As a consequence of loop linearity and the absence of further hits for rules (A) and (B), $lb^d = {\min(}|\mathbb{S}_j^d|, |\mathbb{S}_k^d|{)}$. 
By not choosing the vertex with highest degree (as the rule states),  ...
\end{description}
\end{description}
The algorithm is guaranteed to terminate because of the jump to step (7) as $\mathbb{S} = \mathbb{T}$. 
\end{enumerate}
\end{proof}
