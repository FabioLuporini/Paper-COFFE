...
\subsection{Transformation Space - Sharing Elimination}
...
\subsection{Transformation Space - Pre-evaluation}
...
\subsection{Completeness of the Transformation Space}
...
Sharing elimination and pre-evaluation allow us to construct an algorithm to explore all ways in which spatial and temporal redundancy can be eliminated from loop nests and expressions as in Definition~\ref{def:fem-loopnest}.
...
We reinforce the idea that eliminating sharing from the outer product loop nest does not suffice. As presaged in Section~\ref{sec:optimal-impl}, we consider whether, and under what conditions, the reduction imposed by $L_i$ could be pre-evaluated, thus reducing the operation count. 

Two issues should be addressed: understanding for which monomials pre-evaluation is profitable; coordinating pre-evaluation with sharing elimination, since the latter inhibits the former. In this section, we tackle the second issue, while we deal with the first one in Section~\ref{sec:op_count}. For this, we start with explaining the transformation algorithm, for which pseudo-code is provided in Figure~\ref{code:intuition}.

\begin{figure}
\begin{lstlisting}[numbers=left, basicstyle=\small\ttfamily, frame=single]
dissect the input expression into monomials
for each monomial $m$:
  if $\theta(m)$ > 0 and memory constraints satisfied:
    extract into a separate loop nest
...
Our choice is to evaluate the impact of pre-evaluation, as number of operations saved or introduced, ``locally''; that is, for each monomial, ``in isolation''. We then introduce the cost function $\theta : M \rightarrow \mathbb{Z}$, where $M$ is the set of monomials in the form. $\theta(m)$ represents the change in operation count if pre-evaluation were applied to monomial $m$: a negative value indicates an increase, whereas a positive value indicates a gain. If pre-evaluation is found profitable (i.e., $\theta(m) > 0$), the sub-expression implementing $m$ is extracted; the surrounding loop nest is suitably ``cloned''; a sequence of transformation steps -- involving expansion, factorization, and code motion -- takes place (details in Section~\ref{sec:codegen}); and, finally, the hoisted code is pre-evaluated using symbolic execution. The result is a set of $n$-dimensional tables (these can be seen as ``slices'' of the reference tensor at the math level), $n$ being the arity of the multilinear form. 
