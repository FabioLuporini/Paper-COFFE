...
From Definition~\ref{def:fem-loopnest} descends the fact that expressions can always be reduced to the form $\sum_{w=1}^m \alpha_{eij}^{w} \beta_{eik}^{w} \sigma_{ei}^{w}$, where $m$ depends on the specific form and discretization, $\alpha_{eij}$ ($\beta_{eik}$) represents the product of the inverse Jacobian matrix with test (trial) functions, and $\sigma_{ei}$ is a function of coefficients and geometry. This particular form has led us to the key observation that, in the space $S$ of all possible sharing elimination alternatives, the optimum $opt$ is almost always reached through a specific strategy. Our approach is therefore as follows: we define families of factorization strategies; we calculate a lower bound ($lb$) for their operation count; we calculate the upper bound ($ub$) of $opt$;  if, as expected, $ub(opt) < lb(S \setminus opt)$, then we know that $opt$ performs optimal sharing elimination. 

The potentially optimal factorization groups together test and trial functions in order to expose sub-expressions independent of at least one loop and, therefore, hoistable. The converse is not true: factorizing $\sigma_{ei}$ (or any of the terms in it) generally inhibits sub-expression hoisting due to dependencies on loop indices. Algorithm~\ref{algo:opt} describes how $opt$ is derived; Algorithm~\ref{algo:se-pick} discuss how we determine the actual optimality of $opt$.
...
If there exists $w_1$ and $w_2$ such that $\sigma_{ei}^{w_1} \neq \sigma_{ei}^{w_2}$, then we also need decide how to schedule operations in $L_i$. For the potential optimum, we factorize $L_i$-dependent terms, which promotes hoisting of $L_e$-dependent sub-expressions.
\end{Algo}

\begin{Algo}[Optimality check of $opt$]
\label{algo:se-pick}
\normalfont
aaa
\end{Algo}
%There are two main possibilities: 1) factorization of $L_i$-dependent terms (i.e., coefficients and their derivatives) followed by code motion of $L_e$-dependent terms; 2) factorization of both $L_i$- and $L_e$-dependent terms, followed by common sub-expression elimination. 
...
Sharing elimination aims to reduce the operation count by applying factorization, code motion and/or common sub-expression elimination. There is a fourth operator impacting the operation count: \textit{reduction pre-evaluation}. In this section, we discuss the role of this operator. We also show that there exists a single point ($\in \overline{PSS}$) in the whole transformation space for which reduction pre-evaluation is legal. 
