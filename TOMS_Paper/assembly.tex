...
\item We explain under what conditions our algorithm leads to optimality. In particular, we show that (i) for a particular class of problems, optimality is reached; (ii) in general, quasi-optimality is at least achieved (i.e., operation count optimal for innermost loops, close-to-optimal for outer loops).
...
Now that we have established the concepts of optimality and finite element integration loop nest, we can elaborate on goals and results of this article. The main objective is a strategy to systematically achieve optimality -- as formalized in Definition~\ref{def:mln-optimality} -- in classes of finite element integration loop nests -- as formalized in Definition~\ref{def:fem-loopnest}. For instance, a class of loop nests may be characterized by the use of only certain differential or tensor operators, or by certain properties in the resulting expressions. What our approach manages to achieve is: (i) optimality whenever the $\sigma_{ei}$  expressions are ``sufficiently regular'';  (ii) quasi-optimality in all other cases. With quasi-optimality we mean that the operation count is minimal in the innermost loop and close to the ideal one in all outer loops.

Our approach needs:
\begin{itemize}
\item a characterization of the space of legal transformations impacting the operation count
\item an algorithm to select points in the transformation space
\end{itemize}
In the following Sections~\ref{sec:sharing-elimination} and~\ref{sec:pre-evaluation}, we focus on the construction of the transformation space and on understanding the conditions for optimality. We will show that whenever an expression (i.e., its monomials) is ``too unstructured'', optimality may not be reached (unless, obviously, using brute-force based approaches). We will consider multilinear loop nests of arity $n=2$ (i.e., bilinear forms), although a similar (simpler) reasoning straightforwardly applies to the case $n < 2$.
...
For instance, Figure~\ref{code:multi_loopnest_a} shows a trivial multilinear loop nest of arity $n=2$ with sharing along dimension $j$ induced by $b_j$. The optimal synthesis for this loop nest is provided in Figure~\ref{code:multi_loopnest_b}. This example illustrates how factorization, besides reducing the number of multiplications required at a given point in the loop nest, can expose code motion opportunities. 

In this section, we study \textit{sharing elimination}, a transformation that aims to minimize the operation count by applying factorization, code motion and (if necessary) common sub-expression elimination to remove sharing. In generic problems, transformations of this sort usually require solving large combinatorial problems; for instance, to evaluate the impact of all possible factorization strategies. However, we show that since we restrict ourselves to a specific class of loops and expressions (described in Section~\ref{sec:lnopt}), we can formulate a simple algorithm ensuring quasi-optimality in general and optimality when expressions have a particular structure (typical of many finite element problems).

\subsubsection{Multilinear loop nest}
\label{sec:se-opln}
In the special case in which test and trial functions are in the same function space, there exists $w_1$ and $w_2$ such that $\alpha_{eij}^{w1} = \beta_{eik}^{w2}$. That is, common sub-expressions arise over different iteration spaces ($L_j$ and $L_k$). Both $\alpha$ and $\beta$ can be hoisted at the level of the closest common loop ($L_i$) and the common sub-expressions be replaced with suitable temporaries. This has been detailed in~\citeN{Luporini}. 

In the general case, instead, test and trial functions belong to different function spaces. By factorizing, in sequence, any test function $a_{ij} \in \alpha_{eij}$ and then any trial function $b_{ik} \in \beta_{eik}$ (or vice-versa), sub-expressions independent of one of the linear loops can be hoisted (a simplistic example is showed in Figure~\ref{code:multi_loopnest}). 

These transformations minimize the operation count \textit{within} the multilinear loop nest. When both of them are applicable, there is no simple way of predicting which one provides the largest decrease in operation count. The difference in operation count, however, tends to be negligible, since only the outer loops are affected.

\subsubsection{Reduction and free order loops}
\label{sec:se-rln}
...
Algorithm~\ref{algo:opt} details the steps for performing sharing elimination on an abstract syntax tree representation of the loop nest. For simplicity, the algorithm neglects the pathological scenario described in Section~\ref{sec:se-opln}, in which the resulting operation count could only be very close to optimality when both test and trial functions are in the same function space. A constructive proof of optimality can be derived taking into account both the algorithm itself and the observations in Sections~\ref{sec:se-opln} and~\ref{sec:se-rln} (in which we have explored the entire factorization space).

\begin{Algo}[Sharing elimination]
\label{algo:opt}
\normalfont
Consider a finite element integration loop nest $\Lambda=[L_e, L_i, L_j, L_k]$ with expressions in the canonical form $\sum_{w=1}^m \alpha_{eij}^{w} \beta_{eik}^{w} \sigma_{ei}^{w}$. 
...
// Find optimum such that memory constraints are honored
...
It remains to find the total number of terms, $\rho$, that will be affected by the increase factor; that is, $\theta_{pre} = \rho \cdot \iota$. Consider again the mass matrix in Figure~\ref{code:increase_factor}. This monomial is characterized by the dot product of test and trial functions (\texttt{$b_{ij}$*$b_{ik}$}), so here, trivially, $\rho = 1$. In general, $\rho$ depends on both form and discretization. As another example, take a standard Poisson equation using P1 Lagrange elements over triangles: we have $\rho = 3$ since there are in total four spatial derivatives and factorizing the expression as explained in Algorithm~\ref{algo:pre-evaluation} leads to 3 integration-dependent terms. We determine $\rho$ by executing parts of Algorithm~\ref{algo:pre-evaluation}, which obviously excludes the reduction pre-evaluation.
