...
\subsubsection{Autovectorization}
...
Another relevant problem instance is $\langle \mathrm{nf}=0,\ q=4 \rangle$, in which \texttt{cfO2} performs 1.5$\times$ more operations than \texttt{tens}, but still exhibits a better execution time. This is again caused by the chosen code generation strategies: while \texttt{tens} prefers to fully unroll the computation to detect all avoidable operations, \texttt{cfO2} preserves the loop structure. This choice mitigates the memory transfer (especially between L1 cache and registers) by maximizing register locality.
...
We also tried to relate the achieved performance to the operation counts obtained by each transformation system. We here limit ourselves to consider the case $\langle \mathrm{nf}=0,\ q=4 \rangle$, for which the speed-ups are generally massive. For the original code, at the source level $37 \cdot 10^9$ operations are counted. However, this number is in practice much lower, since by inspection of the assembly code generated by the underlying compiler, it is clear that common sub-expressions elimination and loop-invariant code motion have been applied in aggressive way. The actual operation count, however, is definitely larger than the $10^8$ operations performed by \texttt{ufls}. \texttt{cfO2} brings the operation count down to $3 \cdot 10^7$; that is, more than 3$\times$ lower than \texttt{ufls}. This is completely in line with the drop in execution times, with \texttt{cfO2} outperforming \texttt{ufls} by slightly more than 3$\times$.

Unsurprisingly, the memory transfer impact is less relevant in this hyperelastic than in all previous forms. With $\langle \mathrm{nf}=0,\ q=1 \rangle$, \texttt{cfO2} is already 1.4$\times$ faster than the untransformed code, and the gain increases with both $nf$ and $q$.
