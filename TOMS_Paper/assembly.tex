...
\item Calculate the number of operations in the innermost loop required for the execution of $P \in \mathbb{P}$. There are two possibilities, depending on which strategy -- (i) or (ii), as described in Section~\ref{sec:se-rln} -- is used. In particular, we have $c_P = {min(}c_P^{i},\ c_P^{ii}{)}$, with $c_P^i = |P|$ and $c_P^{ii} = |S_P|$.
...
\item For all $P \in \mathbb{P}$, determine the profitability of strategy (ii). We introduce $g_P$, which measures the gain in operation count in the innermost loop if (ii) were applied. We have $g_P = \sum_{Q \in \mathbb{Q}} c_{Q_{old}} - c_{Q_{new}}$, with $\mathbb{Q} = \mathbb{P} \setminus \lbrace P \rbrace$. Apply (ii) if $g_P > 0$ and the expansion cost does not exceed the gain in operation count. 

\textit{Note: both $c_{Q_{new}}$ and the expansion cost are easily determined through tree visits.}

\textit{Note: $\mathbb{P}$ is in practice very small, so this pass can be accomplished rapidly.}
\item Perform a depth-first visit of the expression tree and, for each $L_i$-dependent or previously hoisted sub-expression, apply the most profitable between (i) and (ii). 

\textit{Note: this pass speculatively assumes that structured tensor operations are in place. If the assumption does not hold, the result will generally be sub-optimal since only a subset of code motion opportunities may have been exposed.}
\end{enumerate}
\end{Algo}
