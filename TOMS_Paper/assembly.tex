...
In this section, we study \textit{sharing elimination}, a transformation that aims to reduce the operation count by removing sharing through the application of factorization, generalized code motion and (if necessary) common sub-expression elimination. If the objective were reaching optimality and expressions were systematically unstructured, a transformation of this sort would require solving a large combinatorial problem -- for instance to evaluate the impact of all possible factorization strategies. Our sharing elimination strategy, instead, attempts to exploit the structure inherent in finite element integration expressions to guarantee quasi-optimality\footnote{This require coordination with other transformations, as discussed in the following sections. For the moment -- and for ease of treatise -- we neglect this aspect.}, with optimality being achieved whenever stronger conditions hold. Relaxing the problem is essential to produce simple and computationally efficient algorithms -- which, as such, can be integrated with a compiler. 

In Sections~\ref{sec:se-mln} and~\ref{sec:se-rln} we discuss the structural (i.e., inherent in finite element integration) and algebraic (i.e., related to the use of differential and vector operators) properties of our expressions, while in Section~\ref{sec:sec:se-algo} we present the sharing elimination algorithm. We recall that we are assuming a generic finite element integration loop nest $\Lambda=[L_e, L_i, L_j, L_k]$ with expressions in normal form $\sum_{w=1}^m \alpha_{eij}^{w} \beta_{eik}^{w} \sigma_{ei}^{w}$.
...
\begin{enumerate}
\item We leave the expression unmodified in the body of $L_i$; we may want to assign it to a temporary in the case it is a common sub-expression
...
Algorithm~\ref{algo:opt} details the steps for performing sharing elimination on a tree representation of the loop nest. The algorithm is meant to be applied to each statement appearing in the independent basic block of the input finite element integration loop nest.

\begin{Algo}[Sharing elimination]
\label{algo:opt}
\normalfont
The first step consists of a depth-first visit of the expression tree with recursive collection of identical sub-expressions. This is to ensure that the operands of a generic operation are unique. For example, the expression $(a + a + b + c + a)$, in which we have a single n-ary operator \texttt{+}, is transformed into $(3a + b + c)$. 

We then perform a second depth-first visit of the expression tree and we recursively label each node $n$ with a couple ${<}I_{jk},\ cost{>}$, where: $I_{jk}$ represents the intersection of the symbols depending on the multilinear loop nest (i.e., depending on test and trial functions) within $n$, whereas $cost$ denotes the operation count if generalized code motion were applied to all sub-expressions rooted in $n$ (i.e., if they were placed sufficiently far in the loop nest such that redundant computation were avoided). A crucial observation is that $I_{jk}$ is, in practice, always very small (a few units), even if the form is complex and includes several monomials. In addition, its computation can be made extremely efficient by also tracking the union of the symbols rooted in $n$.

On a final depth-first visit, we examine the label of each operator \texttt{+}. If $I_{jk} = \emptyset$ -- that is, there is no spatial sharing -- we just proceed to the parent node. Otherwise, we speculatively assume that structured tensor operations are performed in the sub-tree rooted in \texttt{+}, and compare the operational cost of the two alternatives described in Section~\ref{sec:se-rln} (see points (1) and (2)). Alternative (1) has an operation count equal to $cost$. Alternative (2) requires collecting the shared symbols and estimating the impact of generalized code motion on the new factorization. For this, we need decide the collection order; that is, whether to start with symbols in $I_j$ or $I_k$. It can easily be demonstrated by contradiction that starting with $I_o$ such that $|L_o| |I_o|$ is minimum ($o \in {\lbrace} j, k{\rbrace}, |.|$ represents a generic size operator) leads to the lowest operation count. If found to be the most favorable, alternative (2) is straightforwardly applied; alternative (1) is instead applied only after all operands rooted in \texttt{+} have been examined. 



%If alternative (1) results better, the node \texttt{+} is marked as no longer expandable, which makes the recursion more efficient. Conversely, 




%We then perform a second depth-first visit of the expression tree and we recursively label each node $n$ with a triple ${<}I_{jk},\ U_{jk},\ cost{>}$, where: $I_{jk}$ and $U_{jk}$ represent, respectively, the intersection and the union of the symbols depending on the multilinear loop nest (i.e., depending on test and trial functions) within $n$, whereas $cost$ denotes the operation count if generalized code motion were applied to all sub-expressions rooted in $n$ (i.e., if they were placed sufficiently far in the loop nest such that redundant computation were avoided). A crucial observation is that $I_{jk}$ and $U_{jk}$ are, in practice, always very small (a few units), even if the form is complex and includes several monomials. In addition, we note that $U_{jk}$ is tracked for the sole purpose of making the calculation of the parent $I_{jk}$ sets even faster.
...
% matrix notation, fixed normal form, so no need for fancy algors to do CSE
\end{Algo}
