...
\item Calculate the cost of each $P \in \mathbb{P}$, as $c_P = {min(}|P|,\ |S_P|{)}$. Intuitively, $c_P$ indicates the number of operations induced by $P$ at every innermost loop iteration once sharing elimination will be applied. Take $P$ such that $c_P$ is minimum.
\item Choose the optimal strategy between points (i) and (ii), described in Section~\ref{sec:se-rln}. If $|P| < |S_P|$, (i) is applied. If $|P| > |S_P|$, (ii) is applied. Otherwise, $|P| = |S_P|$ means that (i) and (ii) have the same impact on the innermost loop. However, the increase in operation count in the outer loops, $i_l$, may still be different. In particular, if (i) were applied, $i_l = \sum_{o \in P} (c_{o}^{l} + n_{o} - 1)$, while if (ii) were applied, $i_l = \sum_{s \in S_P} (n_{s} - 1)$. Note that $n_o$ and $n_s$ capture the number of additions required after collection of $o$ and $s$. We then select (i) or (ii) such that $i_l$ is minimum. 
\item If $\mathbb{P} \setminus \lbrace P\rbrace$ is not empty, go back to point 2. Otherwise, step to point 6.
\item Perform a depth-first visit of the expression tree and, for each $L_i$-dependent expression, apply sharing elimination assuming a structured tensor operation is in place (i.e., as in point 4 above). The result may be sub-optimal if this speculative assumption does not hold (i.e., only a subset of code motion opportunities are exposed).
\end{enumerate}
\end{Algo}

\subsubsection{Examples}
Figure~\ref{code:poisson} applies Algorithm~\ref{algo:sharing-elimination} to the bilinear form arising from a Poisson equation in 2D. We observe that $P_k^0 = \lbrace (z_0 a_{ik} + z_2 b_{ik}), (z_1 a_{ik} + z_3 b_{ik})\rbrace$ and $P_j^1 = \lbrace (z_0 a_{ij} + z_2 b_{ij}), (z_1 a_{ij} + z_3 b_{ij})\rbrace$, with $c_{P^0} = c_{P^1} = {min(}2, 2{)} = 2$. A much more complex example, using the hyperelastic model evaluated in Section~\ref{sec:perf-results}, is made available online\footnote{Sharing elimination example - application to a hyperelastic model: \url{https://gist.github.com/FabioLuporini/14e79457d6b15823c1cd}}.
\begin{figure}[h]\begin{CenteredBox}
...
\end{enumerate}
\item Consider Algorithm~\ref{algo:sharing-elimination}. The operation count induced by a partition $P$ in the innermost loop is given by $c_P = {min(}|P|,\ |S_P|{)}$. This descends directly from two facts: the assumption that the input expression is in normal form and multilinearity. Therefore, what we have to prove is the fact that by taking each time $P$ such that $c_P$ is minimum -- henceforth, $P_{min}$ -- leads to quasi-optimality. We do it by contradiction. Let us assume that taking any $P$ such that $c_{P} > c_{P_{min}}$ would lead to a lower operation count. We first observe that a partition (i.e., its multilinear symbols) depends on one of the two multilinear loops; we then indicate with $P^l$ the fact that $P$ depends on $L_l$, $l \in \lbrace j, k\rbrace$. Assuming $P_{min}^l$, we have two possibilities:
\begin{itemize}
