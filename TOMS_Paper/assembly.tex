...
From Definition~\ref{def:fem-loopnest} descends the fact that expressions can always be reduced to the form $\sum_{w=1}^m \alpha_{eij}^{w} \beta_{eik}^{w} \sigma_{ei}^{w}$, where $m$ depends on the specific form and discretization, $\alpha_{eij}$ ($\beta_{eik}$) represents the product of the inverse Jacobian matrix with test (trial) functions, and $\sigma_{ei}$ is a function of coefficients and geometry. This particular form has led us to the key observation that, in the space $S$ including all possible ways of performing sharing elimination, the optimum ($opt$) is almost always reached through a specific sequence of transformations. Our approach is therefore as follows: we define families of factorization strategies; we calculate a lower bound ($lb$) for their operation count; we calculate the upper bound ($ub$) of $opt$;  if, as expected, $ub(\lbrace opt \rbrace) < lb(S \setminus \lbrace opt \rbrace)$, then we know that $opt$ performs optimal sharing elimination. 

The idea behind $opt$ is that by grouping together test and trial functions we expose sub-expressions independent of at least one loop and, therefore, hoistable. The converse is not true: factorizing $\sigma_{ei}$ (or any of the terms in it) generally inhibits code motion due to dependencies on loop indices. Algorithm~\ref{algo:opt} describes how $opt$ is derived. We postpone to Section~\ref{sec:optimal-synthesis} the discussion on how we assess the actual optimality of $opt$.
...
Sharing elimination aims to reduce the operation count by applying three operators: factorization, code motion and/or common sub-expression elimination. There is a fourth operator impacting the operation count: \textit{reduction pre-evaluation}. In this section, we discuss its role and show that there exists a single point in the whole transformation space for which the transformation is legal. 
...
Sharing elimination and reduction pre-evaluation tackle the problem of minimizing spatial and temporal redundancy in the loop nests and expressions of Definition~\ref{def:fem-loopnest}. The space of all possible factorizations is explored, and for each point in this space, code motion, common sub-expression elimination, and reduction pre-evaluation opportunities are analyzed. 

\section{Optimal Synthesis}
\label{sec:optimal-synthesis}

In this section, we build a transformation algorithm capable of reducing finite element integration loop nests to optimal form. A fundamental aspect is the definition of suitable cost functions to ascertain: 1) the profitability of pre-evaluation; 2) the optimality of sharing elimination (recall that Algorithm~\ref{algo:opt} provides, optimistically, a potentially optimal implementation of the transformation).
...
We discuss how we can systematically reach the goal set by Definition~\ref{def:optimality} under Constraints~\ref{const:Le} and~\ref{const:TH} for the class of loop nests in Definition~\ref{def:fem-loopnest}.

At this point, three main issues are still to be addressed: 
\begin{enumerate}
\item Given a monomial, deciding whether to apply pre-evaluation (instead of sharing elimination). Recall from Section~\ref{sec:pre-evaluation} that pre-evaluation can actually increase the operation count because of product expansion. In the following, by saying that pre-evaluation is profitable, we simply mean that the resulting operation count is smaller than that produced through sharing elimination.
\item 
\item Consider a form comprising two monomials $m1$ and $m2$. Assume that pre-evaluation is profitable for $m1$ but not for $m2$, and that $m1$ and $m2$ share at least one term (e.g. some basis functions). If pre-evaluation were applied to $m1$, sharing between $m1$ and $m2$ would be lost. We then need a mechanism to understand what transformation -- pre-evaluation or sharing elimination -- results in the highest operation count reduction. In other words, we seek a strategy for finding the global optimum. 
\end{enumerate}

For the moment, let us assume that we own a cost function $\theta : M \rightarrow \mathbb{Z}$ that, given a monomial $m \in M$, returns the gain/loss of pre-evaluating $m$ in place of sharing elimination. $\theta(m) < 0$ indicates that pre-evaluation is not profitable, with the operation count increasing by $|\theta(m)|$. Similarly for $\theta(m) > 0$. We return to the issue of constructing $\theta$ in Section~\ref{sec:op_count}. Having defined $\theta$, we now show the algorithm driving the transformation process. The pseudo-code is provided in Figure~\ref{code:intuition}.
...
It remains to tie up one loose end: the construction of cost functions to 1) estimate the impact of pre-evaluating monomials (Section~\ref{sec:op_count}); 2) check that the default sharing elimination strategy is optimal (Section~\ref{sec:check-se}). Since the cost functions are expected to be used by a compiler to drive the transformation process, requirements are simplicity and velocity.

\subsubsection{Profitability of pre-evaluation}
\label{sec:op_count}

%TODO : say that avoiding useless pre-evaluation is extremely important !!!!!!!!!!! Performing sharing elimination is fast and easy, pre-evaluationi s not !

We build the cost function $\theta$. According to what established in the previous section, we have $\theta(m) = \theta_{wo}(m) - \theta_{w}(m)$; we recall that $\theta_{wo}$ and $\theta_{w}$ represent the operation counts without and with pre-evaluation. In the following, we explain how to derive these two values.
...
\subsubsection{Optimality of sharing elimination}
\label{sec:check-se}
aaa
