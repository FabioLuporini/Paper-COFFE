...
\item However, the difference between the two tends to be negligible, since the outer product loop nests eventually perform the same number of operations.
\end{enumerate}
\item If we neglect the pathological cases in which the loops degenerate (i.e., when the trip count becomes 1, the case of P0 discontinuous elements), points 1) and 2) always result in reduced operation count.
...
\begin{enumerate}
\item For all terms $w$, there exists a common symbol $f_i \in \sigma_{ei}$ (e.g., a scalar coefficient) such that the normal form can be reduced to $f_i \sum_{w=1}^m \alpha_{eij}^{w} \beta_{eik}^{w} \eta_{ei}^{w}$ and $\eta_{ei} = \sigma_{ei} \setminus f_i$. 
\item There are no common symbols among the terms. Factorizing any $f_i \in \sigma_{ei}$ would then 

\item The starting point of the factorization is the normal form $\sum_{w=1}^m \alpha_{eij}^{w} \beta_{eik}^{w} \sigma_{ei}^{w}$. Since the symbols in $\sigma_{ei}$ are generally not common factors

In this case, the production of non-factorizable, non-hoistable sub-expressions tends to drastically inhibit code motion. In the best case (e.g., there is a single scalar coefficient pre-multiplying the form) this simply reduces to cases 1) or 2). In other words, this is a ``non-natural factorization'', which tries to group potentially non-common factors and does not take advantage of loop dependencies.
\item Point 1) was applied -- for example, we have $a_{ij}(b_{ik}(\sigma'_{ei} + \sigma''_{ei}))$, with $\sigma_{ei} = \sigma'_{ei} + \sigma''_{ei}$ -- and we factorize symbols within $\sigma_{ei}$. Splitting $L_e$-dependent from $L_i$-dependent sub-expressions (through suitable re-factorization) and applying code motion naturally tends to operation count minimization. Similarly to point 3), however, with very low trip counts this strategy may be sub-optimal. For example, consider the following sub-expression, $(a v_i + b w_i) (c v_i + d w_i)$, which is excerpted from a multiplication of the inverse Jacobian matrix for the change of coordinates with the derivative of a vector-valued coefficient. It is true that factorizing $v_i$ and $w_i$ exposes $L_i$-independent sub-expressions, but this may increase the operation count when the size of $L_i$ is 1.
\end{enumerate}
\end{enumerate}

Algorithm~\ref{algo:opt} turns these observations into a sequence of steps leading to optimal sharing elimination.

\begin{Algo}[Sharing elimination]
\label{algo:opt}
\normalfont
Consider a finite element integration loop nest $\Lambda=[L_e, L_i, L_j, L_k]$. The algorithm starts with reducing the input expression to the normal form $\sum_{w=1}^m \alpha_{eij}^{w} \beta_{eik}^{w} \sigma_{ei}^{w}$. The sub-expressions $\alpha_{eij}^{w} \beta_{eik}^{w}$ are then ``flattened'' (by multiplication expansion) such that all $L_j$- and $L_k$-dependent terms lie on the same level of the expression tree. We factorize, in sequence, $L_j$- ($L_k$-) and $L_k$- ($L_j$-) dependent terms. The choice concerning what factorize first, which impacts the resulting operation count, depends on two factors: the number of unique terms in each loop, $n$, and the trip counts, $tc$. It is fairly simple to see (and easily demonstrable by contradiction) that by starting from the loop such that $n tc$ is smallest leads to the maximal operation count reduction. Assume then we start with $L_j$. Thanks to linearity, each factored product only has one symbol depending on $L_j$ ($L_k$), and this symbol is now unique in the expression. The other sub-expression, independent of $L_j$ ($L_k$), is, by definition, loop-invariant, and therefore hoisted such that redundant computation is avoided. Note that multilinearity ensures deterministic factorization, while perfectness ensures correctness of hoisting. Finally, we factorize $L_i$-dependent terms such that $L_e$-dependent sub-expressions are hoisted, checking that this actually results in reduced operation count (see point 4b) above).
\end{Algo}
