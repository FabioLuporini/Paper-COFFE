...
\begin{itemize}
\item optimized quadrature mode, \texttt{quad}
\item tensor mode, \texttt{tens}
...
Figures~\ref{fig:mass},~\ref{fig:helmholtz},~\ref{fig:elasticity}, and~\ref{fig:hyperelasticity}, which will be commented in Section~\ref{sec:perf-results-forms}, are "nested" plots. We refer to the outer plot as the "grid", while the inner are the actual "plots". In a grid, $p$ varies along the horizontal axis and $q$ varies along the vertical axis. The top-left plot in a grid shows the speed up over unmodified code for $[q=1, p=1]$; the plot on its right for $[q=1, p=2]$, and so on. The diagonal of the grid provides the behaviour when test, trial and coefficient functions have same polynomial order, that is $q=p$. A grid can therefore be looked at from different perspectives, which allows us to make structured considerations on the performance achieved. In each plot there are six groups of bars, each group referring to a particular code variant (\texttt{quad, tens, ...}). There are four bars per group: the leftmost bar corresponds to the case $nf = 0$, the one on its right to the case $nf = 1$, and so on. 


\subsection{Performance of Forms}
\label{sec:perf-results-forms}
The first, general observation is that optimality, as we defined it, does not necessarily imply best achievable performance. There are two reasons for this. The first reason is that low-level optimizations can impact the actual runtime. Compare, as an example, \texttt{tens} and \texttt{cfO2} in \texttt{Mass} and \texttt{Helmholtz} when $q=1$. With $p \in [2,3]$, \texttt{tens} tend to slightly outperform \texttt{cfO2} even though pre-evaluation is triggered. With $p=4$, \texttt{tens} is quite faster then \texttt{cfO2}; in particular, in this case \texttt{cfO2} considers pre-evaluation unprofitable. The motivation is the aggressive unrolling performed in \texttt{tens}, which allows to eliminate any operations involving zero-valued entries. In COFFEE we never unroll loops to maximize the chances of autovectorization, which results in considerable speed ups throughout the majority of test cases. The second reason is a direct consequence of our axiomatic modeling of optimality: if we are in a regime as in Section~\ref{sec:heuristic-int-dep}, then other code generation strategies for the integration loop might result in larger performance improvements. This is the primary reason we are studying \texttt{Hyperelasticity}, a form characterized by extensive tensor algebra. Nevertheless, as we will see, we found \texttt{cfO2} to be the best scheme also in this circumstance. 


\paragraph{Mass}
\begin{figure}[t] 
 \makebox[\textwidth][c]{\includegraphics[scale=0.77]{perf-results/mass}}
\caption{The mass matrix performance evaluation.}\label{fig:mass}
\end{figure}
