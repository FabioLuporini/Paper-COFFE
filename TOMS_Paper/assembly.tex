...
\paragraph{Mass}
From analysis of Figure~\ref{fig:mass}, we can see that \texttt{cfO2} represents the best variant among the ones tested, apart from the few points previously discussed. This suggests that the optimality model holds in these simple bilinear forms; that is, \texttt{cfO2} generates loop nests that are optimal from both a theoretical and practical viewpoints. It is worthwhile noting how \texttt{auto} is not capable of selecting the proper synthesis in many of the test cases, especially as $nf \geq 2$. 

\paragraph{Helmholtz}
Figure~\ref{fig:helmholtz} shows that the performance improvements achieved by various code variants over a non-optimized implementation can be significant. We appreciate the fact that the relative order of the employed function spaces plays a key role in the code synthesis choice. Take, for example, the case $[q=3, p=1, nf=2]$. The adoption of pre-evaluation makes a tremendous speed up be achievable by $cfO2$ over the competitors. On the other hand, \texttt{auto} triggers \texttt{quad} (since one monomial in the form has two derivatives and two coefficients, which exceeds the threshold within which \texttt{tens} is selected), which, just like the other quadrature-based variants \texttt{ufls} and \texttt{cfO1}, is sub-optimal. 

\paragraph{Elasticity}
The performance results for the elastic model are displayed in Figure~\ref{fig:elasticity}. The fact that \texttt{auto} opts for \texttt{tens} when $nf=1$ leads, generally, to sub-optimal execution times. This is different than in the case of the Helmholtz equation, in which \texttt{tens} (and, generally, pre-evaluation through \texttt{cfO2}), tends to be the best alternative when a single coefficient is present. With $q=4$ and $nf \in [0, 1]$, pre-evaluation in \texttt{cfO2} is found profitable in terms of operation count, although it is not triggered because of exceeding the working set threshold $T_H$. Running the same experiments with $T_H = \infty$ resulted in significant runtime increases for all values of $p$. Running the same experiments with $T_H = L3_{size}$ resulted in a considerable improvement for $nf = 0$, with \texttt{cfO2} becoming the fastest alternative. Our explanation for this is that by running a sequential program, a process has exclusive access to the memory bandwidth of the system, so pre-fetching minimizes cache misses without hurting concurrent processes (since there are none). 
