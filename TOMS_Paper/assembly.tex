...
A finite element integration loop nest is a loop nest in which appear, in order, an imperfect free order loop, an imperfect (perfect only in special cases), linear or non-linear reduction loop, and a multilinear loop nest whose body is an independent basic block in which all expressions are in normal form.
\end{Def}
...
In this section, we study \textit{sharing elimination}, a transformation that aims to reduce the operation count by removing sharing through the application of expansion, factorization, and generalized code motion. If the objective were reaching optimality and the expressions were systematically unstructured, a transformation of this sort would require solving a large combinatorial problem -- for instance to evaluate the impact of all possible factorizations. Our sharing elimination strategy, instead, attempts to exploit the structure inherent in finite element integration expressions to guarantee quasi-optimality\footnote{This requires coordination with other transformations, as discussed in the following sections. For the moment -- and for ease of exposition -- we neglect this aspect.}, with optimality being achieved if stronger preconditions hold. Setting quasi-optimality, instead of optimality, as primary goal is essential to produce simple and computationally efficient algorithms -- two necessary conditions for integration with a compiler. 
...
\end{enumerate}
The reader can verify that (ii) improves the operation count over (i) independently of the size of $L_i$. In general, however, the optimal choice depends on multiple factors: the loop size, the increase in operation count due to expansion, and the gain in code motion. We regard as \textit{structured} any tensor operation producing expressions for which there is no ambiguity in how to perform spatial sharing elimination -- implying that the optimal operation scheduling is to be determined out of alternatives (i) and (ii). Of notable importance, from the perspective of achieving quasi-optimality, is the fact that all tensor operations involving test and trial functions are structured; this observation is fundamental for understanding the sharing elimination algorithm, described in the next section\footnote{It is also remarkable that structured tensor operations often characterize the \textit{entire} variational form. For instance, they systematically arise in the weak variational form of the complex hyperelastic model analyzed in Section~\ref{sec:perf-results}.}. 


\subsubsection{Algorithm}
\label{sec:se-algo}
Algorithm~\ref{algo:sharing-elimination} describes how we perform sharing elimination assuming as input a tree representation of the loop nest. The algorithm is meant to be applied to each statement appearing in the independent basic block of a finite element integration loop nest. Section~\ref{sec:proof} will elucidate the claims of quasi-optimality and optimality. 
...
\item If $\mathbb{S} = \emptyset$, jump to step (6). Otherwise apply, in order, the following rewrite rules (\textit{precondition}: \textit{action}):
\begin{enumerate}[label=(\Alph*)]
\item $\lbrace T^1, ..., T^n \rbrace \subset \mathbb{T}$ adjacent to $S$, $n > 1$: apply strategy (ii); $\mathbb{S} = \mathbb{S} \setminus \lbrace T^1, ..., T^n \rbrace$.\\ \textit{Note: strategy (ii) recasts the expression as $S (...T^0 + ... + ...T^n)$}. 
...
Let $\theta : M \rightarrow \mathbb{Z}$ be a cost function that, given a monomial $m \in M$, returns the gain/loss achieved by pre-evaluation over sharing elimination. In particular, we define $\theta(m) = \theta_{se}(m) - \theta_{pre}(m)$, where $\theta_{se}$ and $\theta_{pre}$ represent the operation counts resulting from applying sharing elimination and pre-evaluation, respectively. Thus pre-evaluation is profitable for $m$ if and only if $\theta(m) > 0$. We return to the issue of deriving $\theta_{se}$ and $\theta_{pre}$ in Section~\ref{sec:op_count}. Having defined $\theta$, we can now focus on the transformation algorithm (Algorithm~\ref{algo:gamma}).
