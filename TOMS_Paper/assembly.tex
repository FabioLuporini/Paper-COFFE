...
\end{Prop}
\begin{proof}
We first observe that the cost function $\theta$ predicts the \textit{exact} gain/loss in monomial pre-evaluation, so $X$ is constructed correctly.

Let $c_\Lambda$ denote the operation count in $\Lambda$ and let $\Lambda_I \subset \Lambda$ be the subset of innermost loops (all $L_k$ loops in Figure~\ref{code:loopnest-opt}). We need to show that there is no other synthesis $\Lambda''_{I}$ preserving the memory constraints such that $c_{\Lambda''_I} < c_{\Lambda'_I}$. This holds if and only if
\begin{enumerate}
...
\end{enumerate}
\item Consider Algorithm~\ref{algo:sharing-elimination}. Loop linearity is a sufficient (although not necessary) condition for removing spatial sharing through factorization. This induces a lower bound on the innermost loop operation count induced by $P \in \mathbb{P}$, which cannot be less than $c_P = 2{min(}|P|,\ |S_P|{)} - 1$. This observation ensures that step (3), which hoists loop-invariant multilinear operands, never prunes optimal solutions from the search space. Step (4) determines an optimal ordering for the factorization of multilinear symbols and temporaries produced at step (3). In particular, we need to demonstrate that the rewrite rules based on properties of the sharing graph bring to a quasi-optimal synthesis:
\begin{enumerate}[label=(\Alph*)]
...
\item Rule (C) ...
\end{enumerate}
Finally, note that step (5) is irrelevant for quasi-optimality.
