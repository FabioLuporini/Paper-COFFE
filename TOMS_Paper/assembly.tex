...
\item $n_o$: the number of occurrences of a sub-expression $o$ appears in the input expression.
\item \textit{multilinear operand}: any $\alpha_{eij}$ or $\beta_{eik}$ in the expression in normal form;
...
We build a reverse lexicographic ordering for $\mathbb{P}$, in which the value of $P$ is given by ${<} ...,\ \sum_{o \in P} c_o^l{>}$.  ...

%By handling more costly operands first, the operation count achieved by temporal sharing elimination will be minimized.

For each partition, we then need choose the optimal strategy between points (i) and (ii), described in Section~\ref{sec:se-rln}. We adopt the following criteria.
\begin{enumerate}
