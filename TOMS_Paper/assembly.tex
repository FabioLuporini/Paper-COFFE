...
Algorithm~\ref{algo:opt} details the steps for performing sharing elimination on an abstract syntax tree representation of the loop nest. For simplicity, the algorithm neglects the pathological scenario described in Section~\ref{sec:se-mln}, in which the resulting operation count could only be very close to optimality when both test and trial functions are in the same function space. A constructive proof of optimality can be derived taking into account both the algorithm itself and the observations in Sections~\ref{sec:se-mln} and~\ref{sec:se-rln} (in which we have explored the entire factorization space).
