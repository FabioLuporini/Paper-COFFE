...
\item $\mathbb{T} = \emptyset$: take $S$ adjacent to $\lbrace S^1, ..., S^d \rbrace$ such that $d$ is maximum (i.e. $S$ is the vertex with highest degree in $\mathbb{G}$) and apply strategy (ii);  $\mathbb{S} = \mathbb{S} \setminus \lbrace S^1, ..., S^d \rbrace$.
\end{enumerate}
Go back to step (2).
...
\item[Rule (B)] Since $S^{nt}$ is no terminal, there must exist at least one edge between $S^{nt}$ and a generic vertex $V$. Let us consider the alternative factorization $S^{nt}(...S + ... + ...V)$. If $V$ were a terminal, we would simply be back to where we started, because rule (B) would be applicable by setting $V = T$ and $S = S^{nt}$. Otherwise ($V$ non terminal), we would have $S$ appearing in two different sub-expressions, which implies sub-optimality because of the presence of spatial sharing.
\item[Rule (C)] We can interpret $G$ as a bipartite graph $\mathbb{B} = (\mathbb{S}_j, \mathbb{S}_k, \mathbb{E})$, in which $\mathbb{S}_j$ and $\mathbb{S}_k$ represent the partitions of symbols depending on $L_j$ and $L_k$, respectively. For each disconnected component $\mathbb{B^d} = (\mathbb{S}_j^d, \mathbb{S}_k^d, \mathbb{E}^d) \in \mathbb{B}$ we consider two possibilities, namely $\mathbb{B^d}$ is 1) not complete or 2) complete. Case 1) is the simplest: since, at this point, there are no more hits for rules (A) and (B), the lower bound for the operation count (as number of products performed at every loop iteration) is given by ${min(}|\mathbb{S}_j^d|, |\mathbb{S}_k^d|{)}$. This is reachable by taking the vertex with highest degree, as the rule states (note that other choices may break factorization opportunities). In case 2), the lower bound is 1 because factorization can be applied iteratively (e.g., $a_j a_k + a_j b_k + b_j a_k + b_j b_k = (a_j + b_j)(a_k + b_k) = t_j t_k$); this is handled by going back to step (2), at the end of step (5).

% At this point, recall . 
%In order to have $\mathbb{T} = \emptyset$, which would trigger rule (C), it must be the case that ....
\end{description}
Finally, we note that step (6) is irrelevant for quasi-optimality.
...
\item UFLACS: a novel back-end for the FEniCS Form Compiler (whose primary goals are improved code generation and execution times). Referred to as \texttt{ufls} 
