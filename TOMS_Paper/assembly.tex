...
\item There exists a complete bipartite sub-graph $\mathbb{B} = (\mathbb{S}_j, \mathbb{S}_k, \mathbb{E})$, with $\mathbb{S}_j$ and $\mathbb{S}_k$ being partitions of symbols depending on $L_j$ and $L_k$, respectively: apply Strategy~\ref{strategy:ii} to either the symbols in $\mathbb{S}_j$ or $\mathbb{S}_k$ (choice is arbitrary).
\item $\mathbb{T} = \emptyset$, $S$ is in a cycle (i.e., a path starting and ending in $S$ with at least one edge and no repeated edges or vertices): take $S$ and apply Strategy~\ref{strategy:ii}.\\
\textit{Note: the precondition basically discards factorizations that can be ``distributed within larger factorizations''. Further details will be provided in Section~\ref{sec:proof}}.
\end{enumerate}
Go back to step (2).
...
Step (6) applies factorization following the ordering dictated by the sharing graph and its rewrite rules. Non flop-decreasing factorizations are excluded by construction at step (5). We first discuss the individual impact of the rewrite rules; then, we explain why applying them in a certain order represents a step forward towards quasi-optimality.
...
\item[Rule (B)] Since $S^{nt}$ is not a terminal, there must exist at least one edge between $S^{nt}$ and a generic vertex $V$. Let us consider the alternative factorization $S^{nt}(...S + ... + ...V)$. If $V$ were a terminal, we see that setting $V = T$ and $S = S^{nt}$ would simply expose another hit for rule (B). Otherwise ($V$ non terminal), we would have $S$ now appearing in two different sub-expressions ($(...S + ... + ...V)$ and $(...S\ T)$), which would clearly be sub-optimal as a consequence of the spatial sharing induced by $S$. From this, we evince that applying rule (D) prior to rule (B) may prune the optimum from the search space.
\item[Rule (C)] The fact that $\mathbb{B} = (\mathbb{S}_j, \mathbb{S}_k, \mathbb{E})$ is complete implies that a sub-expression can be implemented by only one multiplication at every inner loop iteration. For example, the expression $S^0_j S^0_k + S^0_j S^1_k + S^1_j S^0_k + S^1_j S^1_k$ can first be recast (factorization) as $(S^0_j + S^1_j) (S^0_k + S^1_k)$ and finally (code motion) as $P_j P_k$. Rule (C) will apply Strategy~\ref{strategy:ii} ${\min(}|\mathbb{S}_j^d|, |\mathbb{S}_k^d|{)}$ times, thus reaching the objective. 
\item[Rule (D)] The precondition implies that whatever factorization is chosen, no symbols will be eradicated from the expression. The rule then acts conservatively: the absence of a cycle through $S$ simply implies that $S$ will be hoisted through other applications of Strategy~\ref{strategy:ii}, so there is no need to perform a factorization.
\end{description}

Instances of (A) and (B) represent ``unavoidable'' factorizations, for which one multiplication must be paid regardless of how the applications of Strategy~\ref{strategy:ii} are scheduled. (C), similarly, guarantees a single multiplication for a whole set of symbols. From this, we deduce that what Algorithm~\ref{algo:sharing-elimination} actually has to guarantee is a partial ordering: (A), (B), and (C) can be applied in arbitrary order, whereas (D), as explained above, must be considered only when none of the other rules can be hit. Rule (D) is clearly the most expensive from a computational point of view, because it must prune all choices that will not lead to the optimum. However, this rule becomes relevant only in extremely complex forms, in which extensive use of differential operators is made. 
