...
The memory transfer has sometimes a non-negligible impact, especially when low order polynomials are used. This is more evident in the simpler bilinear forms (e.g., \texttt{Mass}), when the speed-ups over the baseline are limited. The effect of the memory transfer is quantified, to some extent, in the sections below. For a comprehensive analysis of its cost, the reader is invited to refer to~\cite{extrusion-paper}, a study based upon the same Firedrake technology used in this article.
...
A relevant experiment we additionally performed was relaxing the memory threshold by setting $T_H = \operatorname{size}(\mathrm{L3})$. We found that this makes \texttt{cfO2} generally slower for $\mathrm{nf} \geq 2$, with a maximum slow-down of 2.16$\times$ with $\langle \mathrm{nf}=2, q=2\rangle$. This effect could be worse when running in parallel, since the L3 cache is shared and different threads would end up competing for the same resource.
