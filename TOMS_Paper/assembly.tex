...
\lstinputlisting[basicstyle=\small]{listings/loopnest.code}
...
\begin{itemize}
\item contrary to what happens with hoisting in perfect multilinear loop nests, in this case the size of the temporaries would be proportional to the number of non-reduction loops (in the example, \texttt{N$\cdot$O} for \texttt{ijk} sub-expressions, \texttt{L$\cdot$N$\cdot$O} for \texttt{eijk} ones). This might shift the loop nest from a CPU-bound to a memory-bound regime, which might be counter-productive for actual runtime;
...
The synthesis of a loop nest is \textit{optimal} if, under a set of memory constraints \texttt{C}, the amount of operations performed in the innermost loop is minimum.
\end{Def}
...
Eliminating sharing from the multilinear loop nest does not suffice to reach nest optimality. This is because we are considering the whole nest, so the presence of the integration loop $L_i$ makes us wonder whether the reduction it imposes can be pre-evaluated outside of $L_e$, as already suggested in~\ref{def:optimality}, thus reducing the operation count.

To answer this question, we make use of a result in~\cite{Kirby:TC}, the foundation of tensor contraction mode. Essentially, multilinear forms can be seen as sums of monomials, each monomial being an integral over the domain of product (of derivatives) of functions from discrete spaces; such monomials can always be reduced to the product of two tensor, a reference tensor and a geometry tensor. An interpretation of this result at the level of loop nest is that if our input is, conceptually, as in Figure~\ref{code:loopnest}, then we can always dissect \texttt{F} into distinct sub-expressions (one per monomial), refactorize each sub-expression to split constant from $[L_i, L_j, L_k]$-dependent terms, hoist the latter one outside of $L_e$, pre-evaluate the reduction.
