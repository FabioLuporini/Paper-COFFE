...
Since we are running sequential programs, with respect to the memory constraint $C_2$ introduced in Section~\ref{sec:mem-const}, we set $T_H = \infty$; that is, temporaries can occupy an arbitrary amount of memory. This is because a single process that has exclusive access to all of the bandwidth available in the system can count on pre-fetching to minimize cache misses. If, on the other hand, a parallel program were run, then a more sensible value for $T_H$ would be the size of the last level of private cache; this would avoid memory bank conflicts and, therefore, potential drops in runtime. 
