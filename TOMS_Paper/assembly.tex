...
A finite element integration loop nest is a loop nest in which appear, in order, an imperfect free order loop, an imperfect (perfect only in special cases), linear or non-linear reduction loop, and a multilinear loop nest whose body is an independent basic block in which expressions are in normal form.
\end{Def}
...
\item there is at least one non-trivial sub-expression (i.e., at least an addition or a product) that is redundantly executed, as independent of $\lbrace L_{i_0}, L_{i_1}, ...L_{i_n} \rbrace \subset \Lambda$ (temporal sharing)
\end{enumerate}
\end{Def}
...
\label{sec:se-rln}
Finite element expressions can be seen as composition of operations between tensors. We observe that, when multiplying two tensors, it often happens that the optimal scheduling strategy is to be searched in a space of size 2. To illustrate the concept, consider the operation $J^{-1} \nabla x \cdot J^{-1} \nabla x$, with $J^{-1}$ being the inverse Jacobian matrix for the change of coordinates and $x$ a generic two-dimensional vector. One of the resulting sub-expressions will be $(a x^0_i + b x^1_i) (c x^0_i + d x^1_i)$, in which $x^0_i$ and $x^1_i$ represent components of $x$ depending on loop $L_i$. To schedule sub-expressions of this kind, we have two options:
\begin{enumerate}[label=(\roman*)]
\item Using generalized code motion -- not useful in the example as $x$ only depends on $L_i$.
\item Searching for spatial sharing, expanding, and then factorizing the expression to promote temporal sharing -- the expression in the example would be recast as $x^0_i (a + c) + x^1_i (b + d)$, with $(a + c)$ and $(b + d)$ being two hoistable sub-expressions.
\end{enumerate}
The reader can verify that (ii) improves the operation count over (i) independently of the size of $L_i$. In general, however, the optimal choice depends on multiple factors: the loop(s) size, the increase in operation count due to expansion, and the gain in code motion. We regard as \textit{structured} any tensor operation producing expressions for which there is no ambiguity in how to perform spatial sharing elimination -- implying that the optimal operation scheduling is to be determined out of alternatives (i) and (ii). Of notable importance, from the perspective of achieving quasi-optimality, is the fact that all tensor operations involving test and trial functions are structured\footnote{It is also remarkable that structured tensor operations often characterize the \textit{entire} variational form. For instance, they systematically arise in the weak variational form of the complex hyperelastic model analyzed in Section~\ref{sec:perf-results}.}. This observation is fundamental for understanding the sharing elimination algorithm. 
...
\textit{Note: $|P|$ and $|S_P|$ represent the number of products in the innermost loop induced by $P$ if, respectively, strategies (i) or (ii) were applied. We will clarify that this directly descends from loop linearity and the structure inherent in tensor operations.}
...
\item Remove $e = (S^i, S^j) \in \mathbb{E}$ if applying strategy (ii) would cause an increase in operation count (i.e., the elimination of temporal sharing after factorization of $S^i$ or $S^j$ would be outweighed by the expansion cost).\\
...
\textit{Note: this pass speculatively assumes that structured tensor operations are in place. If the assumption does not hold, the result will generally be sub-optimal since only a subset of code motion opportunities may be considered.}
\end{enumerate}
\end{Algo}
...
\end{enumerate}
\item Consider Algorithm~\ref{algo:sharing-elimination}. Because of the normal form of expressions and the structure inherent in tensor operations involving test and trial functions, there are only two ways of scheduling the multilinear operands in $P \in \mathbb{P}$: through generalized code motion (strategy (i)) or factorization of multilinear symbols (achieved through strategy (ii)). If applied, these two strategies would lead, respectively, to performing $|P|$ and $|S_P|$ multiplications at every loop iteration. Since strategy (i) is applied if and only if $|P| < |S_P|$ and does not change the structure of the expression (it requires neither expansion nor factorization), step (3) will not prune optimal solutions from the search space. 
...
Throughout the paper we have reiterated the idea that Algorithm~\ref{algo:gamma} reaches optimality if some preconditions are satisfied by the input variational form. We here discuss such preconditions, in increasing order of complexity.
\begin{enumerate}
\item There is a single monomial and only a specific coefficient, the coordinates field. This is by far the simplest scenario, which requires no particular transformation at the level of the outer loops, so optimality naturally follows.
\item There is a single monomial, but multiple coefficients are present. Optimality is achieved if only structured tensor operations are present (see Section~\ref{sec:se-rln}), since there is no ambiguity in factorization, which is exploited during step (7) of Algorithm~\ref{algo:sharing-elimination}.
\item There are multiple monomials, but no coefficients other than the coordinates field are present, or there is no spatial sharing in coefficients across different monomials. This reduces to cases (1) and (2) above.
\item There are multiple monomials and there is sharing of coefficients across different monomials. We can still in some cases ....
\end{enumerate}
