...
\label{sec:se-algo}
Algorithm~\ref{algo:sharing-elimination} details the steps for performing sharing elimination on a tree representation of the loop nest. The algorithm is meant to be applied to each statement appearing in the independent basic block of the input finite element integration loop nest. It exploits the observation that test and trial functions are always part of structured tensor operations, due to the nature of finite element integration expressions. Section~\ref{sec:proof} will elucidate the claims of quasi-optimality and optimality. 

\begin{Algo}[Sharing elimination]
\label{algo:sharing-elimination}
\normalfont
The initialization step consists of a depth-first visit of the expression tree with recursive collection of identical sub-expressions, to ensure the uniqueness of operands. For example, an expression $(a + a + b + c + a)$ would be transformed into $(3a + b + c)$. 

We perform a second depth-first visit of the expression tree to track all occurrences of multilinear operands (i.e., any $\alpha_{eij}$ or $\beta_{eik}$ in the input expression in normal form).  We create disjoint partitions of multilinear operands such that no spatial sharing is present among them. For each partition, we have two alternatives: (i) applying generalized code motion, SCSE (if possible), and CCSE (if necessary); (ii) expanding the expression to expose spatial sharing and enable a subsequent factorization. These two alternatives basically correspond to points 1) and 2) in Section~\ref{sec:se-rln}. 

Given a partition $P$, to decide between (i) and (ii) we adopt the two following criteria:
\begin{enumerate}
\item We compare the cardinality of $P$ to that of the set of multilinear symbols (e.g., test functions) in $P$. If the former is lower, we use (i); if the latter is lower, we use (ii); if they are identical, we check the next criterion, which attempts to optimize for the outer loops.
\item We know how many times a multilinear operand $o$ and a multilinear symbol $s$ appear in the input expression. Let $n_{o}$ and $n_{s}$ denote these quantities. Let $X$ be the set of multilinear operands, and let $Y$ be the set of multilinear symbols. Let $c_{o}$ be the cost of $o$ (i.e., the number of operations). Assume, for this example, that $P$ depends on $[L_i, L_k]$ (which we know, from Figure~\ref{code:loopnest}, to have extent $M$ and $O$, respectively). We observe that the increase in operation count at the level of the outer loops would be: $M (O \sum_{o \in X} c_{o} + \sum_{o \in X} (n_{o} - 1))$ if (i) were applied; $M \sum_{s \in Y} (n_{s} - 1)$ if (ii) were applied. We then take (i) or (ii) depending on which quantity is minimum.
\end{enumerate}

On a last depth-first visit of the expression tree, we apply point (2) above to all of the $L_i$-dependent expressions. 

%TODO what partition do we take first ? we need a partial ordering here
\end{Algo}
...
Consider a multilinear form and the corresponding finite element integration loop nest $\Lambda$ implementing it. The form comprises a set of monomials, $M$. Let $P$ be the set of pre-evaluated monomials, and let $Z = M \setminus P$. Assume that pre-evaluating different monomials does not result in identical tables. Then, given the memory Constraints~\ref{const:Le} and~\ref{const:TH}, the transformation algorithm produces a $\Lambda'$ which is quasi-optimal (as in Definition~\ref{def:mln-quasi-optimality}). 
\end{Prop}
\begin{proof}
Given a monomial $m$, $\theta(m)$ provides the cost/gain due to pre-evaluating $m$. $\theta(m)$ is \textit{not} an estimate\footnote{An estimate is used, instead, in~\citeN{Kirby:TC}. The estimate is actually for the whole form, rather than for its monomials.}, so $P$ (and $Z$) are constructed correctly. 

Let $\Lambda_{ops}$ denote the operation count (additions, products) in $\Lambda$. Let $\Lambda'_I \subset \Lambda'$ be the set of all innermost loops produced by the transformation algorithm (all $L_k$ loops in Figure~\ref{code:loopnest-opt}). We need to show that there is no other synthesis $\Lambda''_{I}$ such that $\Lambda''_{I_{ops}} < \Lambda'_{I_{ops}}$. For this, we need to show that:
\begin{enumerate}
\item the coordination of pre-evaluation with sharing elimination is optimal. This boils down to show that
\begin{enumerate}
\item pre-evaluating any $Z_P: Z_P \subseteq Z$ would result $\Lambda''_{ops} > \Lambda'_{ops}$
\item not pre-evaluating any $P_Z: P_Z \subseteq P$ would result in $\Lambda''_{ops} > \Lambda'_{ops}$
\end{enumerate}
\item sharing elimination (Algorithm~\ref{algo:sharing-elimination}) produces loop nests that are at least quasi-optimal (again, in the sense of Definition~\ref{def:mln-quasi-optimality})
\end{enumerate}
