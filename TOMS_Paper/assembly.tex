...
We perform a second depth-first visit of the expression tree to collect and partition multilinear operands into $p$ disjoint sets, with $\mathbb{P} = \lbrace P^0, P^1, ..., P^{p-1}\rbrace$. The multilinear operands within a partition share the same set of multilinear symbols, whereas there is no sharing across different partitions. 

We build a reverse lexicographic ordering for $\mathbb{P}$, with the position of $P$ given by ${<}|\mathbb{P}_{l}|,\ \sum_{o \in P} c_o^l{>}$, in which $\mathbb{P}_{l}$ represents the set of partitions depending on loop $L_l$. Intuitively, this simply means that operands appearing more often in the expression are handled first. We will later elaborate on this.

For each partition, we need choose the optimal strategy between the points (i) and (ii) described in Section~\ref{sec:se-rln}. We adopt the following criteria.
\begin{enumerate}
\item Let $S_P$ be the set of multilinear symbols in $P$. If $|P| < |S_P|$, (i) is applied; if $|P| > |S_P|$, (ii) is applied; if $|P| = |S_P|$, we go on to criterion 2), which attempts to optimize for the outer loops.
\item Let $n_{o}$ and $n_{s}$ denote, respectively, the number of times a multilinear operand $o$ and a multilinear symbol $s$ appear in the input expression. The increase in operation count at the level of the outer loops is: if (i) were applied, $\sum_{o \in P_l} (c_{o}^{l} + n_{o} - 1)$; if (ii) were applied, $\sum_{s \in S_P} (n_{s} - 1)$. We then pick (i) or (ii) such that the increase is minimum. Intuitively, this is a way of estimating how two different factorizations affect the operation count, given the fact (criterion 1) that both will eventually minimize the innermost loop cost.
\end{enumerate}

Eventually, $L_i$-dependent expressions are scheduled. We perform a last depth-first visit of the expression tree and, for each of such expressions, we apply sharing elimination following a strategy which is very close to point (2) above. In doing this, we speculatively assume that all expressions result from structured tensor operations. If this were not the case, factorization would be applied greedily (i.e., some code motion opportunities will be exposed, but this may lead to a local optimum).
\end{Algo}
